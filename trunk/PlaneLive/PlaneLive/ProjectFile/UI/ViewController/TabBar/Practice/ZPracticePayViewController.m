...
-(void)innerInit
{
    ...
    self.footerFrame = CGRectMake(0, APP_FRAME_HEIGHT, APP_FRAME_WIDTH, [ZPracticePayFooterView getH]);
...
-(void)setFooterViewShow
{
    ...
    tvFrame.size.height -= footerFrame.size.height;
...
-(void)setChangeCartCount
{
    ...
    __weak UILabel *lbChatCount = nil;
    for (UIBarButtonItem *btnItem in self.navigationItem.rightBarButtonItems) {
        BOOL isBreak = false;
        for (UIView *view in btnItem.customView.subviews) {
            BOOL isViewBreak = false;
            if ([view isKindOfClass:[UIButton class]] && view.tag == 100) {
                for (UIView *item in view.subviews) {
                    if ([item isKindOfClass:[UILabel class]] && item.tag == 100) {
                        lbChatCount = (UILabel *)item;
                        isBreak = true;
                        isViewBreak = true;
                        break;
                    }
    ...
    if (lbChatCount) {
        CGPoint chatCountPoint = [self.view convertPoint:lbChatCount.frame.origin fromView:lbChatCount];
        chatCountPoint = CGPointMake(chatCountPoint.x-30, chatCountPoint.y);
        GBLog(@"chatCountPoint: %lf, %lf", chatCountPoint.x, chatCountPoint.y);
        __weak UIButton *btnChat = lbChatCount.superview;
        CGPoint addChatPoint = [self.view convertPoint:self.viewFooter.btnJoin.frame.origin fromView:self.viewFooter.btnJoin];
        addChatPoint = CGPointMake(addChatPoint.x-20, addChatPoint.y+10);
        GBLog(@"addChatPoint: %lf, %lf", addChatPoint.x, addChatPoint.y);
        
        [ZAddCartAnimationLayer addCartAnimationWithView:self.view startPoint:addChatPoint endPoint:chatCountPoint completion:^(BOOL finished) {
            [lbChatCount setHidden:modelU.shoppingCartCount==0];
            if (modelU.shoppingCartCount > kNumberMaxCount) {
                [lbChatCount setText:[NSString stringWithFormat:@"%d", kNumberMaxCount]];
            } else {
                [lbChatCount setText:[NSString stringWithFormat:@"%d", modelU.shoppingCartCount]];
            }
            [ZAddCartAnimationLayer addCartViewScale:btnChat];
            [self.viewFooter setViewDataWithModel:self.model];
        }];
    }
}
