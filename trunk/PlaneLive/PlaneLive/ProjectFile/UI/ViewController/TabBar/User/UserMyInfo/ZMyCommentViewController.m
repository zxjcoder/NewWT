...
-(void)setRefreshCommentData:(BOOL)isRefresh
{
    ...
    [snsV1 getMyCommentArrayWithUserId:self.modelUB.userId pageNum:self.pageNum resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [snsV1 getMyCommentArrayWithUserId:self.modelUB.userId pageNum:self.pageNum+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
