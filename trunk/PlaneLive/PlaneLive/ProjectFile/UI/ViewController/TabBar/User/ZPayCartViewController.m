...
-(void)innerInit
{
    ...
    tvFrame.size.height -= [ZPayCartFooterView getH];
    ...
    self.viewFooter = [[ZPayCartFooterView alloc] initWithFrame:CGRectMake(0, APP_FRAME_HEIGHT-[ZPayCartFooterView getH], APP_FRAME_WIDTH, [ZPayCartFooterView getH])];
...
-(void)setBuyClick
{
    ...
    if ([AppSetting getAutoLogin] && ![[AppSetting getUserId] isEqualToString:kUserAuditId]) {
        ...
        ZWEAKSELF
        ZAlertPayView *viewPlayTool = [[ZAlertPayView alloc] initWithTitle:kJoinedPayCartDePractice];
        [viewPlayTool setOnBalanceClick:^{
            ...
            [snsV2 getGenerateOrderClearingWithMoney:[NSString stringWithFormat:@"%.2f", totalPrice] type:WTPayTypePractice objid:[arrObjIds toString] title:kEmpty payType:(WTPayWayTypeBalance) resultBlock:^(ModelOrderWT *model, NSDictionary *dicResult, NSDictionary *result) {
                [snsV2 updOrderStateWithOrderId:model.order_no resultBlock:^(NSDictionary *result) {
                    ...
                    [ZAlertPromptView showWithMessage:kCMsgPurchaseSuccess];
                    ///发送购物车清空通知
                    [[NSNotificationCenter defaultCenter] postNotificationName:ZCartPaySuccessNotification object:kOne];
                } errorBlock:^(NSString *msg) {
                    [ZProgressHUD dismiss];
