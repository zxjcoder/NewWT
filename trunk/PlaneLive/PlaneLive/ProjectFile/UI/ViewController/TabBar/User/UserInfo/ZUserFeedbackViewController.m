...
-(void)innerNavRight
{
    ...
    [self.btnRecord setTitle:@"反馈记录" forState:(UIControlStateNormal)];
    [[self.btnRecord titleLabel] setFont:[UIFont systemFontOfSize:kFont_Default_Size]];
    [self.btnRecord setTitleColor:COLORCONTENT1 forState:(UIControlStateNormal)];
    [self.btnRecord setContentHorizontalAlignment:(UIControlContentHorizontalAlignmentRight)];
    self.btnRecord.frame = CGRectMake(0, 0, 65, 44);
    [self.btnRecord addTarget:self action:@selector(btnRecordClick) forControlEvents:(UIControlEventTouchUpInside)];
    
    self.lbRecord = [[ZLabel alloc] initWithFrame:(CGRectMake(self.btnRecord.width-13, 1, 20, 20))];
    [self.lbRecord setBackgroundColor:COLORCOUNTBG];
    [self.lbRecord setTextColor:WHITECOLOR];
    [self.lbRecord setViewRoundNoBorder];
    [self.lbRecord setText:@"0"];
    [self.lbRecord setHidden:true];
    [self.lbRecord setTextAlignment:(NSTextAlignmentCenter)];
    [self.lbRecord setFont:[UIFont systemFontOfSize:kFont_Min_Size]];
    [self.lbRecord setUserInteractionEnabled:false];
    [self.btnRecord addSubview:self.lbRecord];
    
    self.navigationItem.rightBarButtonItem = nil;
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:self.btnRecord];
}
-(void)btnRecordClick
{
    if (kIsNeedLogin) {
...
-(void)setRecordCount
{
    ...
    self.lbRecord.hidden == modelU.myFeedbackReplyCount==0;
    [self.lbRecord setText:[NSString stringWithFormat:@"%d", modelU.myFeedbackReplyCount]];
}
-(void)innerData
{
    ModelUser *modelU = [AppSetting getUserLogin];
