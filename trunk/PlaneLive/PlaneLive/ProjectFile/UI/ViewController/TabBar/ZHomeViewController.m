...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(setNetWorkChange) name:@"statusNetworkChange" object:nil];
}
...
-(void)setViewNil
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"statusNetworkChange" object:nil];
...
-(void)innerInit
{
    ...
    CGRect tvFrame = CGRectMake(0, APP_TOP_HEIGHT, APP_FRAME_WIDTH, APP_FRAME_HEIGHT-APP_TABBAR_HEIGHT-APP_TOP_HEIGHT);
...
{
    if (self.viewMain) {
        int status = [AppDelegate app].statusNetwork;
        GBLog(@"statusNetwork: %d", status);
        GBLog(@"restrictedState: %d", self.cellularData.restrictedState);
        if (status == 0) {
            [self.viewMain setShowNoNetwork:self.cellularData.restrictedState==kCTCellularDataRestricted];
        } else {
            [self.viewMain setShowNoNetwork:false];
        }
    }
}
...
-(void)setInnerEvent
{
    ...
    [self.viewMain setOnGoNoNetworkClick:^{
...
-(void)setBannerEvent:(ModelBanner *)model
{
    ...
    switch (model.type) {
        case ZBannerTypePractice://实务
        {
            ...
            [snsV2 getPracticeDetailWithPracticeId:model.code resultBlock:^(ModelPractice *resultModel) {
    ...
    if (row < array.count) {
        ...
        if ([[modelPlay.ids stringByAppendingString:kMultipleZeros] integerValue] == itemVC.modelTrack.trackId) {
            if ([ZPlayerViewController sharedSingleton].isPlaying) {
                ...
                [sqlite setLocalPlayListPracticeWithArray:array userId:kLoginUserId];
            }
        } else {
            [itemVC setInnerPlayTabbarWithType:(ZPlayTabBarViewTypePractice)];
            [itemVC setRawdataWithArray:array index:row];
