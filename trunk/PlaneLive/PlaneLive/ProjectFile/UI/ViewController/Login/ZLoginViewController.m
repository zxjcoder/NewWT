...
-(void)innerInit
{
    ...
    self.tvcSpace.selectionStyle = UITableViewCellSelectionStyleNone;
    ...
    self.mainFrame = VIEW_ITEM_FRAME;
    self.viewMain = [[ZView alloc] initWithFrame:self.mainFrame];
    [self.viewMain setBackgroundColor:WHITECOLOR];
    [self.view addSubview:self.viewMain];
    
    _tvFrame = self.viewMain.bounds;
...
//    [self.view addSubview:imageBG];
    ...
    if ([Utils isWXAppInstalled]) {
        self.btnWeChat = [ZButton buttonWithType:(UIButtonTypeCustom)];
        self.btnWeChat.userInteractionEnabled = true;
        [self.btnWeChat addTarget:self action:@selector(btnWeChatClick) forControlEvents:(UIControlEventTouchUpInside)];
        CGFloat wechatW = 60;
        CGFloat wechatH = 70;
        [self.btnWeChat setFrame:(CGRectMake(APP_FRAME_WIDTH/2-wechatW/2, APP_FRAME_HEIGHT-30-wechatH, wechatW, wechatH))];
        ...
        [self.btnWeChat addSubview:imageWechat];
        
        UILabel *lbWechat = [[UILabel alloc] initWithFrame:(CGRectMake(-5, self.btnWeChat.height-25, self.btnWeChat.width+10, 20))];
        ...
        [self.btnWeChat addSubview:lbWechat];
...
//    [self.view sendSubviewToBack:imageBG];
    ...
    [self.btnClose setFrame:(CGRectMake(0, 10, 45, 45))];
    [self.btnClose addTarget:self action:@selector(btnLeftClick) forControlEvents:(UIControlEventTouchUpInside)];
    [self.navigationItem setTitleView:self.btnClose];
