...
@interface ZWebContentViewController ()<ZWebViewDelegate, DownloadManagerDelegate, UIScrollViewDelegate>
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [StatisticsManager eventIOBeginPageWithName:kZhugeIOPageCoursewareKey];
}
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [StatisticsManager eventIOEndPageWithName:kZhugeIOPageCoursewareKey dictionary:nil];
}
...
-(void)innerDataCurriculumContentWithType:(int)type
{
    ...
    [snsV2 getCurriculumDetailWebUrlWithCurriculumId:self.modelC.ids subscribeId:self.modelC.subscribeId type:type resultBlock:^(NSString *webUrl, NSDictionary *result) {
        if (![webUrl isKindOfClass:[NSNull class]] && [webUrl isKindOfClass:[NSString class]]) {
            ...
            [weakSelf.viewBackground setViewStateWithState:(ZBackgroundStateError)];
        }
...
-(void)setRewardSendBuy:(NSString *)price
{
    if ([AppSetting getAutoLogin] && [[AppSetting getUserId] isEqualToString:kUserAuditId]) {
        ...
        NSString *rewardname = self.modelTrack.announcer.nickname;
...
-(void)setStartDownload
{
    if ([AppSetting getAutoLogin] && ![[AppSetting getUserId] isEqualToString:kUserAuditId]) {
        ...
        GBLog(@"setStartDownload: %d------0:已加入下载队列；1:已下载完成；2:URL无效；4:正在下载；5:数据库错误；9:此音频不允许被下载；10:已注册但不立即下载；21:此音频正在下载中；22:此音频已下载", (int)status);
    } else {
        [self showLoginVC];
    }
}
...
-(void)sendMessageSuccess:(NSDictionary *)dicResult
{
    if (dicResult) {
        ...
        [dicComment setObject:messageId forKey:@"id"];
        [dicComment setObject:content forKey:@"content"];
        [dicComment setObject:userId forKey:@"user_id"];
        [dicComment setObject:nickname forKey:@"nickname"];
        [dicComment setObject:headImg forKey:@"head_img"];
        [dicComment setObject:sign forKey:@"sign"];
        
        NSString *script = [NSString stringWithFormat:@"wutonglive_set('addcomment', '%@')", dicComment.mj_JSONString];
...
-(void)webViewDidFinishLoad:(ZWebView *)webView
{
    ...
    [webView evaluateJavaScript:@"document.body.scrollHeight" completionHandler:^(id _Nullable result,NSError * _Nullable error) {
        GBLog(@"document.body.scrollHeight: %@", result);
        if (result && [result isKindOfClass:[NSString class]]) {
            CGFloat webViewHeight = [result doubleValue]+[ZGlobalPlayView getH];
            webView.scrollView.contentSize = CGSizeMake(webView.width, webViewHeight);
        }
    }];
    //设置图片事件函数
...
{
    [self.viewBackground setViewStateWithState:(ZBackgroundStateError)];
}
