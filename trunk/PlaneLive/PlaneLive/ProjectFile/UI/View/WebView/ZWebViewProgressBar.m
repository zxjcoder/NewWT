...
#import "ZWebViewProgressBar.h"

NSString *completeRPCURLPath = @"/viewprogressproxy/complete";
...
@implementation ZWebViewProgressBar
...
- (BOOL)webView:(ZWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if (request.URL.fragment) {
        NSString *nonFragmentURL = [request.URL.absoluteString stringByReplacingOccurrencesOfString:[@"#" stringByAppendingString:request.URL.fragment] withString:kEmpty];
...
- (void)webViewDidFinishLoad:(ZWebView *)webView
{
    ...
    if (interactive) {
        ...
        NSString *waitForCompleteJS = [NSString stringWithFormat:@"window.addEventListener('load',function() { var iframe = document.createElement('iframe'); iframe.style.display = 'none'; iframe.src = '%@://%@%@'; document.body.appendChild(iframe);  }, false);",
                                       webView.originRequest.mainDocumentURL.scheme,
                                       webView.originRequest.mainDocumentURL.host,
                                       completeRPCURLPath];
        [webView stringByEvaluatingJavaScriptFromString:waitForCompleteJS];
    }
    
    BOOL isNotRedirect = self.currentURL && [self.currentURL isEqual:webView.originRequest.mainDocumentURL];
...
- (void)webView:(ZWebView *)webView didFailLoadWithError:(NSError *)error
{
    ...
    if (interactive) {
        ...
        NSString *waitForCompleteJS = [NSString stringWithFormat:@"window.addEventListener('load',function() { var iframe = document.createElement('iframe'); iframe.style.display = 'none'; iframe.src = '%@://%@%@'; document.body.appendChild(iframe);  }, false);", webView.originRequest.mainDocumentURL.scheme, webView.originRequest.mainDocumentURL.host, completeRPCURLPath];
        [webView stringByEvaluatingJavaScriptFromString:waitForCompleteJS];
    }
    
    BOOL isNotRedirect = _currentURL && [_currentURL isEqual:webView.originRequest.mainDocumentURL];
    BOOL complete = [readyState isEqualToString:@"complete"];
