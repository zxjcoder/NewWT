...
-(void)initButton
{
    if (!self.btnLogin) {
        CGFloat btnW = 120;
        CGFloat btnH = 36;
        self.btnLogin = [[ZShadowButtonView alloc] initWithFrame:(CGRectMake(0, 0, btnW, btnH))];
        ZWEAKSELF
        [self.btnLogin setOnButtonClick:^{
            [weakSelf btnLoginClick];
        }];
        [self.btnLogin setButtonTitle:@"立即登录"];
...
//        [self.btnLogin setBackgroundImage:[SkinManager getImageWithName:@"btn_gra3"] forState:(UIControlStateNormal)];
//        [self.btnLogin setBackgroundImage:[SkinManager getImageWithName:@"btn_gra3_c"] forState:(UIControlStateHighlighted)];
...
-(void)setViewFrame
{
    ...
    if (self.btnLogin) {
        CGFloat btnW = self.btnLogin.width;
        CGFloat btnH = self.btnLogin.height;
        [self.btnLogin setFrame:CGRectMake(self.width/2-btnW/2, self.lbTitle.y+self.lbTitle.height+20, btnW, btnH)];
