...
-(CGFloat)setCellDataWithModel:(ModelCollection *)model
{
    ...
    ///主讲人
    if (model.stitle.length > 0) {
        [self.lbLecturer setText:[NSString stringWithFormat:@"%@ %@",  model.speaker, model.stitle]];
    } else {
        [self.lbLecturer setText:model.speaker];
    }
    [self.lbPlayTime setText:[NSString stringWithFormat:@"%@: %@", kTimeDuration, [Utils getMinuteFromSS:model.play_time]]];
