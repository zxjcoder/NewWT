...
//    [deleteUtilityButtons sw_addUtilityButtonWithColor:DELECOLOR title:kCancelCollection];
...
-(CGFloat)setCellDataWithModel:(ModelCollection *)model
{
    ...
    if (model) {
        ...
        switch ([model.type integerValue]) {
            case 0:
                [self.lbName setText:kTheListOfPerformanceOfALawFirm];
                [self.imgPhoto setImageURLStr:model.img placeImage:[SkinManager getImageWithName:@"mymark_icon_rank"]];
                break;
            case 1:
                [self.lbName setText:kAccountingFirmPerformanceList];
                [self.imgPhoto setImageURLStr:model.img placeImage:[SkinManager getImageWithName:@"mymark_icon_rank"]];
                break;
            case 2:
                [self.lbName setText:kListOfSecuritiesCompaniesPerformance];
                [self.imgPhoto setImageURLStr:model.img placeImage:[SkinManager getImageWithName:@"mymark_icon_rank"]];
                break;
            case 3:
