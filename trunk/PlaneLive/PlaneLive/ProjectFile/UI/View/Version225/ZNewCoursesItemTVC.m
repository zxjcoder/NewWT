...
{
    [self.viewListen setHidden:true];
    [self.lbListen setHidden:true];
    [self.imgListen setHidden:true];
}
...
-(CGFloat)setCellDataWithCurriculumModel:(ModelCurriculum *)model
{
    ...
    [self.imgLogo setImageURLStr:model.audio_picture];
    [self.lbTitle setText:model.ctitle];
    [self.lbLecturer setText:model.speaker_name];
    [self.lbPlayTime setText:model.create_time];
    if (model.hits > kNumberMaximumCount) {
        [self.lbListen setText:[NSString stringWithFormat:@"%.1f%@", (float)(model.hits/(float)(kNumberMaximumCount+1)), kMillion]];
    } else {
        [self.lbListen setText:[NSString stringWithFormat:@"%ld", model.hits]];
