...
@implementation ZNewPracticeItemTVC
...
-(void)innerInitItem
{
    ...
    self.cellH = [ZNewPracticeItemTVC getH];
    ...
    self.imgListen = [[ZImageView alloc] init];
    ...
    // 2.2.7版本不需要这个了
    [self.lbUpdateTime setHidden:true];
    [self.lbPraise setHidden:true];
}
...
-(void)setViewFrame
{
    ...
    self.imgListen.image = [SkinManager getImageWithName:@"listen_icon2"];
...
//    CGFloat lecturerMaxH = [[ZCalculateLabel shareCalculateLabel] getALineWidthWithFont:self.lbLecturer.font height:self.lbMinH text:self.lbLecturer.text];
...
-(void)setHiddenTime
{
    ...
    self.cellH = [ZNewPracticeItemTVC getMinH];
}
///隐藏分割线
...
-(CGFloat)setCellDataWithModel:(ModelPractice *)model
{
    ...
    if (model.increasedCourseCount > 0) {
        [self.lbPrice setHidden:false];
        [self.lbPrice setText:@"更新"];
        [self.lbPrice setTextColor:COLORCONTENT3];
        [self.lbPrice setFont:[ZFont systemFontOfSize:kFont_Small_Size]];
    } else {
        [self.lbPrice setTextColor:COLORCONTENT3];
