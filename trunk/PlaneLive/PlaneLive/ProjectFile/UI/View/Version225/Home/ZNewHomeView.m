...
///广告栏
@property (strong, nonatomic) ZBannerCardView *viewBannerCard;
///工具栏
@property (strong, nonatomic) ZHomeTooolView *viewTool;
///无网络状态
@property (strong, nonatomic) ZNoNetworkView *viewNoNetwork;
///每日推荐
...
-(void)innerInitItem
{
    ...
    CGFloat bannerHeight = [ZBannerCardView getViewH];
    self.viewHeader = [[ZView alloc] init];
    [self.viewHeader setBackgroundColor:WHITECOLOR];
    [self.viewMain addSubview:self.viewHeader];
    CGFloat viewSpace = 1;
    CGFloat bannerY = 16;
    self.viewBannerCard = [[ZBannerCardView alloc] initWithFrame:(CGRectMake(0, bannerY, self.width, bannerHeight))];
    [self.viewHeader addSubview:self.viewBannerCard];
...
//    [self.viewHeader addSubview:self.cycleScrollView];
    ...
    self.viewNoNetwork = [[ZNoNetworkView alloc] initWithFrame:(CGRectMake(0, self.viewTool.y+self.viewTool.height+viewSpace, self.viewMain.width, [ZNoNetworkView getViewH]))];
    [self.viewMain addSubview:self.viewNoNetwork];
    
    CGFloat itemDefault = [ZNewHomeButtonView getH];
    self.viewRecommend = [[ZNewHomeRecommendView alloc] initWithFrame:(CGRectMake(0, self.viewNoNetwork.y+self.viewNoNetwork.height+viewSpace, self.viewMain.width, itemDefault))];
    [self.viewMain addSubview:self.viewRecommend];
    
    self.viewFree = [[ZNewHomeFreeView alloc] initWithFrame:(CGRectMake(0, self.viewRecommend.y+self.viewRecommend.height+viewSpace, self.viewMain.width, itemDefault))];
    [self.viewMain addSubview:self.viewFree];
    
    self.viewTrain = [[ZNewHomeTrainView alloc] initWithFrame:(CGRectMake(0, self.viewFree.y+self.viewFree.height+viewSpace, self.viewMain.width, itemDefault))];
    [self.viewMain addSubview:self.viewTrain];
    
    self.viewSeries = [[ZNewHomeSeriesView alloc] initWithFrame:(CGRectMake(0, self.viewTrain.y+self.viewTrain.height+viewSpace, self.viewMain.width, itemDefault))];
    [self.viewMain addSubview:self.viewSeries];
    
    self.viewTiny = [[ZNewHomeTinyView alloc] initWithFrame:(CGRectMake(0, self.viewSeries.y+self.viewSeries.height+viewSpace, self.viewMain.width, itemDefault))];
    [self.viewMain addSubview:self.viewTiny];
    
    self.viewLaw = [[ZNewHomeLawView alloc] initWithFrame:(CGRectMake(0, self.viewTiny.y+self.viewTiny.height+viewSpace, self.viewMain.width, itemDefault))];
    [self.viewMain addSubview:self.viewLaw];
    
    self.viewNoNetwork.hidden = true;
...
-(void)innerEvent
{
    ...
    [self.viewBannerCard setOnBannerClick:^(ModelBanner *model) {
        if (weakSelf.onBannerClick) {
            weakSelf.onBannerClick(model);
        }
    }];
    [self.viewNoNetwork setOnSettingClick:^{
        if (weakSelf.onGoNoNetworkClick) {
            weakSelf.onGoNoNetworkClick();
        }
    }];
...
-(void)setViewData:(NSDictionary *)dicData
{
    ...
    if (dicResult && [dicResult isKindOfClass:[NSDictionary class]]) {
        ...
        if (arrayBanner && [arrayBanner isKindOfClass:[NSArray class]] && arrayBanner.count > 0) {
            ...
            self.arrBanner = nil;
            [self.viewBannerCard setViewData:array];
//            if (array.count > 0) {
...
{
    CGFloat itemSpace = 1;
    CGFloat nonetwork = self.viewHeader.y+self.viewHeader.height;
    if (!self.viewNoNetwork.isHidden) {
        self.viewNoNetwork.frame = CGRectMake(self.viewNoNetwork.x, nonetwork+itemSpace, self.viewNoNetwork.width, [ZNoNetworkView getViewH]);
        nonetwork = self.viewNoNetwork.y+self.viewNoNetwork.height;
    } else {
        self.viewNoNetwork.frame = CGRectMake(self.viewNoNetwork.x, nonetwork, self.viewNoNetwork.width, 0);
    }
    ...
    if (self.arrRecommend.count==0) {
        ...
        self.viewRecommend.frame = CGRectMake(self.viewRecommend.x, nonetwork, self.viewRecommend.width, viewRecommendHeight);
    } else {
        self.viewRecommend.frame = CGRectMake(self.viewRecommend.x, nonetwork+itemSpace, self.viewRecommend.width, viewRecommendHeight);
    }
    ...
    CGFloat bottomHeight = [ZGlobalPlayView getMinH];
...
{
    if (show == self.viewNoNetwork.hidden) {
        [self.viewNoNetwork setHidden:!show];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.onContentOffsetClick) {
        self.onContentOffsetClick(offsetY, self.viewBannerCard.height);
    }
}
