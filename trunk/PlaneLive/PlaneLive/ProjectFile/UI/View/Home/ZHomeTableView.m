...
///律所模块
@property (strong, nonatomic) ZHomeLawFirmTVC *tvcLawFirm;
///订阅模块
...
-(void)innerInit
{
    ...
    self.tvcLawFirm = [[ZHomeLawFirmTVC alloc] initWithReuseIdentifier:@"tvcLawFirm"];
    ...
    self.arrMain = @[self.tvcPractice, self.tvcLawFirm, self.tvcSubscribe, self.tvcCurriculum, self.tvcQuestion];
    //self.arrMain = @[self.tvcPractice, self.tvcLawFirm, self.tvcCurriculum, self.tvcQuestion];
    ...
    [self.tvcLawFirm setOnAllClick:^{
        if (weakSelf.onAllLawFirmClick) {
            weakSelf.onAllLawFirmClick();
        }
    }];
    [self.tvcLawFirm setOnLawFirmClick:^(ModelLawFirm *model) {
...
{
    [self.tvMain setScrollsToTop:scrollsToTop];
}
-(void)setTableViewScrollsToTop
{
    [self.tvMain setContentOffset:(CGPointMake(0, 0))];
}
-(void)dealloc
{
    ...
    OBJC_RELEASE(_onAllLawFirmClick);
                    ...
                    arrCurriculum:(NSArray *)arrCurriculum
{
    ...
    [self.tvcLawFirm setViewDataWithArray:arrLawFirm];
    ...
    //[self.tvMain reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:1 inSection:0]] withRowAnimation:(UITableViewRowAnimationAutomatic)];
}
