...
#define BUTTONTITLE_FONT                        [ZFont systemFontOfSize:15]
#define BUTTONTITLE_BOLDFONT                    [ZFont boldSystemFontOfSize:15]
...
#define TITLE_FONT                              [ZFont systemFontOfSize:14]
    ...
    if (self) {
        ...
        if (APP_SYSTEM_VERSION >= 8) {
            ...
            if (cancelButtonTitle) {
                UIAlertAction *itemAction = [UIAlertAction actionWithTitle:cancelButtonTitle style:(UIAlertActionStyleCancel) handler:^(UIAlertAction * _Nonnull action) {
                    if (weakSelf.delegate) {
                        if ([weakSelf.delegate respondsToSelector:@selector(actionSheetDidClickOnCancelButton)] == YES) {
                            [weakSelf.delegate actionSheetDidClickOnCancelButton];
                        }
                    }
                }];
            ...
            if (destructiveButtonTitle) {
                UIAlertAction *itemAction = [UIAlertAction actionWithTitle:destructiveButtonTitle style:(UIAlertActionStyleDestructive) handler:^(UIAlertAction * _Nonnull action) {
                    if (self.delegate) {
                        if ([self.delegate respondsToSelector:@selector(actionSheet:didButtonClickWithIndex:)] == YES) {
                            [self.delegate actionSheet:self didButtonClickWithIndex:0];
                        }
                    }
                }];
                [alertVC addAction:itemAction];
            }
            for (NSString *itemTitle in otherButtonTitlesArray) {
                UIAlertAction *itemAction = [UIAlertAction actionWithTitle:itemTitle style:(UIAlertActionStyleDestructive) handler:^(UIAlertAction * _Nonnull action) {
                    if (self.delegate) {
                        NSInteger buttonIndex = 0;
                        if (destructiveButtonTitle) {
                            buttonIndex = [otherButtonTitlesArray indexOfObject:action.title]+1;
                        } else {
                            buttonIndex = [otherButtonTitlesArray indexOfObject:action.title];
                        }
                        if ([self.delegate respondsToSelector:@selector(actionSheet:didButtonClickWithIndex:)] == YES) {
                            [self.delegate actionSheet:self didButtonClickWithIndex:buttonIndex];
                        }
                    }
                }];
                [alertVC addAction:itemAction];
            ...
            [self showActionSheetWithOtherArray:otherButtonTitlesArray title:title cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle];
        }
#else
        [self showActionSheetWithOtherArray:otherButtonTitlesArray title:title cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle];
#endif
...
-(void)showActionSheetWithOtherArray:(NSArray *)otherButtonTitlesArray title:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle
{
    switch (otherButtonTitlesArray.count) {
        case 0:
        {
        ...
        case 1:
        {
            actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitlesArray.firstObject, nil];
            break;
        }
        case 2:
        {
            actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitlesArray.firstObject, otherButtonTitlesArray.lastObject, nil];
            break;
        }
        case 3:
        {
            actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitlesArray.firstObject, [otherButtonTitlesArray objectAtIndex:1], otherButtonTitlesArray.lastObject, nil];
            break;
        }
        case 4:
        {
            actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitlesArray.firstObject, [otherButtonTitlesArray objectAtIndex:1], [otherButtonTitlesArray objectAtIndex:2], otherButtonTitlesArray.lastObject, nil];
            break;
        }
        case 5:
        {
            actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitlesArray.firstObject, [otherButtonTitlesArray objectAtIndex:1], [otherButtonTitlesArray objectAtIndex:2], [otherButtonTitlesArray objectAtIndex:3], otherButtonTitlesArray.lastObject, nil];
            break;
        }
        case 6:
        {
            actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitlesArray.firstObject, [otherButtonTitlesArray objectAtIndex:1], [otherButtonTitlesArray objectAtIndex:2], [otherButtonTitlesArray objectAtIndex:3], [otherButtonTitlesArray objectAtIndex:4], otherButtonTitlesArray.lastObject, nil];
            break;
        }
        case 7:
        {
            actionSheet = [[UIActionSheet alloc] initWithTitle:title delegate:self cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitlesArray.firstObject, [otherButtonTitlesArray objectAtIndex:1], [otherButtonTitlesArray objectAtIndex:2], [otherButtonTitlesArray objectAtIndex:3], [otherButtonTitlesArray objectAtIndex:4], [otherButtonTitlesArray objectAtIndex:5], otherButtonTitlesArray.lastObject, nil];
            break;
        }
        default: break;
    }
...
- (id)initWithPictureSelectionDelegate:(id<ZActionSheetDelegate>)delegate
{
    ...
    if (self) {
        ...
        [self creatButtonsWithCancelButtonTitle:kCancel otherButtonTitles:@[kCAlbum,kCPhotograph] otherButtonIcons:@[@"gallery",@"photo"]];
...
- (void)tappedCancel
{
    ...
    [UIView animateWithDuration:ANIMATE_DURATION animations:^{
        [self.backGroundView setFrame:CGRectMake(BACKGROUNDVIEW_INTERVAL_WIDTH, APP_FRAME_HEIGHT, BACKGROUNDVIEW_WIDTH, 0)];
        self.alpha = 0;
    } completion:^(BOOL finished) {
        if (finished) {
