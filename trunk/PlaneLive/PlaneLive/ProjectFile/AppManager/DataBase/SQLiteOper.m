...
-(BOOL)setLocalAppConfigWithModel:(ModelAppConfig *)model
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        result = [db executeUpdate:@"DELETE FROM sns_appconfig"];
        if (!result) {
            *rollback = YES;
        }
...
{
    if (array == nil || array.count == 0 || practiceId == nil) {
        return NO;
    }
    BOOL __block result = NO;
...
-(NSDictionary *)getLocalHomeAllDataWithUserId:(NSString *)userId
{
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        // 读取微课
        FMResultSet *rsPractice = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_home_practice WHERE userId='%@' ORDER BY sortby ASC", userId]];
        if (rsPractice != nil)
        {
            ...
            while ([rsPractice next]) {
                ...
                [array addObject:modelP];
                
            }
            [result setObject:array forKey:@"Practice"];
        }
        [rsPractice close];
        // 读取广告
        FMResultSet *rsBanner = [db executeQuery:@"SELECT * FROM sns_home_banner ORDER BY sortby ASC"];
        if (rsBanner != nil)
        {
            NSMutableArray *array = [NSMutableArray array];
            ...
            while ([rsBanner next]) {
                ...
                [array addObject:[[ModelBanner alloc] initWithAuto:[rsBanner resultDictionary]]];
                
            }
            [result setObject:array forKey:@"Banner"];
        }
        [rsBanner close];
        // 读取律所
        FMResultSet *rsLawFirm = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_home_lawfirm WHERE userId = '%@' ORDER BY sortby DESC", userId]];
        if (rsLawFirm != nil)
        {
            NSMutableArray *array = [NSMutableArray array];
            
            while ([rsLawFirm next]) {
                
                [array addObject:[[ModelLawFirm alloc] initWithAuto:[rsLawFirm resultDictionary]]];
                
            }
            [result setObject:array forKey:@"LawFirm"];
        }
        [rsLawFirm close];
        // 读取系列课
        FMResultSet *rsCurriculum = [db executeQuery:@"SELECT * FROM sns_home_curriculum ORDER BY sortby ASC"];
        if (rsCurriculum != nil)
        {
            NSMutableArray *array = [NSMutableArray array];
            
            while ([rsCurriculum next]) {
                
                [array addObject:[[ModelSubscribe alloc] initWithAuto:[rsCurriculum resultDictionary]]];
                
            }
            [result setObject:array forKey:@"Curriculum"];
        }
        [rsCurriculum close];
        // 读取系列课
        FMResultSet *rsSubscribe = [db executeQuery:@"SELECT * FROM sns_home_subscribe ORDER BY sortby ASC"];
        if (rsSubscribe != nil)
        {
            NSMutableArray *array = [NSMutableArray array];
            
            while ([rsSubscribe next]) {
                
                [array addObject:[[ModelSubscribe alloc] initWithAuto:[rsSubscribe resultDictionary]]];
                
            }
            [result setObject:array forKey:@"Subscribe"];
        }
        [rsSubscribe close];
        // 读取问答
        FMResultSet *rsQuestion = [db executeQuery:@"SELECT * FROM sns_home_question ORDER BY sortby ASC"];
        if (rsQuestion != nil)
        {
            NSMutableArray *array = [NSMutableArray array];
            ...
            while ([rsQuestion next]) {
                ...
                [array addObject:[[ModelQuestionBoutique alloc] initWithAuto:[rsQuestion resultDictionary]]];
                
            }
            [result setObject:array forKey:@"Question"];
        }
        [rsQuestion close];
    }];
    return result;
...
-(BOOL)setLocalHomeSubscribeWithArray:(NSArray *)array userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelSubscribe *model in array) {
            ...
            NSNumber *increasedCourseCount = [NSNumber numberWithInt:model.increasedCourseCount];
            
            result = [db executeUpdate:@"REPLACE INTO sns_home_subscribe(ids,userId,title,team_name,team_intro,theme_intro,create_time,price,units,illustration,status,hits,is_series_course,increasedCourseCount,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,user_Id,title,team_name,team_intro,theme_intro,create_time,price,units,illustration,status,hits,is_series_course,increasedCourseCount]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(BOOL)setLocalSubscribeRecommendWithArray:(NSArray *)array userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelSubscribe *model in array) {
            ...
            result = [db executeUpdate:@"REPLACE INTO sns_subscribe(ids,userId,title,team_name,team_info,theme_intro,illustration,price,count,units,status,hits,is_series_course,increasedCourseCount,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,team_name,team_info,theme_intro,illustration,price,count,units,status,hits,is_series_course,increasedCourseCount]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(BOOL)setLocalPurchasePracticeWithArray:(NSArray *)arr userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelPractice *model in arr) {
            ...
            ///添加微课
            result = [db executeUpdate:@"REPLACE INTO sns_purchase_practice(ids,userId,title,time,play_time,speech_img,bkimg,speech_url,share_content,nickname,create_time,speech_content,ccount,mcount,applauds,isCollection,isPraise,rowIndex,unlock,person_synopsis,person_title,showText,joinCart,price,buyStatus,increasedCourseCount,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,time,play_time,speech_img,bkimg,speech_url,share_content,nickname,create_time,speech_content,ccount,mcount,applauds,isCollection,isPraise,rowIndex,unlock,person_synopsis,person_title,showText,joinCart,price,buyStatus,increasedCourseCount]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(BOOL)setLocalDownloadListWithModelTrack:(ModelTrack *)model
{
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        ...
        NSNumber *announcerid = [NSNumber numberWithInteger:[userId longLongValue]];
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if (rs != nil)
        {
            ...
            while ([rs next]) {
                ...
                announcer.id = [[rs stringForColumn:@"announcerid"] longLongValue];
                ...
                subordinatedAlbum.albumId = [[rs stringForColumn:@"subordinatedAlbumalbumId"] longLongValue];
...
///获取喜马拉雅下载记录
-(NSMutableArray *)getLocalDownloadListAll
{
    __block NSMutableArray *result = nil;
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:@"SELECT * FROM sns_xmtrack ORDER BY sortby ASC"];
        if (rs != nil)
        {
            result = [NSMutableArray array];
...
///获取下载状态列表 - 全部
-(int)getLocalDownloadListAllCount
{
    __block int result = 0;
    [dbQueue inDatabase:^(FMDatabase *db) {
        result = [db intForQuery:@"SELECT count(*) FROM sns_xmtrack"];
    }];
    return result;
}
///删除喜马拉雅下载记录
-(BOOL)delLocalDownloadListAll
{
    BOOL __block result = NO;
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        result = [db executeUpdate:@"DELETE FROM sns_xmtrack "];
    }];
    return result;
}

/// 改变下载列表状态
-(BOOL)setLocalDownloadWithTracks:(NSArray *)array
{
    ...
    [dbQueue inTransaction:^(FMDatabase * _Nonnull db, BOOL * _Nonnull rollback) {
        for (XMCacheTrack *modelCT in array) {
            NSString *userId = [NSString stringWithFormat:@"%ld", (long)modelCT.announcer.id];
            if (![userId isEqualToString:@"0"]) {
                NSString *status = [NSString stringWithFormat:@"%ld", (long)modelCT.status];
                NSString *trackId = [NSString stringWithFormat:@"%ld", (long)modelCT.trackId];
                NSString *dataType = [NSString stringWithFormat:@"%ld", (long)modelCT.source];
                result = [db executeUpdate:@"REPLACE INTO sns_download_status(trackId,dataType,status,userId,sortby) VALUES(?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[trackId,dataType,status,userId]];
                if (!result) {
                    *rollback = YES;
                    break;
                }
...
{
    if (status == nil || status == nil || dataType == nil) return NO;
...
-(NSMutableArray *)getLocalDownloadStatusEndDownload:(NSInteger)dataType
{
    ...
    NSString *userId = kLoginUserId;
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_download_status WHERE userId = '%@' AND status = '%@' AND dataType = '%@' ORDER BY sortby ASC", userId, [NSString stringWithFormat:@"%d", (int)XMCacheTrackStatusDownloaded], [NSString stringWithFormat:@"%d", (int)dataType]]];
        if (rs != nil)
        {
            result = [NSMutableArray array];
            ...
            while ([rs next]) {
                ...
                NSString *trackId = [rs stringForColumn:@"trackId"];
                NSString *dataType = [rs stringForColumn:@"datatype"];
                NSString *status = [rs stringForColumn:@"status"];
                [dic setObject:trackId == nil ? kEmpty : trackId forKey:@"id"];
                [dic setObject:dataType == nil ? kEmpty : dataType forKey:@"type"];
                [dic setObject:status == nil ? kEmpty : status forKey:@"status"];
...
-(NSMutableArray *)getLocalDownloadStatusNotEndDownload
{
    ...
    NSString *userId = kLoginUserId;
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_download_status WHERE userId = '%@' AND status != %@ ORDER BY sortby ASC", userId, [NSString stringWithFormat:@"%d", (int)XMCacheTrackStatusDownloaded]]];
        if (rs != nil)
        {
            result = [NSMutableArray array];
...
/// 获取下载状态列表 - 全部
-(int)getLocalDownloadStatusAll
{
    __block int result = 0;
    [dbQueue inDatabase:^(FMDatabase *db) {
        result = [db intForQuery:@"SELECT count(*) FROM sns_download_status"];
    }];
    return result;
}
/// 删除下载状态列表
-(BOOL)delLocalDownloadStatusWithArray:(NSArray *)array
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (XMTrack *model in array) {
            result = [db executeUpdate:[NSString stringWithFormat:@"DELETE FROM sns_download_status WHERE trackId = '%@' AND dataType = '%@' AND userId = '%@' ", [NSString stringWithFormat:@"%ld", (long)model.trackId], [NSString stringWithFormat:@"%d", (int)model.source], userId]];
        }
    }];
    return result;
}
...
-(BOOL)setLocalHomeLawFirmWithArray:(NSArray *)array
{
    ...
    NSString *userId = kLoginUserId;
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        result = [db executeUpdate:[NSString stringWithFormat:@"DELETE FROM sns_home_lawfirm WHERE userId = '%@'", userId]];
        if (!result) {
            *rollback = YES;
        }
        for (ModelLawFirm *model in array) {
            
            NSString *ids = model.ids == nil?kEmpty:model.ids;
            NSString *title = model.title == nil?kEmpty:model.title;
            NSString *desc = model.desc == nil?kEmpty:model.desc;
            NSString *logo = model.logo == nil?kEmpty:model.logo;
            NSString *picture = model.picture == nil?kEmpty:model.picture;
            NSNumber *total = [NSNumber numberWithInt:model.total];
            
            result = [db executeUpdate:@"REPLACE INTO sns_home_lawfirm(ids,userId,title,desc,logo,picture,total,sortby) VALUES(?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,desc,logo,picture,total]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(NSMutableArray *)getLocalHomeLawFirmWithArrayAll
{
    ...
    NSString *userId = kLoginUserId;
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_home_lawfirm WHERE userId = '%@' ORDER BY sortby DESC", userId]];
        if (rs != nil)
        {
            result = [NSMutableArray array];
            ...
            while ([rs next]) {
                ...
                [result addObject:[[ModelLawFirm alloc] initWithAuto:[rs resultDictionary]]];
...
-(BOOL)setLocalLawFirmListWithArray:(NSArray *)array
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelLawFirm *model in array) {
            ...
            result = [db executeUpdate:@"REPLACE INTO sns_lawfirm_list(ids,userId,title,desc,logo,picture,total,sortby) VALUES(?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,desc,logo,picture,total]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(NSMutableArray *)getLocalLawFirmListWithArrayAll
{
    ...
    NSString *userId = kLoginUserId;
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_lawfirm_list WHERE userId = '%@' ORDER BY sortby DESC", userId]];
        if (rs != nil)
        {
            result = [NSMutableArray array];
