...
-(NSInteger)length
{
    ...
    return [NSString stringWithFormat:@"%ld", (long)[self integerValue]].length;
}
...
{
    [self.layer setMasksToBounds:YES];
    [self setViewRound:radius borderWidth:0 borderColor:CLEARCOLOR];
}
...
@implementation UIView (ShadowColor)

-(void)setShadowColorWithRadius:(CGFloat)radius
{
    [self.layer setShadowColor:COLORVIEWSHADOW.CGColor];
...
-(void)setAllShadowColorWithRadius:(CGFloat)radius
{
    ...
    [self.layer setBorderColor:TABLEVIEWCELL_LINECOLOR.CGColor];
}
...
{
    [self.layer setShadowColor:COLORBUTTONSHADOW.CGColor];
