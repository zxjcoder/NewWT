...
/// 默认游客ID
#define kUserDefaultId @"1"
/// 默认审核ID
#define kUserAuditId @"20963"
/// 登录用户ID
#define kLoginUserId [AppSetting getUserId]
/// 是否审核
#define kIsAppAudit [[AppDelegate app] isAppAudit]
/// 是否需要登录
#define kIsNeedLogin (![AppSetting getAutoLogin] || ([AppSetting getAutoLogin] && [[AppSetting getUserId] isEqualToString:kUserAuditId]))

@interface UINavigationBar (ClassCategory)
...
- (void)setViewRound:(float)radius;
...
- (void)setLabelFontWithRange:(NSRange)range font:(UIFont*)font color:(UIColor*)color;
...
/// 设置图片透明度
- (UIImage *)setImageAlpha:(CGFloat)alpha;
...
- (UIImage *)drawRectWithSizetoFit:(CGSize)size;
...
@interface UIView (ShadowColor)

///左右下偏移
-(void)setShadowColorWithRadius:(CGFloat)radius;
///左右下偏移
-(void)setShadowColor;
///左右上下偏移
-(void)setAllShadowColor;
///设置全方位投影,边框,弧形
-(void)setAllShadowColorWithRadius:(CGFloat)radius;
///设置全方位投影,弧形
-(void)setAllShadowColorNoBorderWithRadius:(CGFloat)radius;
///圆角偏移投影
-(void)setButtonShadowColorCircular;
///左右下偏移
-(void)setButtonShadowColorWithRadius:(CGFloat)radius;
///左右下偏移
-(void)setButtonShadowColorWithRadius:(CGFloat)radius color:(UIColor *)color;
///左右下偏移
-(void)setButtonShadowColor;
