 ...
 *  @param errorBlock  错误回调
 */
-(void)getAppConfigWithAppVersion:(NSString *)appVersion resultBlock:(void(^)(ModelAppConfig *currentVersion, ModelAppConfig *newVersion, ModelUser *userInfo))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock;
{
    NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
    if (appVersion) {
    ...
    [self postJsonWithAction:@"share/v4/appversion/queryAppVersion" postParam:dicParam postFile:nil resultBlock:^(Response *result) {
        if (result.success) {
            NSDictionary *dicResult = [result.body objectForKey:kResultKey];
            if (dicResult && [dicResult isKindOfClass:[NSDictionary class]]) {
                NSDictionary *dicCurrent = [dicResult objectForKey:@"currentVersion"];
                ModelAppConfig *currentVersion = nil;
                if (dicCurrent && [dicCurrent isKindOfClass:[NSDictionary class]]) {
                    currentVersion = [[ModelAppConfig alloc] initWithCustom:dicCurrent];
                }
                NSDictionary *dicNew = [dicResult objectForKey:@"newVersion"];
                ModelAppConfig *newVersion = nil;
                if (dicNew && [dicNew isKindOfClass:[NSDictionary class]]) {
                    newVersion = [[ModelAppConfig alloc] initWithCustom:dicNew];
                }
                NSDictionary *dicUser = [dicResult objectForKey:@"userInfo"];
                ModelUser *userInfo = nil;
                if (dicUser && [dicUser isKindOfClass:[NSDictionary class]]) {
                    userInfo = [[ModelUser alloc] initWithCustom:dicUser];
                }
                if (resultBlock) {
                    resultBlock(currentVersion, newVersion, userInfo);
                }
            } else {
                if (errorBlock) {
                    errorBlock(result.msg);
 ...
 *
 *  @param account      登录账户
 *  @param flag         1:注册时获取验证码 2:找回密码获取验证码 3:绑定手机号(未注册才能绑定) 4:验证手机号 5:更换手机号 6:解绑手机号 7:绑定手机号(注册依然可以绑定)
 *  @param resultBlock 成功回调
 *  @param errorBlock  错误回调
 */
...
{
    if ([userId isEqualToString:kUserDefaultId]) {
        if (errorBlock) {
            errorBlock(kSNS_RETURN_SERVER_DEFAULT_Text);
        }
    }
    NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
    if (userId) {
    ...
    [self postJsonWithAction:@"share/v7/user/refresh" postParam:dicParam postFile:nil resultBlock:^(Response *result) {
        if (result.success) {
            if (resultBlock) {
                NSMutableDictionary *dicUser = [NSMutableDictionary dictionaryWithDictionary:[result.body objectForKey:kResultKey]];
                if (dicUser && [dicUser isKindOfClass:[NSDictionary class]]) {
...
{
    NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
    [dicParam setObject:kLoginUserId forKey:@"userId"];
    [dicParam setObject:[NSNumber numberWithInt:pageNum] forKey:@"pageNum"];
    [dicParam setObject:[NSNumber numberWithInt:kPAGE_MAXCOUNT] forKey:@"pageSize"];
 ...
 *  @param userId       用户ID
 *  @param aid          点赞对象ID
 *  @param type         类型0问题1人2答案3话题4语音5订阅内容
 *  @param resultBlock 成功回调
 *  @param errorBlock  错误回调
 */
 ...
 *  @param userId       用户iD
 *  @param cid          收藏的ID,行业时可不传
 *  @param flag         1：微课 2：机构或人员 3:行业 4:答案 5:课程内容
 *  @param type         0律所 1 会计 2证券 3语音 4人 5机构 6答案 7:课程内容
 *  @param resultBlock 成功回调
 *  @param errorBlock  错误回调
 */
...
-(void)getUserNoticeListWithUserId:(NSString *)userId pageNum:(int)pageNum resultBlock:(void(^)(NSArray *arrResult, NSDictionary *result))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock
{
    ...
    [self postJsonWithAction:@"share/v7/user/getNoticeList" postParam:dicParam postFile:nil resultBlock:^(Response *result) {
        if (result.success) {
            if (resultBlock) {
                NSMutableArray *arrResult = [NSMutableArray array];
                NSArray *arrR = [result.body objectForKey:kResultKey];
