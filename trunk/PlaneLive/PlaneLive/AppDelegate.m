...
{
    GBLog(@"sourceApplication-Url-Host: %@", url.host);
...
-(void)application:(UIApplication *)application performActionForShortcutItem:(UIApplicationShortcutItem *)shortcutItem completionHandler:(void (^)(BOOL))completionHandler
{
    if (@available(iOS 9.0, *)) {
        if (shortcutItem) {
            if ([shortcutItem.type isEqualToString:k3DTouch_PlayPractice_Key]) {
...
-(void)createTouchItemsWithIcons
{
    if (@available(iOS 9.0, *)) {
        ...
        [UIApplication sharedApplication].shortcutItems = @[itemPlay, itemSearchPractice];
    }
}
///显示提问界面
...
//    } else {
//        if ([PlayerManager sharedPlayer].isPlayering && [ZPlayerViewController sharedSingleton].playIndex < [ZPlayerViewController sharedSingleton].arrayRawdata.count) {
//            [[ZPlayerViewController sharedSingleton] setRawdataWithArray:[ZPlayerViewController sharedSingleton].arrayRawdata index:[ZPlayerViewController sharedSingleton].playIndex];
//        } else {
//            [[PlayerManager sharedPlayer] stopTrackPlay];
//            [[ZPlayerViewController sharedSingleton] setModelTrack:nil];
//            [[ZPlayerViewController sharedSingleton] setModelPractice:nil];
...
-(void)setCheckPlayList
{
    ...
    if (arrP && arrP.count > 0) {
        self.isPlay = YES;
        [[ZPlayerViewController sharedSingleton] setInnerPlayTabbarWithType:(ZPlayTabBarViewTypePractice)];
        [self setUpdatePlayerList:arrP];
    } else {
        NSArray *arrC = [sqlite getLocalPlayListSubscribeCurriculumWithUserId:kLoginUserId];
        if (arrC && arrC.count > 0) {
            self.isPlay = YES;
            [[ZPlayerViewController sharedSingleton] setInnerPlayTabbarWithType:(ZPlayTabBarViewTypeSubscribe)];
            [self setUpdatePlayerList:arrC];
        } else {
            self.isPlay = NO;
            [self setUpdatePlayerList:nil];
        }
    }
}
///更新播放列表
-(void)setUpdatePlayerList:(NSArray *)array
{
    [[PlayerManager sharedPlayer] stopTrackPlay];
    [[ZPlayerViewController sharedSingleton] setModelTrack:nil];
    [[ZPlayerViewController sharedSingleton] setModelPractice:nil];
    ...
    [[ZPlayerViewController sharedSingleton] setArrayTrack:nil];
    [[ZPlayerViewController sharedSingleton] setArrayRawdata:nil];
    [[ZPlayerViewController sharedSingleton] setPlayIndex:0];
    if (array && [array isKindOfClass:[NSArray class]] && array.count > 0) {
        [[ZPlayerViewController sharedSingleton] setRawdataWithArray:array index:0];
    }
}
///去APP更新
-(void)gotoAppUpdate
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            ...
            case AFNetworkReachabilityStatusReachableViaWWAN://2G,3G,4G...的网络
                weakSelf.statusNetwork = 2;
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi://WiFi的网络
                weakSelf.statusNetwork = 1;
                break;
        }
        [[NSNotificationCenter defaultCenter] postNotificationName:@"statusNetworkChange" object:nil];
    }];
