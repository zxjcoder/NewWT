...
-(void)setRefreshUserData
{
    if (self.pageNumUser == 1) {
        ZWEAKSELF
        self.pageNumUser = 1;
        [snsV1 postMyGetAttentionWithUserId:self.modelUB.userId flag:1 pageNum:self.pageNumUser resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
...
{
    [snsV1 postDeleteAttentionWithAId:model.ids userId:[AppSetting getUserDetauleId] type:@"0" resultBlock:nil errorBlock:nil];
}
-(void)setDeleteTopicClick:(ModelTag *)model
{
    [snsV1 postDeleteAttentionWithAId:model.tagId userId:[AppSetting getUserDetauleId] type:@"3" resultBlock:nil errorBlock:nil];
}
-(void)setDeleteUserClick:(ModelUserBase *)model
{
    [snsV1 postDeleteAttentionWithAId:model.userId userId:[AppSetting getUserDetauleId] type:@"1" resultBlock:nil errorBlock:nil];
}
...
-(void)setRefreshTopicData:(BOOL)isRefresh
{
    ...
    [snsV1 postMyGetAttentionWithUserId:self.modelUB.userId flag:2 pageNum:self.pageNumTopic resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)setRefreshQuestionData:(BOOL)isRefresh
{
    ...
    [snsV1 getCircleAttentionListWithUserId:self.modelUB.userId pageNum:self.pageNumQuestion resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)setRefreshQuestionHeader
{
    ZWEAKSELF
    self.pageNumQuestion = 1;
    [snsV1 getCircleAttentionListWithUserId:[AppSetting getUserDetauleId] pageNum:self.pageNumQuestion resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
