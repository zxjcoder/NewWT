...
@interface ZUserFeedbackViewController ()<UIImagePickerControllerDelegate,UINavigationControllerDelegate,ZActionSheetDelegate,QBImagePickerControllerDelegate>
...
-(void)showAlbum
{
    if ([ZImagePickerController isSourceTypeAvailable: UIImagePickerControllerSourceTypePhotoLibrary]) {
        QBImagePickerController *itemVC = [QBImagePickerController new];
        itemVC.delegate = self;
        itemVC.allowsMultipleSelection = YES;
        itemVC.minimumNumberOfSelection = 1;
        itemVC.maximumNumberOfSelection = self.imgMaxCount;
