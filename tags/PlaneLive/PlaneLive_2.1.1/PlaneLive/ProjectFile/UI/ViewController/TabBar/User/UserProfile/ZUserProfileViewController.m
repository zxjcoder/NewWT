...
-(void)innerData
{
    ...
    [snsV1 postGetUserInfoWithUserId:[AppSetting getUserDetauleId] hisId:self.model.userId resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnAttClick
{
    ...
    if (self.modelUser.isAtt) {
        ...
        ZWEAKSELF
        [snsV1 postDeleteAttentionWithAId:self.model.userId userId:[AppSetting getUserDetauleId] type:@"1" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
        ...
        [self.tvcButton setCellDataWithModel:self.modelUser];
        ZWEAKSELF
        [snsV1 postAddAttentionWithuserId:[AppSetting getUserDetauleId] hisId:self.model.userId type:@"1" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
                [weakSelf setIsAtting:NO];
...
-(void)btnBlackListClick
{
    if ([AppSetting getAutoLogin]) {
        ZWEAKSELF
        [snsV1 postAddBlackListWithUserId:[AppSetting getUserDetauleId] addUserId:self.model.userId resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
