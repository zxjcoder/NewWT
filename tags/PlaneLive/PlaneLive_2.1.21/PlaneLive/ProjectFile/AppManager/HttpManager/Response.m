...
+ (id)sharedWithJSONString:(NSString *)result
{
    ...
    if (dicResult==nil) {
        ...
        resp.msg = [self getErrorMessageWithDictionary:dicResult];
        resp.body = dicResult;
...
+ (id)sharedWithJSONDictionary:(NSDictionary *)result
{
    ...
    if (result == nil || [result isKindOfClass:[NSNull class]]) {
        ...
        resp.msg = [self getErrorMessageWithDictionary:result];
        resp.body = result;
        resp.success = resp.code == kSNS_RETURN_SUCCESS;
    }
    return resp;
...
+(NSString *)getErrorMessageWithDictionary:(NSDictionary *)result
{
    NSString *msg = [Utils getSNSString:result[kResponseMsgKey]];
    if (msg == nil || [msg isKindOfClass:[NSNull class]] || ![msg isKindOfClass:[NSString class]]) {
        msg = kSNS_RETURN_SERVER_DECRYPTDATA_ERROR_Text;
    }
    return msg;
}
