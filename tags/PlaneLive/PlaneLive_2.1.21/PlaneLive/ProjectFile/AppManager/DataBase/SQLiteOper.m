    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if (rs != nil)
        {
            ...
            while ([rs next]) {
                ...
                FMResultSet *rsPractice = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_type_practice WHERE userId = '%@' AND typeId = '%@' AND address = 2 ORDER BY sortby ASC", userId, modelPT.ids]];
                if (rsPractice != nil)
                {
...
-(BOOL)setLocalSubscribePlayWithArray:(NSArray *)array userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelSubscribePlay *model in array) {
            ...
            result = [db executeUpdate:@"REPLACE INTO sns_subscribe_play(ids,userId,title,pay_time,price,units,type,sortby) VALUES(?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,pay_time,price,units,type]];
            if (!result) {
                *rollback = YES;
                break;
            }
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if (rs != nil)
        {
            ...
            while ([rs next]) {
                ...
                NSDictionary *dicResult = [rs resultDictionary];
                ModelCurriculum *modelC = [[ModelCurriculum alloc] initWithAuto:dicResult];
                NSString *courseImges = [dicResult objectForKey:@"course_imges"];
                modelC.course_imges = [courseImges componentsSeparatedByString:@"@"];
