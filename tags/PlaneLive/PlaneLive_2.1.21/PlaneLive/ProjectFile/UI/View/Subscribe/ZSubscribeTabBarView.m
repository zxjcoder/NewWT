...
-(void)innerInit
{
    ...
    [self.btnSubscribe setTitle:kSubscribe forState:(UIControlStateNormal)];
...
-(void)setViewDataWithModel:(ModelSubscribe *)model
{
    if (model) {
        if (model.units && model.units.length > 0) {
            [self.btnSubscribe setTitle:[NSString stringWithFormat:@"%@ ¥%.2f/%@", kSubscribe, [model.price floatValue], model.units] forState:(UIControlStateNormal)];
        } else {
            [self.btnSubscribe setTitle:[NSString stringWithFormat:@"%@ ¥%.2f", kSubscribe, [model.price floatValue]] forState:(UIControlStateNormal)];
        }
    } else {
        [self.btnSubscribe setTitle:kSubscribe forState:(UIControlStateNormal)];
    }
...
{
    [self.btnProbation setHidden:isHidden];
    [self.imgline2 setHidden:isHidden];
    if (isHidden) {
        [self.btnProbation setFrame:CGRectMake(0, 0, 0.1, 0.1)];
        [self.btnSubscribe setFrame:CGRectMake(0, 0, self.width, self.height)];
    } else {
        [self.btnProbation setFrame:CGRectMake(0, 0, self.width/2, self.height)];
        [self.btnSubscribe setFrame:CGRectMake(self.width/2, 0, self.width/2, self.height)];
