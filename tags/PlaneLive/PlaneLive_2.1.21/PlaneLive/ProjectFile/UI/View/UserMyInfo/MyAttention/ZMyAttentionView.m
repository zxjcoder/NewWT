...
-(void)innerInit
{
    ...
    [self.viewTool setOnItemClick:^(NSInteger index) {
        switch (index) {
            case 2:
                [weakSelf.tvAttQuestion setScrollsToTop:NO];
                [weakSelf.tvAttTopic setScrollsToTop:YES];
                [weakSelf.tvAttUser setScrollsToTop:NO];
                break;
            case 3:
                [weakSelf.tvAttQuestion setScrollsToTop:NO];
                [weakSelf.tvAttTopic setScrollsToTop:NO];
                [weakSelf.tvAttUser setScrollsToTop:YES];
                break;
            default:
                [weakSelf.tvAttQuestion setScrollsToTop:YES];
                [weakSelf.tvAttTopic setScrollsToTop:NO];
                [weakSelf.tvAttUser setScrollsToTop:NO];
                break;
        }
    ...
    [self.tvAttQuestion setBackgroundColor:VIEW_BACKCOLOR1];
    [self.tvAttQuestion setOnRowSelected:^(ModelQuestionBase *model) {
        if (weakSelf.onQuestionRowClick) {
            weakSelf.onQuestionRowClick(model);
        }
    }];
    [self.tvAttQuestion setOnAnswerClick:^(ModelAnswerBase *model) {
        if (weakSelf.onAnswerRowClick) {
            weakSelf.onAnswerRowClick(model);
        }
    }];
    [self.tvAttQuestion setOnImagePhotoClick:^(ModelUserBase *model) {
    ...
    [self.tvAttTopic setBackgroundColor:VIEW_BACKCOLOR1];
    ...
    [self.tvAttUser setBackgroundColor:VIEW_BACKCOLOR1];
