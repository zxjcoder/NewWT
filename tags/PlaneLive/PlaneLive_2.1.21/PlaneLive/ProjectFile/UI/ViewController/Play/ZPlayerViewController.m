...
- (void)setNetworkReachabilityStatus:(ZNetworkReachabilityStatus)status
{
    switch (status) {
        ...
        case ZNetworkReachabilityStatusReachableViaWiFi:
            if (self.isPlaying && self.lastFileUrl) {
                [self setPlayWithFileUrl:self.lastFileUrl];
            }
            break;
        default:
            break;
...
{
    if (self.modelArray
        && [self.modelArray isKindOfClass:[NSArray class]]
        && self.modelArray.count > _index
        && self.practiceArray
        && [self.practiceArray isKindOfClass:[NSArray class]]
        && self.practiceArray.count > _index) {
...
- (void)updateCurriculumAudioPlayer
{
    if (self.modelArray
        ...
        && [self.curriculumArray isKindOfClass:[NSArray class]]
        && self.curriculumArray.count > _index) {
        ModelAudio *modelA = [self.modelArray objectAtIndex:_index];
        if (modelA) {
            //播放的是同一个
...
-(void)setPlayWithFileUrl:(NSString *)file
{
    ...
    if (filePath
        && ![filePath isKindOfClass:[NSNull class]]
        && [[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        NSURL *sourceMovieUrl = [NSURL fileURLWithPath:filePath];
        AVAsset *movieAsset = [AVAsset assetWithURL:sourceMovieUrl];
        //[AVURLAsset URLAssetWithURL:sourceMovieUrl options:nil];
        self.playerItem = [AVPlayerItem playerItemWithAsset:movieAsset];
    } else {
        self.playerItem = [AVPlayerItem playerItemWithURL:[NSURL URLWithString:file]];
    }
...
- (void)updateVideoSlider:(NSTimeInterval)currentTime
{
    ...
    /*if (self.playingModel && (currentTime-self.playingModel.audioPlayTime)>2) {
        self.playingModel.audioPlayTime = currentTime;
        [self setPlayTimeToFile];
    } else {
        self.playingModel.audioPlayTime = currentTime;
...
-(void)setPlayTimeToFile
{
    ...
    GCDGlobalBlock(^{
        @try {
            if (self.dicPlayTime && self.dicPlayTime.allKeys.count > 0) {
...
{
    if (self.playingModel == nil || [self.playingModel isKindOfClass:[NSNull class]]) {
        return;
    }
    if(NSClassFromString(@"MPNowPlayingInfoCenter")) {
        NSMutableDictionary *dicMusicInfo = [NSMutableDictionary dictionary];
        // 设置Singer
        if (self.playingModel.audioAuthor && ![self.playingModel.audioAuthor isKindOfClass:[NSNull class]]) {
            [dicMusicInfo setObject:self.playingModel.audioAuthor forKey:MPMediaItemPropertyArtist];
        }
        // 设置歌曲名
        if (self.playingModel.audioTitle && ![self.playingModel.audioTitle isKindOfClass:[NSNull class]]) {
            [dicMusicInfo setObject:self.playingModel.audioTitle forKey:MPMediaItemPropertyTitle];
        }
        // 设置封面
        UIImage *img = nil;
        if (self.playingModel.audioImage && ![self.playingModel.audioImage isKindOfClass:[NSNull class]] && self.playingModel.audioImage.pathExtension) {
            NSString *imgPath = [[[AppSetting getPictureFilePath] stringByAppendingPathComponent:[Utils stringMD5:self.playingModel.audioImage]] stringByAppendingPathExtension:self.playingModel.audioImage.pathExtension];
            if ([[NSFileManager defaultManager] fileExistsAtPath:imgPath]) {
                img = [UIImage imageWithContentsOfFile:imgPath];
            }
        }
        if (img == nil) {
            img = [SkinManager getImageWithName:@"Icon.png"];
        }
        ...
        //音乐剩余时长
        [dicMusicInfo setObject:[NSNumber numberWithInteger:self.durationTime] forKey:MPMediaItemPropertyPlaybackDuration];
        //音乐当前播放时间
...
{
    if (model && self.playingModel && [self.playingModel.ids isEqualToString:model.ids]) {
