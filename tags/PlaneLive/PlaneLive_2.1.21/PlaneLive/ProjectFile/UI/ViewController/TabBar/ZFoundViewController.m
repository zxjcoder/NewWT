...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self.viewLastPlay setPlayButtonIsAnimation:[[ZPlayerViewController sharedSingleton] isStartPlaying]];
}
...
-(void)innerInit
{
    ...
    [self.tvMain setOnPracticeTypeClick:^(ModelPracticeType *model) {
        if (!weakSelf.isGeting) {
            ...
            [snsV2 getFoundPracticeArrayWithPracticeTypeId:model.ids pageNum:1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
