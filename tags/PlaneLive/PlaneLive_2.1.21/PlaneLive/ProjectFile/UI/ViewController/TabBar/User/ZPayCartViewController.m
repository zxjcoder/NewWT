...
-(void)innerData
{
    NSArray *arrPayCart = [sqlite getLocalPayCartPracticeArrayWithUserId:[AppSetting getUserDetauleId]];
...
-(void)setRefreshHeader
{
    ...
    [snsV2 getPayCartPracticeArrayResultBlock:^(NSArray *array, NSDictionary *result) {
        ...
        [sqlite setLocalPayCartPracticeWithArray:array userId:[AppSetting getUserDetauleId]];
...
-(void)setBuyClick
{
    [StatisticsManager event:kShoppingCart_Pay_Recharge];
    if ([AppSetting getAutoLogin]) {
        ...
        [viewPlayTool setOnBalanceClick:^{
            ...
            [snsV2 getGenerateOrderClearingWithMoney:[NSString stringWithFormat:@"%.2f", priceCount] type:WTPayTypePractice objid:[arrObjIds toString] title:kEmpty payType:(WTPayWayTypeBalance) resultBlock:^(ModelOrderWT *model, NSDictionary *dicResult, NSDictionary *result) {
                [snsV2 updOrderStateWithOrderId:model.order_no resultBlock:^(NSDictionary *result) {
                    ...
                    [sqlite setLocalPayCartPracticeWithArray:arrayNewMain userId:[AppSetting getUserDetauleId]];
