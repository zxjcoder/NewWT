...
-(void)textFieldDidChange:(NSNotification *)sender
{
    ...
    if (content.length > 0) {
        ...
        [snsV1 getSearchUserWithUserId:userId questionId:self.modelQB.ids flag:@"1" content:content pageNum:1 resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
...
-(void)innerData
{
    ...
    self.pageNum = 1;
    NSString *userId = [AppSetting getUserDetauleId];
    [snsV1 getRecommendUserWithUserId:userId questionId:self.modelQB.ids pageNum:self.pageNum resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)setRefreshFooter
{
    ZWEAKSELF
    NSString *userId = [AppSetting getUserDetauleId];
    [snsV1 getSearchUserWithUserId:userId questionId:self.modelQB.ids flag:@"1" content:self.lastContent pageNum:self.pageNum+1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
...
-(void)btnInvitationClick:(ModelUserInvitation *)model itemTVC:(ZInvitationItemTVC *)tvc
{
    ...
    NSString *userId = [AppSetting getUserDetauleId];
    [snsV1 postInviteUserWithUserId:userId hisId:model.userId questionId:self.modelQB.ids resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
