...
-(void)innerLocalData
{
    NSArray *arr = [sqlite getLocalMyMessageArrayWithUserId:[AppSetting getUserDetauleId]];
    if (arr && arr.count > 0) {
...
-(void)setRefreshHeader
{
    ...
    [snsV2 getMyMessageArrayWithPageNum:self.pageNum resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        ...
        [sqlite setLocalMyMessageWithArray:arrResult userId:[AppSetting getUserDetauleId]];
...
-(void)setShowSubscribeVC:(ModelMyMessage *)model
{
    if (model.free_read == 0) {
        ...
        [modelC setIds:model.course_id];
        [modelC setSubscribeId:model.subscribe_id];
        [modelC setTitle:model.title];
        [modelC setFree_read:model.free_read];
...
{
    ZWEAKSELF
    [ZAlertView showWithMessage:@"确定删除该留言吗?" doneCompletion:^{
