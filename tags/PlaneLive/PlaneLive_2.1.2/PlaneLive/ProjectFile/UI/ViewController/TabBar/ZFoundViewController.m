...
-(void)innerInit
{
    ...
    [self.viewCoverFlow setOnPracticeTypeClick:^(ModelPracticeType *model) {
        if (model.arrPractice && model.arrPractice.count > 0) {
            ...
            [weakSelf showPlayVCWithFoundArray:model.arrPractice index:0];
        }
    }];
...
-(void)innerData
{
    ...
    [self.lbTitle setText:[NSString stringWithFormat:@"%@，%@", timeSlot, kListenToSomePractice]];
    ZWEAKSELF
