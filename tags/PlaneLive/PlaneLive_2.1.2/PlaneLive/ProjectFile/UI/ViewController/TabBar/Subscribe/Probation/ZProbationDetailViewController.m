...
-(void)innerInit
{
    ...
    [self.viewTabbar setOnSubscribeClick:^{
        [StatisticsManager event:kSubscription_AcademicProbation_Pay];
        if ([AppSetting getAutoLogin]) {
            ...
            [viewPlayTool setOnBalanceClick:^{
                ...
                ///下订单
                [snsV2 getGenerateOrderWithMoney:weakSelf.model.price type:WTPayTypeSubscribe objid:weakSelf.model.subscribeId title:weakSelf.model.title payType:(WTPayWayTypeBalance) resultBlock:^(ModelOrderWT *model, NSDictionary *dicResult, NSDictionary *result) {
                    ///修改订单状态
...
-(void)innerData
{
    ...
    [snsV2 getCurriculumDetailWebUrlWithCurriculumId:self.model.ids subscribeId:self.model.subscribeId type:0 resultBlock:^(NSString *webUrl, NSDictionary *result) {
