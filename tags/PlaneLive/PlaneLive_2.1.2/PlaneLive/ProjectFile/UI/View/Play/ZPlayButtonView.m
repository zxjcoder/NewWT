...
-(void)innerInitPractice
{
    ...
    [self.btnDownload setImageEdgeInsets:(UIEdgeInsetsMake(10, 10, 10, 10))];
...
-(void)setViewFrame
{
    ...
    CGFloat collectionX = self.width/2-iconS/2;
    CGRect collectionFrame = CGRectMake(collectionX, itemY, iconS, iconS);
    [self.btnCollection setFrame:collectionFrame];
    
    CGFloat lbH = 25;
    CGRect collectionLbFrame = CGRectMake(collectionFrame.origin.x+collectionFrame.size.width, self.height/2-lbH/2, 0, lbH);
    [self.lbCollection setFrame:collectionLbFrame];
    CGFloat collectionLbW = [self.lbCollection getLabelWidthWithMinWidth:0];
    collectionLbFrame.size.width = collectionLbW;
    [self.lbCollection setFrame:collectionLbFrame];
    
    CGRect praiseLbFrame = CGRectMake(0, self.height/2-lbH/2, 0, lbH);
    [self.lbPraise setFrame:praiseLbFrame];
    CGFloat praiseLbW = [self.lbPraise getLabelWidthWithMinWidth:0];
    praiseLbFrame.size.width = praiseLbW;
    praiseLbFrame.origin.x = self.btnCollection.x-praiseLbW-30;
    [self.lbPraise setFrame:praiseLbFrame];
    
    CGRect praiseFrame = CGRectMake(praiseLbFrame.origin.x-iconS, itemY, iconS, iconS);
    [self.btnPraise setFrame:praiseFrame];
    
    CGFloat downloadX = collectionLbFrame.origin.x+collectionLbFrame.size.width+30;
    CGRect downloadFrame = CGRectMake(downloadX, itemY, iconS, iconS);
