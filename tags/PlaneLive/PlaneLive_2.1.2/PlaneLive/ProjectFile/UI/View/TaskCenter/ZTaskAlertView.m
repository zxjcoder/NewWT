...
-(void)innerInit
{
    ...
    self.imgIcon = [[UIImageView alloc] initWithImage:[SkinManager getImageWithName:@"task_alertview_lock_icon"]];
    [self.viewContent addSubview:self.imgIcon];
    
    self.lbTitle = [[UILabel alloc] init];
    [self.lbTitle setText:kUnlockTask];
    ...
    [self.lbContent setTextAlignment:(NSTextAlignmentLeft)];
    ...
    [self.btnSubmit setTitle:kAcceptKey forState:(UIControlStateNormal)];
...
-(void)setViewFrame
{
    ...
    CGFloat topY = 18;
    [self.lbTitle setFrame:CGRectMake(0, topY, contentW, 20)];
    
    CGFloat imgW = 220/2;
    CGFloat imgH = 150/2;
    [self.imgIcon setFrame:CGRectMake(contentW/2-imgW/2, self.lbTitle.y+self.lbTitle.height+topY, imgW, imgH)];
    
    CGRect lbContentFrame = CGRectMake(20, self.imgIcon.y+self.imgIcon.height+topY, contentW-20, 20);
    [self.lbContent setFrame:lbContentFrame];
    CGFloat lbContentH = [self.lbContent getLabelHeightWithMinHeight:20];
    lbContentFrame.size.height = lbContentH;
    [self.lbContent setFrame:lbContentFrame];
    
    [self.viewLine1 setFrame:CGRectMake(0, self.lbContent.y+self.lbContent.height+20, contentW, 1)];
    CGFloat btnH = 40;
    ...
    CGFloat contentH = self.btnCancel.y+self.btnCancel.height;
