...
-(void)innerInit
{
    ...
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, self.cellW, 125)];
    ...
    [self.btnSort setTitle:@"按最新排序" forState:(UIControlStateNormal)];
...
{
    if (self.onSortClick) {
        self.onSortClick(self.btnSort.tag==1);
    }
    if (self.btnSort.tag == 1) {
        [self.btnSort setTag:2];
    } else {
        [self.btnSort setTag:1];
    }
}
