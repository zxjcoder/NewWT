...
-(BOOL)setLocalHomeSubscribeWithArray:(NSArray *)array userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelSubscribe *model in array) {
            ...
            result = [db executeUpdate:@"REPLACE INTO sns_home_subscribe(ids,userId,title,team_name,team_intro,theme_intro,create_time,price,units,illustration,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,user_Id,title,team_name,team_intro,theme_intro,create_time,price,units,illustration]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(BOOL)setLocalHomeCurriculumWithArray:(NSArray *)array userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelSubscribe *model in array) {
            ...
            result = [db executeUpdate:@"REPLACE INTO sns_home_curriculum(ids,userId,title,team_name,team_intro,theme_intro,create_time,price,units,illustration,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,user_Id,title,team_name,team_intro,theme_intro,create_time,price,units,illustration]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(ModelAudio *)checkToDetectWhetherThereALocalDownload
{
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_download_audio WHERE address = %d ORDER BY sortby DESC", (int)ZDownloadStatusStart]];
        if (rs != nil)
        {
            while ([rs next]) {
...
-(ModelAudio *)getLocalDownloadAudioWithUserId:(NSString *)userId
{
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rsPractice = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_download_audio WHERE userId='%@' AND address!='%d' AND address!='%d' ORDER BY sortby DESC LIMIT 0,1", userId, (int)ZDownloadStatusStart, (int)ZDownloadStatusEnd]];
        if (rsPractice != nil)
        {
            while ([rsPractice next]) {
...
-(BOOL)setLocalPurchaseSubscribeWithArray:(NSArray *)array userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelSubscribe *model in array) {
            ...
            result = [db executeUpdate:@"REPLACE INTO sns_purchase_subscribe(ids,userId,title,team_name,team_info,theme_intro,illustration,price,count,units,status,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,team_name,team_info,theme_intro,illustration,price,count,units,status]];
            if (!result) {
                *rollback = YES;
                break;
            }
...
-(BOOL)setLocalPurchaseCurriculumWithArray:(NSArray *)array userId:(NSString *)userId
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        for (ModelSubscribe *model in array) {
            ...
            result = [db executeUpdate:@"REPLACE INTO sns_purchase_curriculum(ids,userId,title,team_name,team_info,theme_intro,illustration,price,count,units,status,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,team_name,team_info,theme_intro,illustration,price,count,units,status]];
            if (!result) {
                *rollback = YES;
                break;
            }
