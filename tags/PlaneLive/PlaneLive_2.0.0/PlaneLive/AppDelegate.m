...
{
    GBLog(@"application forRemoteNotification: %@,  identifier: %@", userInfo, identifier);
}
/// iOS8-10通知消息按钮点击回调事件(本地推送通知)
- (void)application:(UIApplication *)application handleActionWithIdentifier:(nullable NSString *)identifier forLocalNotification:(UILocalNotification *)notification completionHandler:(void(^)())completionHandler
{
    GBLog(@"application forLocalNotification: %@,  identifier: %@", notification.userInfo, identifier);
}
/// iOS9-10通知消息按钮点击回调事件(服务器推送通知)
- (void)application:(UIApplication *)application handleActionWithIdentifier:(nullable NSString *)identifier forRemoteNotification:(NSDictionary *)userInfo withResponseInfo:(NSDictionary *)responseInfo completionHandler:(void(^)())completionHandler
{
    GBLog(@"application withResponseInfo: %@,  identifier: %@", responseInfo, identifier);
}
...
-(void)setUncaughtException
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (errorData && errorData.length > 0) {
            [DataOper postErrorReportWithContent:errorData resultBlock:^{
...
-(void)setRefreshAppConfig
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (modelA && modelA.ids) {
            [AppDelegate app].isPlay = YES;
        } else {
            [AppDelegate app].isPlay = NO;
        }
        [DataOper getAppConfigWithAppVersion:APP_PROJECT_VERSION resultBlock:^(ModelAppConfig *result) {
            ///保存最新的状态
            ...
            if (result.appStatus == 1) {
                if (![result.appVersion isEqualToString:APP_PROJECT_VERSION]) {
                    ...
                    if (isUpd.length == 0) {
                        ...
                        GCDMainBlock(^{
                            [[AppDelegate getTabBarVC] presentViewController:alertVC animated:NO completion:nil];
                        });
                    }
                }
            }
...
//    GCDMainBlock(^{
        ...
        ///处理实务-播放中判断
        [[ZPlayViewController sharedSingleton] stop];
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case AFNetworkReachabilityStatusUnknown://未识别的网络
                weakSelf.isWiFi = NO;
                [[ZPlayViewController sharedSingleton] setNetworkReachabilityStatus:(ZNetworkReachabilityStatusUnknown)];
                break;
            case AFNetworkReachabilityStatusNotReachable://不可达的网络(未连接)
                weakSelf.isWiFi = NO;
                [[ZPlayViewController sharedSingleton] setNetworkReachabilityStatus:(ZNetworkReachabilityStatusNotReachable)];
                break;
            case AFNetworkReachabilityStatusReachableViaWWAN://2G,3G,4G...的网络
                weakSelf.isWiFi = NO;
                [[ZPlayViewController sharedSingleton] setNetworkReachabilityStatus:(ZNetworkReachabilityStatusReachableViaWWAN)];
                break;
            case AFNetworkReachabilityStatusReachableViaWiFi://WiFi的网络
                weakSelf.isWiFi = YES;
                [[ZPlayViewController sharedSingleton] setNetworkReachabilityStatus:(ZNetworkReachabilityStatusReachableViaWiFi)];
                break;
        }
    }];
