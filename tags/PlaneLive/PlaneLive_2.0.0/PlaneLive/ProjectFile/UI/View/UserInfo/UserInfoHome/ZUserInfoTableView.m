...
@property (strong, nonatomic) ZUserInfoShareTVC *tvcShare;
...
-(void)innerInit
{
    ...
    self.tvcShare = [[ZUserInfoShareTVC alloc] initWithReuseIdentifier:@"tvcShare"];
...
{
    if (model) {
        self.arrayMain = @[self.tvcImage, self.tvcQuestion, self.tvcShare, self.tvcCenter];
     
        [self.tvcImage setCellDataWithModel:model];
        
        [self.tvcQuestion setCellDataWithModel:model];
        
        [self.tvcShare setCellDataWithModel:model];
