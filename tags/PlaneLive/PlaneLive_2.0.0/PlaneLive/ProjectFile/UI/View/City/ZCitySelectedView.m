...
-(void)innerInit
{
    ...
    [self.imgline setFrame:CGRectMake(0, self.viewHeader.height-kLineHeight, self.viewHeader.width, kLineHeight)];
    [self.viewHeader addSubview:self.imgline];
    
    self.btnDone = [ZButton buttonWithType:(UIButtonTypeCustom)];
    [self.btnDone setTitle:kClose forState:(UIControlStateNormal)];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:self.tvProvince]) {
        ...
        NSIndexPath *indexPathCity = [NSIndexPath indexPathForRow:0 inSection:0];
        [self.tvCity selectRowAtIndexPath:indexPathCity animated:NO scrollPosition:UITableViewScrollPositionBottom];
