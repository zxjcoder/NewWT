...
-(void)innerInit
{
    ...
    self.cellH = [ZCircleTopicTVC getH];
    ...
    [self.btnMore setTitle:kSayAll forState:(UIControlStateNormal)];
    [self.btnMore setTitle:kSayAll forState:(UIControlStateHighlighted)];
...
-(CGFloat)setCellDataWithModel:(ModelTagType *)model
{
    [self setModelType:model];
    
    [self.lbTitle setText:self.modelType.typeName];
    int index = 0;
    for (ZTopicButton *btnTopic in self.viewContent.subviews) {
        [btnTopic setHidden:YES];
        if (model && model.tagArray.count > 0 && model.tagArray.count > index) {
            [btnTopic setHidden:NO];
            [btnTopic setButtonModel:[model.tagArray objectAtIndex:index]];
        }
        index++;
    }
