...
-(void)setRefreshQuestionData
{
    ...
    self.pageNum = 1;
    NSString *strLocalKey = [NSString stringWithFormat:@"QuestionDetailKey%@",self.modelB.ids];
    [DataOper getQuestionDetailWithQuestionId:self.modelB.ids userId:[AppSetting getUserDetauleId] pageNum:self.pageNum resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)setPublishQuestion
{
    ZWEAKSELF
    NSString *strLocalKey = [NSString stringWithFormat:@"QuestionDetailKey%@",self.modelB.ids];
    [DataOper getQuestionDetailWithQuestionId:self.modelB.ids userId:[AppSetting getUserDetauleId] pageNum:1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [DataOper getQuestionDetailWithQuestionId:self.modelB.ids userId:[AppSetting getUserDetauleId] pageNum:self.pageNum+1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
...
-(void)btnAttentionClick:(ModelQuestionDetail *)model
{
    if ([AppSetting getAutoLogin]) {
        ...
        ZWEAKSELF
        if (model.isAtt) {
            [DataOper postDeleteAttentionWithAId:self.modelD.ids userId:[AppSetting getUserDetauleId] type:@"0" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            [DataOper postAddAttentionWithuserId:[AppSetting getUserDetauleId] hisId:self.modelD.ids type:@"0" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
...
-(void)btnDeleteClick
{
    ...
    [ZAlertView showWithMessage:kCMsgDoneDeleteQuestion doneCompletion:^{
        ...
        [DataOper postDeleteQuestionWithUserId:[AppSetting getUserDetauleId] questionId:weakSelf.modelD.ids resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
