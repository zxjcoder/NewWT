...
-(void)refreshData:(BOOL)isRefresh
{
    ...
    ZWEAKSELF
    self.pageNum = 1;
    [DataOper getUserNoticeListWithUserId:[AppSetting getUserDetauleId] pageNum:self.pageNum resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [DataOper getUserNoticeListWithUserId:[AppSetting getUserDetauleId] pageNum:self.pageNum+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
            if (arrResult && arrResult.count > 0) {
