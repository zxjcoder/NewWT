...
-(void)innerInit
{
    ...
    self.userId = [AppSetting getUserDetauleId];
...
-(void)refreshData:(BOOL)isRefresh
{
    ...
    [DataOper getTopicListWithUserId:[AppSetting getUserDetauleId] typeId:self.modelTT.typeId pageNum:self.pageNum resultBlock:^(NSArray *arrReulst, NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [DataOper getTopicListWithUserId:[AppSetting getUserDetauleId] typeId:self.modelTT.typeId pageNum:self.pageNum+1 resultBlock:^(NSArray *arrReulst, NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
