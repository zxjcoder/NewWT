...
-(void)setAttTopicClickWithModel:(ModelTag *)model
{
    if ([AppSetting getAutoLogin]) {
        ...
        if (model.isAtt) {
            [DataOper postDeleteAttentionWithAId:self.model.tagId userId:[AppSetting getUserDetauleId] type:@"3" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            [DataOper postAddAttentionWithuserId:[AppSetting getUserDetauleId] hisId:self.model.tagId type:@"3" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    [weakSelf setIsAttentioning:NO];
