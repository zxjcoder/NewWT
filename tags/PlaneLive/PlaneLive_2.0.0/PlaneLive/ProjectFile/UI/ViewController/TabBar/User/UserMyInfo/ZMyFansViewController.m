...
-(void)setRefreshData:(BOOL)isRefresh
{
    ...
    [DataOper postMyGetFunsWithUserId:self.modelUB.userId pageNum:self.pageNum resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [DataOper postMyGetFunsWithUserId:self.modelUB.userId pageNum:self.pageNum+1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
...
{
    [DataOper postDeleteMyFunsWithUserId:[AppSetting getUserDetauleId] hisId:model.userId resultBlock:nil errorBlock:nil];
}
