...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self setChangeNavigationAlpha:self.lastNavigationAlpha];
}
...
-(void)innerInit
{
    ...
    [self.tvMain setOnContentOffsetY:^(CGFloat alpha) {
        ...
        [weakSelf setChangeNavigationAlpha:alpha];
    }];
...
-(void)innerData
{
    ...
    ZWEAKSELF
    [DataOper200 getSubscribeRecommendArrayWithSubscribeId:self.model.ids resultBlock:^(ModelSubscribeDetail *model, NSDictionary *result) {
        if (isPop) {
            [ZProgressHUD dismiss];
        }
...
///改变导航栏的透明颜色
-(void)setChangeNavigationAlpha:(CGFloat)alpha
{
    [self setLastNavigationAlpha:alpha];
    if (alpha >= 0.75) {
        [self.navigationController.navigationBar setTintColor:[NAVIGATIONTINTCOLOR colorWithAlphaComponent:alpha]];
    } else {
