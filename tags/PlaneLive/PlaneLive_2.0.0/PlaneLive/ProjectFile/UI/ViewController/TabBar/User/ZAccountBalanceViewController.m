...
-(void)innerInit
{
    ...
    [self.viewMain setOnRechargeClick:^(NSString *money) {
        ...
        [DataOper200 getGenerateOrderWithMoney:money type:WTPayTypeRecharge objid:nil title:title payType:(WTPayWayTypeBalance) resultBlock:^(ModelOrderWT *model, id resultThree, NSDictionary *result) {
...
-(void)innerData
{
    ...
    if (arrOrder && arrOrder.count > 0) {
        ...
        [DataOper200 updOrderStateWithOrderIds:arrOrderId resultBlock:^(NSDictionary *result) {
            
            [sqlite updLocalOrderDetailWithOrderIds:arrOrderId];
        } errorBlock:nil];
        
//        __weak ModelOrderWT *modelOrder = (ModelOrderWT*)arrOrder.firstObject;
//        [DataOper200 updOrderStateWithOrderId:modelOrder.order_no resultBlock:^(NSDictionary *result) {
//            if (modelOrder) {
//                [modelOrder setStatus:1];
//            }
//            [sqlite setLocalOrderDetailWithModel:modelOrder userId:[AppSetting getUserDetauleId]];
//        } errorBlock:nil];
    }
}
///向苹果发起支付
