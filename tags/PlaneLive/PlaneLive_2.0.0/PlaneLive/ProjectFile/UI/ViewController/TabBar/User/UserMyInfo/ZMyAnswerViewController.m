...
{
    [DataOper postDeleteAnswerWithAnswerId:model.aid userId:[AppSetting getUserDetauleId] resultBlock:nil errorBlock:nil];
}
...
-(void)setRefreshAnswerData:(BOOL)isRefresh
{
    ...
    [DataOper postMyGetReplyWithUserId:self.modelUB.userId pageNum:self.pageNumAnswer resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [DataOper postMyGetReplyWithUserId:self.modelUB.userId pageNum:self.pageNumAnswer+1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
