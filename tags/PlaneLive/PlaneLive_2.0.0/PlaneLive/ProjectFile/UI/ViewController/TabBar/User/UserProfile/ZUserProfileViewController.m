...
-(void)innerData
{
    ...
    [DataOper postGetUserInfoWithUserId:[AppSetting getUserDetauleId] hisId:self.model.userId resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnAttClick
{
    ...
    if (self.modelUser.isAtt) {
        ...
        ZWEAKSELF
        [DataOper postDeleteAttentionWithAId:self.model.userId userId:[AppSetting getUserDetauleId] type:@"1" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
        ...
        [self.tvcButton setCellDataWithModel:self.modelUser];
        ZWEAKSELF
        [DataOper postAddAttentionWithuserId:[AppSetting getUserDetauleId] hisId:self.model.userId type:@"1" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
                [weakSelf setIsAtting:NO];
...
{
    ZWEAKSELF
    [DataOper postAddBlackListWithUserId:[AppSetting getUserDetauleId] addUserId:self.model.userId resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
