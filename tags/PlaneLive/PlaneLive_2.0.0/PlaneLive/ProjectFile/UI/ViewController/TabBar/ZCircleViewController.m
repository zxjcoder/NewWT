...
-(void)innerInit
{
    ...
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, contentY, contentW, contentH)];
...
-(void)innerEvent
{
    ...
    [self.tvHot setOnHotArticleViewClick:^(ModelHotArticle *model) {
        //添加统计事件
        [StatisticsManager event:kEvent_Circle_News category:kCategory_Circle];
        
        ZWebViewController *webVC =[[ZWebViewController alloc] init];
        [webVC setTitle:kNewsDetail];
...
-(void)showCircleQuestionVC
{
    if ([AppSetting getAutoLogin]) {
        //添加统计事件
        [StatisticsManager event:kEvent_Circle_Question category:kCategory_Circle];
...
-(void)setRefreshHotHeader
{
    ZWEAKSELF
    self.pageNumHot = 1;
    [DataOper200 getCircleRemdListWithPageNum:self.pageNumHot resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [DataOper200 getCircleRemdListWithPageNum:self.pageNumHot+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
-(void)setRefreshNewHeader
{
    ZWEAKSELF
    self.pageNumNew = 1;
    [DataOper getCircleLatestListWithPageNum:self.pageNumNew resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [DataOper getCircleLatestListWithPageNum:self.pageNumNew+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
-(void)setRefreshDynamicHeader
{
    ...
    [DataOper getCircleTrendListWithUserId:[AppSetting getUserDetauleId] pageNum:self.pageNumDynamic resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [DataOper getCircleTrendListWithUserId:[AppSetting getUserDetauleId] pageNum:self.pageNumDynamic+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
-(void)setRefreshTopicHeader
{
    ZWEAKSELF
    self.pageNumTopic = 1;
    [DataOper getCircleTopicListWithUserId:[AppSetting getUserDetauleId] pageNum:self.pageNumTopic resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [DataOper getCircleTopicListWithUserId:[AppSetting getUserDetauleId] pageNum:self.pageNumTopic+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
