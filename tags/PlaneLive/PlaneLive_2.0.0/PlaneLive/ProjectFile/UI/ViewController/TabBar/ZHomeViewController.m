...
-(void)innerInit
{
        ...
        switch (model.type) {
            case ZBannerTypePractice://实务
            {
                ...
                if (modelP) {
                    ...
                    [DataOper getQuerySpeechDetailWithSpeechId:model.code userId:[AppSetting getUserDetauleId] resultBlock:^(NSDictionary *result) {
                        GCDMainBlock(^{
            ...
            case ZBannerTypeSubscribe://订阅
            {
                ...
                if (modelSD && modelSD.ids) {
                    if (!modelSD.isSubscribe) {
                        ZSubscribeDetailViewController *itemVC = [[ZSubscribeDetailViewController alloc] init];
                        [itemVC setViewDataWithModel:modelSD];
                        [itemVC setViewDataWithModelDetail:modelSD];
                        ...
                        [modelC setSubscribeId:modelSD.ids];
                        modelC.title = modelSD.title;
                        modelC.team_name = modelSD.team_name;
                        modelC.team_intro = modelSD.team_info;
                        modelC.illustration = modelSD.illustration;
                        modelC.course_picture = modelSD.course_picture;
                        [itemVC setViewDataWithModel:modelC];
                        [itemVC setHidesBottomBarWhenPushed:YES];
                        [weakSelf.navigationController pushViewController:itemVC animated:YES];
                    }
                } else {
                    [ZProgressHUD showMessage:kGetSubscriptioning];
                    [DataOper200 getSubscribeRecommendArrayWithSubscribeId:model.code resultBlock:^(ModelSubscribeDetail *model, NSDictionary *result) {
                        [ZProgressHUD dismiss];
                        if (model) {
                            if (!model.isSubscribe) {
                                ...
                                modelC.title = model.title;
                                modelC.team_name = model.team_name;
                                modelC.team_intro = model.team_info;
                                modelC.illustration = model.illustration;
                                modelC.course_picture = model.course_picture;
                                [itemVC setViewDataWithModel:modelC];
                                [itemVC setHidesBottomBarWhenPushed:YES];
                                [weakSelf.navigationController pushViewController:itemVC animated:YES];
                            }
    ...
    [self.tvMain setOnAllSubscribeClick:^{
        ZSubscribeListViewController *itemVC = [[ZSubscribeListViewController alloc] init];
        [itemVC setHidesBottomBarWhenPushed:YES];
        [weakSelf.navigationController pushViewController:itemVC animated:YES];
    }];
    ///订阅单个点击事件
...
-(void)setSearchView
{
    ...
    [self.viewSearch setOnPracticeRefreshFooter:^{
        [DataOper200 getHomeSearchPracticeWithParam:weakSelf.lastSearchText pageNum:weakSelf.pageNumPractice+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
    ...
    [self.viewSearch setOnSubscribeRefreshFooter:^{
        [DataOper200 getHomeSearchSubscribeWithParam:weakSelf.lastSearchText pageNum:weakSelf.pageNumSubscribe+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [DataOper200 getHomeDataWithResultBlock:^(NSArray *arrBanner, NSArray *arrPractice, NSArray *arrQuestion, NSArray *arrSubscribe, NSDictionary *result) {
...
-(void)setSearchContentWithText:(NSString *)text
{
    if (![text isEqualToString:self.lastSearchText]) {
        ...
        [DataOper200 getHomeSearchPracticeWithParam:text pageNum:self.pageNumPractice resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        ...
        [DataOper200 getHomeSearchSubscribeWithParam:text pageNum:self.pageNumSubscribe resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [DataOper200 getHomeSearchPracticeWithParam:self.searchBar.text.toTrim pageNum:self.pageNumPractice+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [DataOper200 getHomeSearchSubscribeWithParam:self.searchBar.text.toTrim pageNum:self.pageNumSubscribe+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
