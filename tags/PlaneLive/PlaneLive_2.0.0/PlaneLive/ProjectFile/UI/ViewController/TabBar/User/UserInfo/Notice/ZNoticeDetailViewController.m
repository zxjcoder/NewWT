...
-(void)refreshData:(BOOL)isRefresh
{
    ...
    ZWEAKSELF
    self.pageNum = 1;
    [DataOper getUserNoticeListWithNoticeId:self.modelN.ids userId:[AppSetting getUserDetauleId] pageNum:self.pageNum resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [DataOper getUserNoticeListWithNoticeId:self.modelN.ids userId:[AppSetting getUserDetauleId] pageNum:self.pageNum+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
