...
-(void)innerInit
{
    ...
    [self setLeftButtonWithImage:@"btn_wentixiangqing_zhankai"];
...
-(void)btnLoginClick
{
    ...
    [DataOper postLoginWithAccount:account password:pwd resultBlock:^(ModelUser *resultModel, NSDictionary *result) {
        GCDMainBlock(^{
    ...
    [ShareSDK authorize:type settings:nil onStateChanged:^(SSDKResponseState state, SSDKUser *user, NSError *error) {
        switch (state) {
            case SSDKResponseStateSuccess:
            {
                ...
                [DataOper postLoginWithOpenid:uid unionid:unionid flag:flag nickname:nickname headImg:profileImage sex:sex address:address resultBlock:^(ModelUser *resultModel, NSDictionary *result) {
                    GCDMainBlock(^(void){
                        [ZProgressHUD dismiss];
