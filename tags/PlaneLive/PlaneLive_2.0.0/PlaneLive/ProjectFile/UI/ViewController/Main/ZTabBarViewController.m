...
-(void)innerInit
{
    ...
    [self.tabBar setBackgroundImage:[UIImage createImageWithColor:TABBARBACKCOLOR]];
    [self.tabBar setTintColor:TABBARTITLECOLOR];
    [self.tabBar setBarTintColor:TABBARBACKCOLOR];
}
///设置主界面VC
-(void)setItemIcon
{
    if (self.viewControllers.count == 0) {
        ...
        [self setAddChildWithViewController:itemVC3 image:nil selectedImage:nil title:nil];
...
{
    [[ZPlayViewController sharedSingleton] setPlayRemoteControlReceivedWithEvent:sender.object];
}
///改变Tabbar选中索引
...
-(void)setAppNumberChange:(NSNotification *)sender
{
    if (self.viewControllers.count > 0) {
        if ([AppSetting getAutoLogin]) {
            ...
            if (number == 0) {
                //隐藏
                self.tabBar.items.lastObject.badgeValue = nil;
                [[AppDelegate app] clearApplicationIcon];
            } else {
                //显示
                self.tabBar.items.lastObject.badgeValue = [NSString stringWithFormat:@"%d", number];
    ...
    if (ids && ![ids isKindOfClass:[NSNull class]]) {
        ...
        if (modelP) {
           ...
           [(ZRootViewController *)self.viewControllers.firstObject pushViewController:itemVC animated:NO];
            ...
            [DataOper getQuerySpeechDetailWithSpeechId:ids userId:[AppSetting getUserDetauleId] resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
...
-(void)setShowSubscribeDetailVCWithIds:(NSString *)ids
{
    ZWEAKSELF
    [ZProgressHUD showMessage:kGetPracticeing];
    [DataOper200 getSubscribeRecommendArrayWithSubscribeId:ids resultBlock:^(ModelSubscribeDetail *model, NSDictionary *result) {
        [ZProgressHUD dismiss];
