...
-(void)btnRightClick
{
    ...
    if (questionId && questionId.length > 0) {
        ...
        if (answerId && answerId.length > 0) {
            ...
            [DataOper postUpdAnswerWithQuestionId:questionId answerId:answerId content:strContent userId:userId answerImg:arrImage resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    [ZProgressHUD dismiss];
                    [ZProgressHUD showSuccess:kEditorSuccessful];
            ...
            [DataOper postAddAnswerWithQuestionId:questionId content:strContent userId:userId answerImg:arrImage resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    [weakSelf postPublishQuestionNotification];
