...
+(void)load:(NSString*)loginaccount
{
    ...
    if (dicAllSettings == nil) dicAllSettings = [[NSMutableDictionary dictionary] retain];
#else
    if (dicAllSettings == nil) dicAllSettings = [NSMutableDictionary dictionary];
#endif
...
{
    [dicCurrUserSettings setObject:[NSNumber numberWithInt:value] forKey:@"Rate"];
}
...
+(void)createFileDir
{
    ...
    [[NSFileManager defaultManager] setAttributes:dicAttributes
                                     ofItemAtPath:[AppSetting getDocumentPath]
                                            error:nil];
    [[NSFileManager defaultManager] setAttributes:dicAttributes
                                     ofItemAtPath:[AppSetting getLibraryCachePath]
                                            error:nil];
}
