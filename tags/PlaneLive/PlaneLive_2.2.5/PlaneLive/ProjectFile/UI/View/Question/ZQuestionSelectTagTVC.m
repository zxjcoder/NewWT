...
-(void)innerInit
{
    ...
    self.imgIcon = [[ZImageView alloc] initWithImage:[SkinManager getDefaultPhoto]];
...
-(CGFloat)setCellDataWithModel:(ModelTag *)model
{
    [self setModel:model];
    if (model) {
        [self.imgIcon setPhotoURLStr:model.tagLogo placeImage:[SkinManager getDefaultPhoto]];
