...
-(void)innerInit
{
    ...
    [self.lbTeam setFont:[ZFont systemFontOfSize:kFont_Min_Size]];
...
-(CGFloat)setCellDataWithModel:(ModelSubscribe *)model
{
    ...
    if (model.units && model.units.length > 0) {
        [self.lbPrice setText:[NSString stringWithFormat:@"%.2f%@/%@", [model.price floatValue], kPlaneMoney, model.units]];
    } else {
