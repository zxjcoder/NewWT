...
-(void)innerTextInit:(NSString *)text imageUrl:(NSString *)imageUrl
{
    ...
    if (!self.imgIcon) {
        ...
        [self.imgIcon setImageURLStr:imageUrl placeImage:[SkinManager getDefaultImage]];
        [self.imgIcon setUserInteractionEnabled:NO];
        [self addSubview:self.imgIcon];
    }
    if (!self.lbTitle) {
...
-(void)innerDownloadInit
{
    ...
    if (!self.shapeLayer) {
        //创建出圆形贝塞尔曲线
        self.shapeLayer = [self createShapeLayerWithSize:self.width/2 lineWith:1.2 color:COLORVIEWBACKCOLOR1];
...
-(void)setIconFrame:(CGRect)frame
{
    ...
    [self.lbTitle setFrame:CGRectMake(0, self.height-20, self.width, 18)];
}
