...
-(void)innerInit
{
    ZWEAKSELF
    /// TODO-ZWW计算播放页面高度
    CGFloat infoContentH = 80+[ZPlayObjectInfoView getViewImageHeight];
    CGFloat infoY = APP_TOP_HEIGHT;
    CGFloat playBottom = (APP_FRAME_HEIGHT-infoContentH-kZPlayBottomViewHeight-kZPlayFunctionViewHeight-kZPlayButtonViewHeight)/3;
    CGFloat infoH = infoContentH+playBottom*2;
    CGFloat functionY = infoY+infoH+kZPlayButtonViewHeight;
...
-(void)setInnerPlayTabbarWithType:(ZPlayTabBarViewType)type
{
    ...
    /// 下载点赞
    viewButton = [[ZPlayButtonView alloc] initWithPoint:CGPointMake(0, viewObjectInfo.y+viewObjectInfo.height) type:type];
    //下载按钮
