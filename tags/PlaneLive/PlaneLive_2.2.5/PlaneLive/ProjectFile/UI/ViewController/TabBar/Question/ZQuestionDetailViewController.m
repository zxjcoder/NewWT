...
-(void)setRefreshQuestionData
{
    ...
    self.pageNum = 1;
    NSString *strLocalKey = [NSString stringWithFormat:@"QuestionDetailKey%@",self.modelB.ids];
    [snsV1 getQuestionDetailWithQuestionId:self.modelB.ids userId:kLoginUserId pageNum:self.pageNum resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)setPublishQuestion
{
    ZWEAKSELF
    NSString *strLocalKey = [NSString stringWithFormat:@"QuestionDetailKey%@",self.modelB.ids];
    [snsV1 getQuestionDetailWithQuestionId:self.modelB.ids userId:kLoginUserId pageNum:1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [snsV1 getQuestionDetailWithQuestionId:self.modelB.ids userId:kLoginUserId pageNum:self.pageNum+1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
...
-(void)btnAttentionClick:(ModelQuestionDetail *)model
{
    if ([AppSetting getAutoLogin] && ![[AppSetting getUserId] isEqualToString:kUserAuditId]) {
        ...
        ZWEAKSELF
        if (model.isAtt) {
            [snsV1 postDeleteAttentionWithAId:self.modelD.ids userId:kLoginUserId type:@"0" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            [snsV1 postAddAttentionWithuserId:kLoginUserId hisId:self.modelD.ids type:@"0" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
...
-(void)btnDeleteClick
{
    ...
    ZWEAKSELF
    [ZAlertPromptView showWithMessage:kCMsgDoneDeleteQuestion completionBlock:^{
        [weakSelf setIsDeleteing:YES];
        [ZProgressHUD showMessage:kCMsgDeleting];
        [snsV1 postDeleteQuestionWithUserId:kLoginUserId questionId:weakSelf.modelD.ids resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
