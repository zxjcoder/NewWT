    ...
    if ([AppDelegate app].isPlay) {
        ...
        if (!self.viewPlay) {
            ...
            if ([self isMemberOfClass:[ZHomeViewController class]] ||
                ...
                [self isMemberOfClass:[ZFoundViewController class]] ||
                [self isMemberOfClass:[ZUserInfoViewController class]]) {
                self.viewPlay = [[ZGlobalPlayView alloc] initWithFrame:(CGRectMake(0, APP_FRAME_HEIGHT-APP_TABBAR_HEIGHT-[ZGlobalPlayView getH], APP_FRAME_WIDTH, [ZGlobalPlayView getH]))];
...
-(void)setPlayVCBlock
{
    ...
    [[ZPlayerViewController sharedSingleton] setOnTrackWillPlaying:^(NSInteger duration) {
        if (weakSelf.viewPlay) {
            [weakSelf.viewPlay setPlayStatus:true];
        }
    }];
    ...
    [[ZPlayerViewController sharedSingleton] setOnTrackPlayFailed:^(NSError *error) {
        if (weakSelf.viewPlay) {
            [weakSelf.viewPlay setPlayStatus:false];
        }
    }];
    [[ZPlayerViewController sharedSingleton] setOnTrackPlayStatusChange:^(BOOL isPlaying) {
        if (weakSelf.viewPlay) {
            [weakSelf.viewPlay setPlayStatus:isPlaying];
        }
    }];
    [self.viewPlay setPlayStatus:[ZPlayerViewController sharedSingleton].isPlaying];
    [self.viewPlay setViewData:[ZPlayerViewController sharedSingleton].modelTrack row:[ZPlayerViewController sharedSingleton].playIndex];
...
-(void)delPlayVCBlock
{
    [[ZPlayerViewController sharedSingleton] setOnTrackPlayStatusChange:nil];
...
-(void)showPhotoBrowserWithArray:(NSArray *)array index:(NSInteger)index
{
    ...
    [itemRVC setModalPresentationStyle:UIModalPresentationPopover];
...
-(UIButton *)getCartButton
{
    ...
    [lbCount setTag:100];
    [lbCount setBackgroundColor:COLORCONTENT4];
...
-(void)setBackButtonWithLeft
{
    ...
    [btnBack setImage:[UIImage imageNamed:@"arrow_left_gra1"] forState:(UIControlStateNormal)];
    [btnBack setImage:[UIImage imageNamed:@"arrow_left_gra1"] forState:(UIControlStateHighlighted)];
...
{
    NSLog(@"scrollViewDelegateNotification %@", sender.object);
