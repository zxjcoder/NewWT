...
@interface ZLoginViewController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>
{
    CGRect _tvFrame;
    NSInteger _rowIndex;
}
...
/// 只要在滚动，就会调用这个方法（监听scrollView的滚动）
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    if (scrollView.contentOffset.y < -self.view.height/4) {
        [self dismissVCEvent:nil];
    }
}
/// 已经停止减速，就会调用这个方法（停止滚动）
