...
-(void)setRefreshData:(BOOL)isRefresh
{
    ...
    [snsV1 postMyGetFunsWithUserId:self.modelUB.userId pageNum:self.pageNum resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [snsV1 postMyGetFunsWithUserId:self.modelUB.userId pageNum:self.pageNum+1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
            [weakSelf.tvMain endRefreshFooter];
            [weakSelf setViewDataWithDictionary:result isHeader:NO isRefresh:NO];
        });
    } errorBlock:^(NSString *msg) {
        GCDMainBlock(^{
            [weakSelf.tvMain endRefreshFooter];
...
{
    [snsV1 postDeleteMyFunsWithUserId:kLoginUserId hisId:model.userId resultBlock:nil errorBlock:nil];
}
