...
///获取大图片地址
+(NSString *)getMaxPicture:(NSString *)imagePath
{
    if (imagePath == nil || imagePath.length == 0) {
        return imagePath;
    }
    return [imagePath stringByAppendingString:@"?imageView2/1/w/600/h/600"];;
}
///获取中图片地址
+(NSString *)getMiddlePicture:(NSString *)imagePath
{
    if (imagePath == nil || imagePath.length == 0) {
        return imagePath;
    }
    return [imagePath stringByAppendingString:@"?imageView2/1/w/300/h/300"];;
}
///获取小图片地址
+(NSString *)getMinPicture:(NSString *)imagePath
{
    ...
    return [imagePath stringByAppendingString:@"?imageView2/1/w/100/h/100"];;
}
///是否是自己
+(BOOL)isMyUserId:(NSString *)userId
{
    if (userId && [userId isEqualToString:kLoginUserId]) {
        return YES;
    }
    return NO;
}
...
//传入 秒  得到 分钟
+(NSString *)getMinuteFromSS:(NSString *)totalTime
{
    if (totalTime && ![totalTime isKindOfClass:[NSNull class]]) {
        NSInteger seconds = [totalTime integerValue];
        if (seconds <= 0) {
        ...
        if ([str_hour isEqualToString:@"00"]) {
            if ([str_minute isEqualToString:@"00"]) {
                return [NSString stringWithFormat:@"1%@", kMinute];
            } else {
                return [NSString stringWithFormat:@"%ld%@", ([str_minute integerValue]), kMinute];
            }
        }
        return [NSString stringWithFormat:@"%ld%@", ([str_hour integerValue]*60+[str_minute integerValue]), kMinute];
    }
    return [NSString stringWithFormat:@"0%@", kMinute];
}
//传入 秒  得到 xx:xx:xx
+(NSString *)getHHMMSSFromSSTime:(NSInteger)totalTime
{
    if (totalTime <= 0) {
        return @"00:00";
    }
...
///写文件
+ (BOOL)writeDictionary:(NSDictionary *)dic toFileAtPath:(NSString *)aPath
{
    if (dic) {
        return [dic writeToFile:aPath atomically:YES];
    }
    return NO;
}
