...
@interface ZWebContentViewController ()<ZWebViewDelegate,SDKDownloadMgrDelegate>
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.isCourse) {
        ...
        if ([ZPlayerViewController sharedSingleton].modelTrack.trackId == self.modelTrack.trackId) {
            [self.viewPlaybackProgress setTotalDuration:[XMSDKPlayer sharedPlayer].currentTrack.duration];
        }
    }
        ...
        if ([key isEqualToString:@"startdownload"]) {
            ...
            [weakSelf setMessageUnPraise:value];
                ...
                switch (weakSelf.modelTrack.dataType) {
                    ...
                    {
                        if (weakSelf.modelP &&
                            [ZPlayerViewController sharedSingleton].modelTrack &&
                            [ZPlayerViewController sharedSingleton].modelTrack.trackId == [[weakSelf.modelP.ids stringByAppendingString:kMultipleZeros] integerValue]) {
                    ...
                    default:
                    {
                        if (weakSelf.modelC &&
                            [ZPlayerViewController sharedSingleton].modelTrack &&
                            [ZPlayerViewController sharedSingleton].modelTrack.trackId == [weakSelf.modelC.ids integerValue]) {
                            [weakSelf setShowPlayVC];
                        } else {
...
-(void)setViewDataWithModel:(ModelEntity *)model isCourse:(BOOL)isCourse
{
    ...
    if ([model isKindOfClass:[ModelPractice class]]) {
        ...
        [sqlite setLocalPlayPracticeDetailWithModel:self.modelP userId:[AppSetting getUserDetauleId]];
        ...
        [sqlite setLocalPlayCurriculumDetailWithModel:self.modelC userId:[AppSetting getUserDetauleId]];
    }
}
-(id)getWebView
{
...
-(void)webViewDidFinishLoad:(ZWebView *)webView
{
    ...
    //设置播放进度
    [self setPlayTimeWithCurrentTime:self.currentSecond totalTime:[[XMSDKPlayer sharedPlayer] currentTrack].duration];
...
- (void)XMTrackDownloadDidFinished:(XMTrack *)track
{
    ...
    GBLog(@"XMTrackDownloadDidFinished");
}
///下载开始时被调用
