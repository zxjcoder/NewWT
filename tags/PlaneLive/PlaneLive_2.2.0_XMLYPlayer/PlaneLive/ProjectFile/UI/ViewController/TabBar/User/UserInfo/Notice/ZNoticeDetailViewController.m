...
-(void)refreshData:(BOOL)isRefresh
{
    ...
    ZWEAKSELF
    self.pageNum = 1;
    [snsV1 getUserNoticeListWithNoticeId:self.modelN.ids userId:[AppSetting getUserDetauleId] pageNum:self.pageNum resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [snsV1 getUserNoticeListWithNoticeId:self.modelN.ids userId:[AppSetting getUserDetauleId] pageNum:self.pageNum+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
