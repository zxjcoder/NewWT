...
-(void)innerInit
{
    ...
    [self.viewMain setOnPracticeRowClick:^(ModelCollection *model) {
        ModelPractice *modelP = [sqlite getLocalPlayPracticeDetailModelWithId:model.hotspot_id userId:[AppSetting getUserDetauleId]];
        if (modelP) {
...
-(void)setShowSubscribeVC:(ModelCollection *)model
{
    if (model.free_read == 0) {
        ModelCurriculum *modelC = [sqlite getLocalPlayCurriculumDetailWithUserId:[AppSetting getUserDetauleId] ids:model.hotspot_id];
...
{
    [snsV1 getDelCollectionWithUserId:[AppSetting getUserDetauleId] cid:model.hotspot_id type:@"7" resultBlock:nil errorBlock:nil];
}
...
-(void)setRefreshSubscribeData:(BOOL)isRefresh
{
    ...
    [snsV2 getMyCollectionWithPageNum:self.pageNumSubscribe resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [snsV2 getMyCollectionWithPageNum:self.pageNumSubscribe+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
