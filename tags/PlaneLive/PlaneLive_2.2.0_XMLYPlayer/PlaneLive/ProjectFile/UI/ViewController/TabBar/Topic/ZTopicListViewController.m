...
-(void)setQuestionRefreshHeader
{
    ...
    [snsV1 getTopicDetailWithTopicId:self.model.tagId userId:[AppSetting getUserDetauleId] pageNum:self.pageQuestionNum resultBlock:^(NSArray *arrResult, ModelTag *modelTag, NSDictionary *dicResult) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [snsV1 getTopicDetailWithTopicId:self.model.tagId userId:[AppSetting getUserDetauleId] pageNum:self.pageQuestionNum+1 resultBlock:^(NSArray *arrResult, ModelTag *modelTag, NSDictionary *dicResult) {
        GCDMainBlock(^{
...
-(void)setPracticeRefreshHeader
{
    ...
    [snsV1 getTopicPracticeArrayWithTopicId:self.model.tagId userId:[AppSetting getUserDetauleId] pageNum:self.pagePracticeNum resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [snsV1 getTopicPracticeArrayWithTopicId:self.model.tagId userId:[AppSetting getUserDetauleId] pageNum:self.pagePracticeNum+1 resultBlock:^(NSArray *arrResult, NSDictionary *dicResult) {
        GCDMainBlock(^{
...
-(void)setAttTopicClickWithModel:(ModelTag *)model
{
    if ([AppSetting getAutoLogin]) {
        ...
        if (model.isAtt) {
            [snsV1 postDeleteAttentionWithAId:self.model.tagId userId:[AppSetting getUserDetauleId] type:@"3" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            [snsV1 postAddAttentionWithuserId:[AppSetting getUserDetauleId] hisId:self.model.tagId type:@"3" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    [weakSelf setIsAttentioning:NO];
