...
-(void)setRefreshAnswerData
{
    ZWEAKSELF
    self.pageNum = 1;
    [snsV1 getAnswerDetailWithAnswerId:self.modelAB.ids userId:[AppSetting getUserDetauleId] commentId:self.modelAC.ids pageNum:self.pageNum resultBlock:^(NSArray *arrComment, ModelAnswerBase *modelAnswer, ModelAnswerComment *modelDefaultComment, NSDictionary *dicResult) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [snsV1 getAnswerDetailWithAnswerId:self.modelAB.ids userId:[AppSetting getUserDetauleId] commentId:self.modelAC.ids pageNum:self.pageNum+1 resultBlock:^(NSArray *arrComment, ModelAnswerBase *modelAnswer, ModelAnswerComment *modelDefaultComment, NSDictionary *dicResult) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
...
-(void)btnPublishClick:(NSString *)content
{
    ...
    if ([AppSetting getAutoLogin]) {
        ...
        if (self.commentType == 0) {
            ...
            [snsV1 postSaveCommentWithUserId:[AppSetting getUserDetauleId] content:content objId:self.modelAB.ids type:@"0" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            if (self.commentType == 1) {
                ///回复对答案评论的内容
                [snsV1 postCommentReplyWithUserId:[AppSetting getUserDetauleId] content:content replyUserId:self.modelLastAC.userId commentId:self.modelLastAC.ids parent_id:kZero resultBlock:^(NSDictionary *result) {
                    GCDMainBlock(^{
                ...
                ///回复对评论回复的内容
                [snsV1 postCommentReplyWithUserId:[AppSetting getUserDetauleId] content:content replyUserId:self.modelLastCR.user_id commentId:self.modelLastCR.commentId parent_id:self.modelLastCR.ids resultBlock:^(NSDictionary *result) {
                    GCDMainBlock(^{
                        [ZProgressHUD showSuccess:kCMsgReplySuccess];
...
-(void)btnAgreeClick
{
    ...
    if ([AppSetting getAutoLogin]) {
        ...
        if (self.modelAB.isAgree == 0) {
            [snsV1 postClickLikeWithAId:self.modelAB.ids userId:[AppSetting getUserDetauleId] type:@"2" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            [snsV1 postClickUnLikeWithAId:self.modelAB.ids userId:[AppSetting getUserDetauleId] type:@"2" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    [weakSelf setIsAgreeing:NO];
...
-(void)btnCollectionClick
{
    if ([AppSetting getAutoLogin]) {
        ...
        [snsV1 getAddCollectionWithUserId:[AppSetting getUserDetauleId] cid:self.modelAB.ids flag:4 type:@"6" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
...
-(void)shareWithType:(WTPlatformType)type
{
    ...
    [ShareManager shareWithTitle:title content:content imgObj:image webUrl:webUrl platformType:type resultBlock:^(bool isSuccess, NSString *msg) {
        if (!isSuccess && msg) {
            ...
            if ([AppSetting getAutoLogin]) {
                [snsV1 postAddShareInfoWithShareId:weakSelf.modelAB.ids userId:[AppSetting getUserDetauleId] resultBlock:nil errorBlock:nil];
            }
        }
    }];
}
...
-(void)btnDeleteClick
{
    ...
    [ZAlertView showWithMessage:kCMsgDoneDeleteAnswer doneCompletion:^{
        ...
        [snsV1 postDeleteAnswerWithAnswerId:weakSelf.modelAB.ids userId:[AppSetting getUserDetauleId] resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
            ...
            switch (buttonIndex) {
                case 0://删除
                {
                    if (!self.isDeleteing) {
                        ...
                        [snsV1 postCommentReplyWithUserId:[AppSetting getUserDetauleId] ids:self.modelLastAC.ids type:0 resultBlock:^(NSDictionary *result) {
                            GCDMainBlock(^{
                                [weakSelf setIsDeleteing:NO];
            ...
            switch (buttonIndex) {
                case 0://删除
                {
                    if (!self.isDeleteing) {
                        ZWEAKSELF
                        self.isDeleteing = YES;
                        [snsV1 postCommentReplyWithUserId:[AppSetting getUserDetauleId] ids:self.modelLastCR.ids type:1 resultBlock:^(NSDictionary *result) {
                            GCDMainBlock(^{
                                [weakSelf setIsDeleteing:NO];
