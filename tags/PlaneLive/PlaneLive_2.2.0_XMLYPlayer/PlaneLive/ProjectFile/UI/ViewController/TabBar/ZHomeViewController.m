...
-(void)innerInit
{
                ...
                [snsV1 getQuerySpeechDetailWithSpeechId:model.code userId:[AppSetting getUserDetauleId] resultBlock:^(NSDictionary *result) {
                    GCDMainBlock(^{
                        ...
                        if (dicReuslt && [dicReuslt isKindOfClass:[NSDictionary class]]) {
                            ...
                            [sqlite setLocalPlayPracticeDetailWithModel:modelPractice userId:[AppSetting getUserDetauleId]];
    ...
    [self.tvMain setOnPracticeClick:^(NSArray *array, NSInteger row) {
        if (array.count > 0 && row < array.count) {
            ...
            if (modelP && modelP.ids && modelP.title) {
                [StatisticsManager event:kHomepage_Practice_List_Item dictionary:@{kObjectId:modelP.ids,kObjectTitle:modelP.title,kObjectUser:[AppSetting getUserDetauleId]}];
            }
        }
    ...
    [self.tvMain setOnSubscribeClick:^(ModelSubscribe *model) {
        if (model && model.ids && model.title) {
            [StatisticsManager event:kHomepage_Subscription_List_Item dictionary:@{kObjectId:model.ids,kObjectTitle:model.title,kObjectUser:[AppSetting getUserDetauleId]}];
        }
        switch (model.type) {
            case 1:
            {
...
-(void)setSearchView
{
    ...
    [self.viewSearch setOnPracticeClick:^(NSArray *array, NSInteger row) {
        if (array.count > 0 && row < array.count) {
            ModelPractice *modelP = [array objectAtIndex:row];
            if (modelP && modelP.ids && modelP.title) {
                [StatisticsManager event:kHomepage_Search_Practice_List_Item dictionary:@{kObjectId:modelP.ids,kObjectTitle:modelP.title,kObjectUser:[AppSetting getUserDetauleId]}];
            } else {
    ...
    [self.viewSearch setOnSubscribeClick:^(ModelSubscribe *model) {
        if (model && model.ids && model.title) {
            [StatisticsManager event:kHomepage_Search_Subscription_List_Item dictionary:@{kObjectId:model.ids,kObjectTitle:model.title,kObjectUser:[AppSetting getUserDetauleId]}];
        } else {
    ...
    [self.viewSearch setOnSeriesCourseClick:^(ModelSubscribe *model) {
        if (model && model.ids && model.title) {
            [StatisticsManager event:kHomepage_Search_SeriesCourse_List_Item dictionary:@{kObjectId:model.ids,kObjectTitle:model.title,kObjectUser:[AppSetting getUserDetauleId]}];
        } else {
...
-(void)innerData
{
    ...
    if (arrLocalBanner && arrLocalBanner.count > 0) {
        ...
        NSArray *arrCurriculum = [sqlite getLocalHomeCurriculumArrayWithUserId:[AppSetting getUserDetauleId]];
        
        [self.tvMain setViewDataWithBannerArray:arrLocalBanner arrayPractice:arrPractice arrQuestion:arrQuestion arrSubscribe:arrSubscribe arrCurriculum:arrCurriculum];
    }
...
{
    ZWEAKSELF
    [snsV2 getHomeDataWithResultBlock:^(NSArray *arrBanner, NSArray *arrPractice, NSArray *arrQuestion, NSArray *arrSubscribe, NSArray *arrCurriculum, NSDictionary *result) {
        [weakSelf setDicResult:result];
        [weakSelf.tvMain endRefreshHeader];
        [weakSelf.tvMain setViewDataWithBannerArray:arrBanner arrayPractice:arrPractice arrQuestion:arrQuestion arrSubscribe:arrSubscribe arrCurriculum:arrCurriculum];
