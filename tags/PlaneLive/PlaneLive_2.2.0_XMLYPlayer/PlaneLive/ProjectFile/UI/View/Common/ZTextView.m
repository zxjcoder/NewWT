...
-(void)innerInit
{
    ...
    [_lbMaxCount setFont:[ZFont systemFontOfSize:kFont_Small_Size]];
...
-(void)setMaxCountText:(NSInteger)count
{
    if (_lbMaxCount && !_lbMaxCount.hidden) {
        if (_maxCount > 0 && (_maxCount-count) >= 0) {
...
-(void)textViewDidChange:(UITextView *)textView
{
    if (self.maxCount > 0) {
        ...
        NSString *lange = textView.textInputMode.primaryLanguage;
        if ([lange isEqualToString:@"zh-Hans"]) {
            UITextRange *selectedRange = [textView markedTextRange];
            if (selectedRange && toBeString.length > self.maxCount) {
                textView.text = [toBeString substringFromIndex:self.maxCount];
            }
        } else if (toBeString.length > self.maxCount) {
            textView.text = [toBeString substringFromIndex:self.maxCount];
        }
    }
