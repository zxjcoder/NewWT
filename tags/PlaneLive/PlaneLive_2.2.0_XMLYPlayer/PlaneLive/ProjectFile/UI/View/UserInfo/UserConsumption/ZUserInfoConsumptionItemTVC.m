...
-(CGFloat)setCellDataWithRechargeRecord:(ModelRechargeRecord *)model
{
    ...
    [self.lbPrice setText:[NSString stringWithFormat:@"¥%.f", [model.price floatValue]]];
...
-(CGFloat)setCellDataWithSubscribePlay:(ModelSubscribePlay *)model
{
    ...
    switch (model.type) {
        case 0:
            [self.lbTitle setText:[NSString stringWithFormat:@"[%@]%@", kSubscribe, model.title]];
            if (model && model.units.length > 0) {
                [self.lbPrice setText:[NSString stringWithFormat:@"¥%.f/%@", [model.price floatValue], model.units]];
            } else {
                [self.lbPrice setText:[NSString stringWithFormat:@"¥%.f", [model.price floatValue]]];
            }
            break;
        case 1:
            ...
            break;
        case 2:
            [self.lbTitle setText:[NSString stringWithFormat:@"[%@]%@", kPractice, model.title]];
            [self.lbPrice setText:[NSString stringWithFormat:@"¥%.f", [model.price floatValue]]];
            break;
        default:
            [self.lbTitle setText:model.title];
