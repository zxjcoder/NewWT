...
-(void)setPlayObject:(ModelTrack *)model isPlaying:(BOOL)isPlaying
{
    ...
    self.modelT = model;
    NSLog(@"trackId: %ld, newtrackId: %ld, isPlaying: %d", self.modelT.trackId, model.trackId, isPlaying);
    [self.tvMain reloadData];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.modelT && [[model.ids stringByAppendingString:kMultipleZeros] integerValue] == self.modelT.trackId) {
