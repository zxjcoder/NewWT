...
#define kAlertViewContentViewWidth 300.0f
#define kAlertViewContentVieHeight 100.0f
...
{
    [_msgLabel setTextAlignment:textAlignment];
}
...
-(void)innerInit
{
    ...
    if (!backgroundView) {
        ...
        [backgroundView setBackgroundColor:COLORVIEWBACKCOLOR3];
        [backgroundView setAlpha:kBackgroundOpacity];
    ...
    if (!contentView) {
        ...
        [contentView setViewRound:8 borderWidth:0 borderColor:CLEARCOLOR];
    ...
    if (!titleLabel) {
        ...
        titleLabel.font = [ZFont boldSystemFontOfSize:kZAlertViewTitleFontSize];
    ...
    if (!_msgLabel) {
        ...
        _msgLabel.font = [ZFont systemFontOfSize:kZAlertViewMessageFontSize];
    ...
    //横线
    if (!lineView) {
        lineView = [UIImageView getDLineView];
...
{
    ZAlertView *alertView = [[ZAlertView alloc] initWithTitle:title message:message buttonTitles:kDetermine, nil];
    [alertView setAlertViewStyle:(ZAlertViewStyleDefault)];
    [alertView showWithCompletion:nil];
}
