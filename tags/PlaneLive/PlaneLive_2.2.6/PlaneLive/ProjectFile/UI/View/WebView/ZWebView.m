...
#import "ZWebView.h"
#import "ZWebViewProgress.h"
#import <TargetConditionals.h>
#import <dlfcn.h>

@interface ZWebView()<UIWebViewDelegate,WKNavigationDelegate,WKUIDelegate,ZWebViewProgressDelegate,WKScriptMessageHandler>
...
@property (nonatomic, strong) ZWebViewProgress* njkWebViewProgress;
@end

@implementation ZWebView
...
- (instancetype)init
{
    return [self initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - APP_TOP_HEIGHT)];
}
- (instancetype)initWithFrame:(CGRect)frame
{
    return [self initWithFrame:frame isUIWebView:NO];
}
...
- (instancetype)initWithFrame:(CGRect)frame registerMonitorScript:(BOOL)registerMonitorScript
{
    ...
    if (self) {
        ...
        if (registerMonitorScript) {
            WKUserScript *script = [[WKUserScript alloc] initWithSource:[self getWKJS] injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
            [configuration.userContentController addUserScript:script];
            [configuration.userContentController addScriptMessageHandler:self name:@"get"];
        }
...
-(void)initWKWebView
{
    ...
    configuration.userContentController = userContentController;//[NSClassFromString(@"WKUserContentController") new];
...
-(void)initUIWebView
{
    ...
    self.njkWebViewProgress = [[ZWebViewProgress alloc] init];
...
{
    WKUserScript *script = [[WKUserScript alloc] initWithSource:[self getWKJS] injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
    [[(WKWebView *)_realWebView configuration].userContentController addUserScript:script];
}
-(NSString *)getWKJS
{
    NSBundle *bundle = [NSBundle mainBundle];
    NSString *filePath = [bundle pathForResource:@"ZWKJavascriptBridge.js" ofType:@"txt"];
    NSString *js = [NSString stringWithContentsOfFile:filePath encoding:NSUTF8StringEncoding error:nil];
    return js;
}
-(void)addScriptMessageHandler:(NSString *)name
{
    [[(WKWebView *)_realWebView configuration].userContentController addScriptMessageHandler:self name:name];
}
- (void)copyNSHTTPCookieStorageToWKHTTPCookieStoreWithCompletionHandler:(nullable void (^)())theCompletionHandler;
{
    NSArray *cookies = [[NSHTTPCookieStorage sharedHTTPCookieStorage] cookies];
    WKHTTPCookieStore *cookieStroe = [(WKWebView *)_realWebView configuration].websiteDataStore.httpCookieStore;
    if (cookies.count == 0) {
...
#pragma mark- CALLBACK ZVKWebView Delegate
...
-(void)dealloc
{
    ...
    [(UIWebView*)_realWebView loadHTMLString:kEmpty baseURL:nil];
    [_realWebView stopLoading];
...
{
    if (self.onDidReceiveScriptMessage) {
        self.onDidReceiveScriptMessage(message);
    }
}
