...
-(void)tapAnswerClick:(UIGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateEnded) {
        if (self.onAnswerClick) {
            ...
            [modelAB setTitle:self.model.aContent];
...
-(void)setViewFrame
{
    ...
    if (self.model.aid && self.model.aid.length > 0 && self.model.aContent.length > 0) {
...
-(CGFloat)setCellDataWithModel:(ModelMyNewQuestion *)model nickNameDescType:(int)nickNameDescType
{
    ...
    switch (nickNameDescType) {
        case 1:
            [self.viewMain setBackgroundColor:WHITECOLOR];
            [self.viewContent setBackgroundColor:WHITECOLOR];
            [self.lbNickName setText:[NSString stringWithFormat:@"%@%@",model.nickname, kAnswerHeQuestion]];
            break;
        case 2:
            [self.viewMain setBackgroundColor:WHITECOLOR];
            [self.viewContent setBackgroundColor:WHITECOLOR];
            [self.lbNickName setText:[NSString stringWithFormat:@"%@%@",model.nickname, kAnswerSheQuestion]];
            break;
        default:
            if (model.status == 0) {
            ...
            [self.lbNickName setText:[NSString stringWithFormat:@"%@%@",model.nickname, kAnswerYourQuestion]];
            break;
    }
...
-(CGFloat)setCellDataWithModel:(ModelMyNewQuestion *)model
{
    return [self setCellDataWithModel:model nickNameDescType:0];
}
