...
-(void)innerInit
{
    ...
    /// 数据库数据V2.2.4->V2.2.5迁移
    int downcount = [sqlite getLocalDownloadStatusAll];
    NSArray *arrayDownload = [[DownloadManager sharedManager] getDownloadedTracks];
    if (downcount == 0 || downcount < arrayDownload.count) {
        NSMutableArray *arrayResult = [NSMutableArray array];
        for (XMCacheTrack *model in arrayDownload) {
            if (model.announcer.id <= 0) {
        ...
        [sqlite setLocalDownloadWithTracks:arrayResult];
    }
