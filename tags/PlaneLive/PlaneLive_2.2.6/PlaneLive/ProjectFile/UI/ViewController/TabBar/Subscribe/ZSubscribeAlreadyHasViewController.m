...
@property (assign, nonatomic) BOOL isShowDetailInfo;
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.isShowDetailInfo) {
        [self.view sendSubviewToBack:self.viewPlay];
    } else {
...
-(void)innerInit
{
    ...
    [self.viewMain setOnDetailInfoShowChange:^(BOOL isShow) {
        if (isShow) {
            [weakSelf.view sendSubviewToBack:weakSelf.viewPlay];
        } else {
            [weakSelf.view bringSubviewToFront:weakSelf.viewPlay];
        }
    }];
...
{
    ZWEAKSELF
    [snsV2 getCurriculumArrayWithSubscribeId:self.subscribeId type:0 pageNum:[self.viewMain getPageNum]+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [snsV2 getCurriculumArrayWithSubscribeId:self.subscribeId type:1 pageNum:[self.viewMain getPageNum]+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
