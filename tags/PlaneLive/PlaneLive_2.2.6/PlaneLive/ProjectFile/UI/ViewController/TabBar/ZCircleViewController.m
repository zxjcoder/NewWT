...
-(void)innerDynamic
{
    if (![AppSetting getAutoLogin]) {
        ...
        NSArray *arrDynamic = [sqlite getLocalCircleDynamicQuestionArrayWithUserId:kLoginUserId];
...
-(void)innerEvent
{
    ...
    [self.tvNew setOnImagePhotoClick:^(ModelUserBase *model) {
        if (![model.userId isEqualToString:kLoginUserId]) {
...
-(void)setRefreshDynamicHeader
{
    ...
    [snsV1 getCircleTrendListWithUserId:kLoginUserId pageNum:self.pageNumDynamic resultBlock:^(NSArray *arrResult, NSDictionary *result) {
        [weakSelf.tvDynamic endRefreshHeader];
        [weakSelf.tvDynamic setViewDataWithArray:arrResult isHeader:YES];
        
        [sqlite setLocalCircleDynamicQuestionWithArray:arrResult userId:kLoginUserId];
    } errorBlock:^(NSString *msg) {
        [weakSelf.tvDynamic endRefreshHeader];
    }];
}
...
{
    ZWEAKSELF
    [snsV1 getCircleTrendListWithUserId:kLoginUserId pageNum:self.pageNumDynamic+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
-(void)setRefreshTopicHeader
{
    ZWEAKSELF
    self.pageNumTopic = 1;
    [snsV1 getCircleTopicListWithUserId:kLoginUserId pageNum:self.pageNumTopic resultBlock:^(NSArray *arrResult, NSDictionary *result) {
...
{
    ZWEAKSELF
    [snsV1 getCircleTopicListWithUserId:kLoginUserId pageNum:self.pageNumTopic+1 resultBlock:^(NSArray *arrResult, NSDictionary *result) {
