//
//  ZAccountUnBindViewController.m
//  PlaneLive
//
...
@interface ZAccountUnBindViewController()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ZAccountUnBindViewController
...
{
    [super viewDidLoad];
    [self setTitle:kMyMobilePhoneNumberUnBind];
    [self innerInit];
}
...
-(void)innerInit
{
    ...
    [self.tvcSpace1.viewMain setBackgroundColor:WHITECOLOR];
    ...
    [self.tvcPhone setCellDataWithModel:[AppSetting getUserLogin]];
    ...
    self.arrayMain = @[self.tvcSpace1,self.tvcPhone,self.tvcCode,self.tvcSpace2,self.tvcSave];
...
-(void)btnSaveClick
{
    ...
    [snsV2 postUnBindMobileWithMobile:phone valiCode:code resultBlock:^(NSDictionary *result) {
        ...
        [ZAlertPromptView showWithMessage:kUnBindingSuccess completionBlock:^{
...
-(void)btnSendCodeClick
{
    ...
    [snsV1 postGetCodeWithAccount:account flag:6 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            [ZProgressHUD dismiss];
