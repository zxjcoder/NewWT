...
{
    return [NSString stringWithFormat:@"keyLawFirmSeriesCourseDataKey%@", self.model.ids==nil?kEmpty:self.model.ids];
}
-(void)innerData
{
...
{
    ZWEAKSELF
    [snsV2 getLawFirmSeriesCourseListWithLawFirmId:self.model.ids pageNum:1 resultBlock:^(NSArray *result, NSDictionary *dic) {
        weakSelf.pageNum = 1;
...
{
    ZWEAKSELF
    [snsV2 getLawFirmSeriesCourseListWithLawFirmId:self.model.ids pageNum:self.pageNum resultBlock:^(NSArray *result, NSDictionary *dic) {
        weakSelf.pageNum += 1;
