...
-(void)innerInit
{
    ...
    [self.viewTool setOnItemClick:^(NSInteger index) {
        ...
        [weakSelf.scrollViewEnd setContentOffset:CGPointMake(_offsetIndex*self.scrollViewEnd.width, 0) animated:YES];
    }];
...
-(void)setSegmentedControlValueChanged:(UISegmentedControl *)sender
{
    ...
    [self.scrollViewAll setContentOffset:CGPointMake(sender.selectedSegmentIndex*self.scrollViewAll.width, 0) animated:YES];
}
///隐藏底部功能按钮
