...
-(void)setChangeSliderValue:(float)value
{
    if (!self.isAgreePlay) {
        NSString *filePath = [[[AppSetting getAudioFilePath] stringByAppendingPathComponent:[Utils stringMD5:self.playingModel.audioPath]] stringByAppendingPathExtension:kPathExtension];
        if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
            [self setChangeSliderValueStartPlay:value];
        } else {
            switch (self.networkStatus) {
                case ZNetworkReachabilityStatusUnknown:
                    [ZProgressHUD showError:kCMsgContentUnusual];
                    ...
                    [ZAlertView showWithTitle:nil message:kCMsgShowDownloadPlayPrompt completion:^(ZAlertView *alertView, NSInteger selectIndex) {
                        ...
                        switch (selectIndex) {
                            case 0:
                            {
                                ...
                                [weakSelf setChangeSliderValueStartPlay:value];
                                break;
                            }
                            default: break;
                        }
...
-(void)setChangeSliderValueStartPlay:(float)value
{
    GCDMainBlock(^{
        if (self.playerItem) {
...
- (void)setLockViewWithCurrentTime:(CGFloat)currentTime
{
    ...
    if(NSClassFromString(@"MPNowPlayingInfoCenter")) {
        [_dicMusicInfo removeAllObjects];
        // 设置Singer
        [_dicMusicInfo setObject:self.playingModel.audioAuthor forKey:MPMediaItemPropertyArtist];
        // 设置歌曲名
        [_dicMusicInfo setObject:self.playingModel.audioTitle forKey:MPMediaItemPropertyTitle];
        // 设置封面
        UIImage *img = nil;
        ...
        [_dicMusicInfo setObject:artwork forKey:MPMediaItemPropertyArtwork];
        //音乐剩余时长
        [_dicMusicInfo setObject:[NSNumber numberWithInteger:self.durationTime] forKey:MPMediaItemPropertyPlaybackDuration];
        //音乐当前播放时间
        [_dicMusicInfo setObject:[NSNumber numberWithDouble:currentTime] forKey:MPNowPlayingInfoPropertyElapsedPlaybackTime];
        GCDMainBlock(^{
            [[MPNowPlayingInfoCenter defaultCenter] setNowPlayingInfo:_dicMusicInfo];
        });
