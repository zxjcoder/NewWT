...
#import "ZActivityView.h"

#define BUTTON_VIEW_SIDE 60.f
#define BUTTON_VIEW_FONT_SIZE 13.f

#pragma mark - ZButtonView

@interface ZButtonView ()
...
- (void)setup
{
    ...
    self.textLabel.font = [ZFont systemFontOfSize:BUTTON_VIEW_FONT_SIZE];
    self.textLabel.textAlignment = NSTextAlignmentCenter;
    self.textLabel.textColor = RGBCOLOR(85, 85, 85);
    ...
    //imageView距离view左右各10, imageView的宽为40
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-10-[imageButton(40)]-10-|" options:0 metrics:nil views:views];
    [self addConstraints:constraints];
    
    //竖直方向imageView和textLabel在一条直线上, 并且挨着, imageView的高为40
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[imageButton(40)][textLabel]|" options:NSLayoutFormatAlignAllCenterX metrics:nil views:views];
    [self addConstraints:constraints];
...
#pragma mark - ZActivityView

@interface ZActivityView ()
...
@implementation ZActivityView

- (void)dealloc
{
    OBJC_RELEASE(_title);
    OBJC_RELEASE(_titleLabel);
    OBJC_RELEASE(_bgColor);
    OBJC_RELEASE(_cancelButton);
    OBJC_RELEASE(_contentView);
    OBJC_RELEASE(_contentViewAndViewConstraint);
    OBJC_RELEASE(_iconView);
    OBJC_RELEASE(_iconViewHeightConstraint);
    OBJC_RELEASE(_buttonArray);
    OBJC_RELEASE(_buttonConstraintsArray);
    OBJC_RELEASE(_referView);
...
- (void)setup
{
    ...
    self.bgColor = RGBCOLOR(85, 85, 85);
    ...
    self.titleLabel.font = [ZFont systemFontOfSize:17.f];
    self.titleLabel.text = self.title;
    [self.contentView addSubview:self.titleLabel];
    
    self.cancelButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [self.cancelButton setTitle:kCancelEmpty forState:UIControlStateNormal];
    [self.cancelButton setTitleColor:BLACKCOLOR forState:UIControlStateNormal];
    [self.cancelButton setBackgroundColor:WHITECOLOR];
...
- (void)updateConstraints
{
    ...
    constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-0-[titleLabel(==30)]-[iconView]-[cancelButton(==50)]-0-|" options:0 metrics:nil views:views];
    [self.contentView addConstraints:constraints];
...
- (void)prepareForShow
{
    ...
    for (int i = 0; i < [self.buttonArray count]; i++) {
        ZButtonView *buttonView = [self.buttonArray objectAtIndex:i];
...
- (void)hide
{
    ...
    [UIView animateWithDuration:0.25f animations:^{
        ws.alpha = 0;
        [ws removeConstraint:ws.contentViewAndViewConstraint];
        ws.contentViewAndViewConstraint = nil;
        [ws layoutIfNeeded];
