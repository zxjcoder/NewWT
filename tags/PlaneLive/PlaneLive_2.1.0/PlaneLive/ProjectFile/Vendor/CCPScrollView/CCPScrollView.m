...
- (void)setTitleArray:(NSArray *)titleArray {
    ...
    for (int i = 0; i < objArray.count; i++) {
        ...
        titleLabel.font = [ZFont systemFontOfSize:_titleFont];
...
- (void)clickTheLabel:(UITapGestureRecognizer *)tap {
    ...
    if (self.clickLabelBlock) {
        ...
        self.clickLabelBlock(tap.view.tag-100);
...
- (void)addTimer{
    ...
    self.timer = [NSTimer scheduledTimerWithTimeInterval:kANIMATION_BANNER_TIME target:self selector:@selector(nextLabel) userInfo:nil repeats:YES];
}
