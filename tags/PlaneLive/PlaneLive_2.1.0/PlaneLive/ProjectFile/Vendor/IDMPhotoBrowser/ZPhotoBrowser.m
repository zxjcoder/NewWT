...
#import "ZZoomingScrollView.h"
#import "POP.h"
#import "ZZoomingScrollView.h"
#import "ZSlider.h"
#import "SQLiteOper.h"

@interface ZPhotoBrowser ()
{
    // Data
...
- (ZZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index;
- (ZZoomingScrollView *)pageDisplayingPhoto:(id<IDMPhoto>)photo;
- (ZZoomingScrollView *)dequeueRecycledPage;
- (void)configurePage:(ZZoomingScrollView *)page forIndex:(NSUInteger)index;
...
- (void)panGestureRecognized:(id)sender {
    // Initial Setup
    ZZoomingScrollView *scrollView = [self pageDisplayedAtIndex:_currentPageIndex];
    ...
    if ([(UIPanGestureRecognizer*)sender state] == UIGestureRecognizerStateBegan) {
        if ([[(UIPanGestureRecognizer*)sender view] tag] != 988) {
...
- (void)performCloseAnimationWithScrollView:(ZZoomingScrollView*)scrollView {
...
- (void)viewWillLayoutSubviews {
    ...
    for (ZZoomingScrollView *page in _visiblePages) {
...
- (void)loadAdjacentPhotosIfNecessary:(id<IDMPhoto>)photo {
    ZZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
...
- (void)tilePages {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            // Add new page
            ZZoomingScrollView *page;
            page = [[ZZoomingScrollView alloc] initWithPrePhotoBrowser:self];
...
- (ZZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    ZZoomingScrollView *thePage = nil;
    for (ZZoomingScrollView *page in _visiblePages) {
...
- (void)configurePage:(ZZoomingScrollView *)page forIndex:(NSUInteger)index {
    ...
    __weak ZZoomingScrollView* weakPage = page;
...
- (ZZoomingScrollView *)dequeueRecycledPage {
    ZZoomingScrollView *page = [_recycledPages anyObject];
    if (page) {
...
- (void)setupScrollViewContentSize
{
    ...
    [self.viewTool setFrame:CGRectMake(0, viewH-45, viewW, 45)];
    [self.viewToolBG setFrame:self.viewTool.bounds];
    
    [self.slider setFrame:CGRectMake(0, 15, viewW, 15)];
}
...
//- (BOOL)areControlsHidden { return (_viewNav.alpha == 0); }
