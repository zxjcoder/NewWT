...
{
    // page items count > 3
    assert(array != nil && [array count] > 3);
    for (UIView *view in self.subviews) {
        [view removeFromSuperview];
    }
    _pageItemSpace = _pageItemWidth - _pageItemCoverWidth;
    _pageItemCount = [array count];
    for (NSInteger i = 0; i < _pageItemCount; i++) {
        ZFoundCardView *foundCardView = [[ZFoundCardView alloc] initWithFrame:CGRectMake(0, 0, _pageItemWidth, _pageItemHeight)];
        [foundCardView setViewDataWithModel:[array objectAtIndex:i]];
        foundCardView.center = CGPointMake(_width / 2.0, _height / 2.0);
        foundCardView.bounds = CGRectMake(0, 0, _pageItemWidth, _pageItemHeight);
        foundCardView.userInteractionEnabled = YES;
        foundCardView.layer.cornerRadius = _pageItemCornerRadius;
        foundCardView.layer.masksToBounds = YES;
        foundCardView.layer.shouldRasterize = YES;
        foundCardView.layer.rasterizationScale = [UIScreen mainScreen].scale;
        foundCardView.tag = i;
        ZWEAKSELF
        [foundCardView setOnPracticeTypeClick:^(ModelPracticeType *model) {
        ...
        longPressGesture.minimumPressDuration = 0.05;
        longPressGesture.cancelsTouchesInView = NO;
        [foundCardView addGestureRecognizer:longPressGesture];*/
        
        [self addSubview:foundCardView];
        
        CATransform3D transform = CATransform3DMakeTranslation(i * _pageItemSpace, 0, 0);
        foundCardView.layer.transform = transform;
    }
...
- (void)animationPageItemsWithDistance:(CGFloat)distance {
    for (NSInteger i = 0; i < _pageItemCount; i++) {
        ZFoundCardView *item = self.views[i];
