...
-(void)setViewDataWithModel:(ModelSubscribe *)model
{
    if (model) {
        ...
        NSRange range = NSMakeRange(btnText.length - kPlaneMoney.length, kPlaneMoney.length);
        if (model.units && model.units.length > 0) {
            btnText = [NSString stringWithFormat:@"%@ %.2f%@/%@", kPayCartMoney, [model.price floatValue], kPlaneMoney, model.units];
            range = NSMakeRange(btnText.length - kPlaneMoney.length - model.units.length - 1, kPlaneMoney.length);
        }
        [self.btnSubscribe setTitle:btnText forState:UIControlStateNormal];
        [[self.btnSubscribe titleLabel] setFont:[ZFont systemFontOfSize:kFont_Middle_Size]];
        [self.btnSubscribe.titleLabel setLabelFontWithRange:range font:[ZFont systemFontOfSize:kFont_Minimum_Size]];
    } else {
        [self setTextDefaultValue];
    }
}
...
-(void)setTextDefaultValue {
    NSString *btnText = [NSString stringWithFormat:@"%@ 0.00%@", kPayCartMoney, kPlaneMoney];
    [self.btnSubscribe setTitle:btnText forState:UIControlStateNormal];
    [[self.btnSubscribe titleLabel] setFont:[ZFont systemFontOfSize:kFont_Middle_Size]];
    [self.btnSubscribe.titleLabel setLabelFontWithRange:(NSMakeRange(btnText.length - kPlaneMoney.length, kPlaneMoney.length)) font:[ZFont systemFontOfSize:kFont_Minimum_Size]];
}
///获取高度
