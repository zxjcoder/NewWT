...
-(void)innerData
{
    ...
    ZWEAKSELF
    [snsV1 getCircleQueryArticleWithArtName:kEmpty pageNum:1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnRightClick
{
    ...
    [snsV1 postCirclePublishQuestionWithUserId:kLoginUserId question:title quizDetail:content articles:[selTagId toString] resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnAddTagClick
{
    ...
    [self.alertEditView setOnSubmitClick:^(NSString *content) {
        ...
        if (tagName.length == 0) {
            ...
            NSString *errorContent = [NSString stringWithFormat:kTopicLengthLimitCharacter, kNumberTopicMaxLength];
            GCDMainBlock(^{
