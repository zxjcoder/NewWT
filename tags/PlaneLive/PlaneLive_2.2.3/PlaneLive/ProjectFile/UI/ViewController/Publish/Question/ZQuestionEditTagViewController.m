...
@interface ZQuestionEditTagViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    NSString *_lastText;
}
///顶部区域
...
-(void)innerInit
{
    ...
    [self.textField setTextFieldIndex:ZTextFieldIndexEditQuestionTag];
...
//    [self.lbAddTag setFont:[ZFont boldSystemFontOfSize:kFont_Middle_Size]];
...
-(void)textFieldDidChange:(NSNotification *)sender
{
    ...
    ZWEAKSELF
    [snsV1 getCircleQueryArticleWithArtName:content pageNum:1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)innerData
{
    ...
    ZWEAKSELF
    [snsV1 getCircleQueryArticleWithArtName:weakSelf.modelQD.title pageNum:1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnAddTagClick
{
    ...
    [self.alertEditView setOnSubmitClick:^(NSString *content) {
        GCDMainBlock(^{
            ...
            [snsV1 getCircleSaveArticleWithUserId:userId artName:content resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    ...
                    if (dicTag) {
                        ...
                        [ZProgressHUD showSuccess:kTopicAddSuccess];
                    } else {
