...
-(void)innerInit
{
    ...
    [self.tvMain setOnToolItemClick:^(NSInteger index) {
        switch (index) {
            case 1:
            {
                [StatisticsManager event:kHomepage_Practice];
                ZPracticeTypeViewController *itemVC = [[ZPracticeTypeViewController alloc] init];
                [itemVC setHidesBottomBarWhenPushed:YES];
                [weakSelf.navigationController pushViewController:itemVC animated:YES];
                break;
            ...
            case 2:
            {
                [StatisticsManager event:kHomepage_PopularTraining];
            ...
            case 3:
            {
                [StatisticsManager event:kHomepage_SeriesCourse];
    ...
    [self.tvMain setOnLawFirmClick:^(ModelLawFirm *model) {
        [StatisticsManager event:kHomepage_ProfessionalOrganizations_ListItem dictionary:@{kObjectId:model.ids==nil?kEmpty:model.ids,kObjectTitle:model.title==nil?kEmpty:model.title,kObjectUser:kLoginUserId}];
    ...
    [self.tvMain setOnQuestionClick:^(ModelQuestionBoutique *model) {
        [StatisticsManager event:kHomepageSeries_BoutiqueQusetion_ListItem dictionary:@{kObjectId:model.ids==nil?kEmpty:model.ids,kObjectTitle:model.title==nil?kEmpty:model.title,kObjectUser:kLoginUserId}];
    ...
    [self.tvMain setOnAllQuestionClick:^{
        [StatisticsManager event:kHomepage_BoutiqueQusetionAll];
...
-(void)setSearchView
{
    ...
    [self.viewSearch setOnPracticeClick:^(NSArray *array, NSInteger row) {
        if (array.count > 0 && row < array.count) {
            ...
            if (modelP && modelP.ids && modelP.title) {
                [StatisticsManager event:kHomepage_Search_ListItem dictionary:@{kObjectId:modelP.ids,kObjectTitle:modelP.title,kObjectUser:kLoginUserId}];
            } else {
                [StatisticsManager event:kHomepage_Search_ListItem];
            }
        }
        [weakSelf.searchBar resignFirstResponder];
        [weakSelf showPlayVCWithPracticeArray:array index:row];
    ...
    [self.viewSearch setOnSubscribeClick:^(ModelSubscribe *model) {
        if (model && model.ids && model.title) {
            [StatisticsManager event:kHomepage_Search_Training_ListItem dictionary:@{kObjectId:model.ids,kObjectTitle:model.title,kObjectUser:kLoginUserId}];
        } else {
            [StatisticsManager event:kHomepage_Search_Training_ListItem];
        }
        [weakSelf showSubscribeDetailVCWithModel:model];
    }];
    ...
    [self.viewSearch setOnSeriesCourseClick:^(ModelSubscribe *model) {
        if (model && model.ids && model.title) {
            [StatisticsManager event:kHomepage_Search_EriesCourse_ListItem dictionary:@{kObjectId:model.ids,kObjectTitle:model.title,kObjectUser:kLoginUserId}];
        } else {
            [StatisticsManager event:kHomepage_Search_EriesCourse_ListItem];
        }
        [weakSelf showSubscribeDetailVCWithModel:model];
    }];
...
-(void)innerData
{
    NSDictionary *dicHome = [sqlite getLocalHomeAllDataWithUserId:kLoginUserId];
    if (dicHome && dicHome.count > 0) {
        NSArray *arrBanner = [dicHome objectForKey:@"Banner"];
        NSArray *arrPractice = [dicHome objectForKey:@"Practice"];
        NSArray *arrLawFirm = [dicHome objectForKey:@"LawFirm"];
        NSArray *arrQuestion = [dicHome objectForKey:@"Question"];
        NSArray *arrSubscribe = [dicHome objectForKey:@"Subscribe"];
        NSArray *arrCurriculum = [dicHome objectForKey:@"Curriculum"];
        if (arrBanner && arrBanner.count > 0) {
            [self.tvMain setViewDataWithBannerArray:arrBanner arrayPractice:arrPractice arrLawFirm:arrLawFirm arrQuestion:arrQuestion arrSubscribe:arrSubscribe arrCurriculum:arrCurriculum];
        }
    }
...
-(void)setSearchContentWithText:(NSString *)text
{
    if (![text isEqualToString:self.lastSearchText]) {
        if (text.length > 0) {
            NSDictionary *dicParams = [NSDictionary dictionaryWithObjectsAndKeys:kZhugeIOHomeKeywordsKey, text, nil];
            /// TODO: ZWW - 诸葛IO统计
            [StatisticsManager eventIOTrackWithKey:kZhugeIOHomeSearchPracticeKey dictionary:dicParams];
            [StatisticsManager eventIOTrackWithKey:kZhugeIOHomeSearchTrainCourseKey dictionary:dicParams];
            [StatisticsManager eventIOTrackWithKey:kZhugeIOHomeSearchSeriesCourseKey dictionary:dicParams];
        }
...
-(BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar
{
    ...
    [StatisticsManager event:kHomepage_Search];
