...
@property (strong, nonatomic) ZSettingItemTVC *tvcBugout;
...
-(void)innerInit
{
    ...
    self.tvcBugout = [[ZSettingItemTVC alloc] initWithReuseIdentifier:@"tvcBugout" cellType:(ZSettingItemTVCTypeBugout)];
    [self.tvcBugout setHiddenLine];
    ...
    if ([modelU.account isMobile] || [modelU.account isEmail]) {
        //手机或邮箱登录
        self.arrMain = @[self.tvcPassword,self.tvcManager,self.tvcClearCache,self.tvcFontSize,self.tvcNotice,self.tvcBugout,self.tvcSpace1
                         ,self.tvcScore,self.tvcAbout,self.tvcSpace2
                         ,self.tvcExit];
    } else {
        //微信或QQ登录
        self.arrMain = @[self.tvcManager,self.tvcClearCache,self.tvcFontSize,self.tvcNotice,self.tvcBugout,self.tvcSpace1
                         ,self.tvcScore,self.tvcAbout,self.tvcSpace2
                         ,self.tvcExit];
    }
    ...
    [self.tvcBugout setBugoutOn:[AppSetting getBugoutOn]];
    ZWEAKSELF
    [self.tvcBugout setOnBugoutValueChange:^(BOOL isOn) {
        
        [StatisticsManager event:kUser_Setting_BUGOUT];
        
        [[AppDelegate app] setBugoutOn:isOn];
        [AppSetting setBugoutOn:isOn];
        [AppSetting save];
    }];
