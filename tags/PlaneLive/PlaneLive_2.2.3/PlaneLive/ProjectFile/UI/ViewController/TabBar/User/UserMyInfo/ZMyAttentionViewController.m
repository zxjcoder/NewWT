...
-(void)innerInit
{
    ...
    [self.viewMain setOnRefreshQuestionFooter:^{
        [weakSelf setRefreshQuestionFooter];
    }];
    ///头像点击
...
{
    [snsV1 postDeleteAttentionWithAId:model.ids userId:kLoginUserId type:@"0" resultBlock:nil errorBlock:nil];
}
-(void)setDeleteTopicClick:(ModelTag *)model
{
    [snsV1 postDeleteAttentionWithAId:model.tagId userId:kLoginUserId type:@"3" resultBlock:nil errorBlock:nil];
}
-(void)setDeleteUserClick:(ModelUserBase *)model
{
    [snsV1 postDeleteAttentionWithAId:model.userId userId:kLoginUserId type:@"1" resultBlock:nil errorBlock:nil];
}
...
-(void)setRefreshQuestionHeader
{
    ZWEAKSELF
    self.pageNumQuestion = 1;
    [snsV1 getCircleAttentionListWithUserId:kLoginUserId pageNum:self.pageNumQuestion resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
