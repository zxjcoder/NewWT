...
-(void)innerInit
{
    ...
    [self.tvMain setOnImageClick:^(UIImage *image, NSURL *imageUrl, NSInteger currentIndex, NSArray *arrImageUrl, CGSize size) {
        ...
        [weakSelf showPhotoBrowserWithArray:arrImageUrl index:defaultIndex];
    }];
    ///开始移动的时候
...
{
    self.isLoaded = YES;
    ModelAnswerBase *modelAnswer = [sqlite getLocalAnswerDetailWithAnswerId:self.modelAB.ids userId:kLoginUserId];
...
-(void)setRefreshAnswerData
{
    ZWEAKSELF
    self.pageNum = 1;
    [snsV1 getAnswerDetailWithAnswerId:self.modelAB.ids userId:kLoginUserId commentId:self.modelAC.ids pageNum:self.pageNum resultBlock:^(NSArray *arrComment, ModelAnswerBase *modelAnswer, ModelAnswerComment *modelDefaultComment, NSDictionary *dicResult) {
        GCDMainBlock(^{
            ...
            if (modelAnswer) {
                ...
                [sqlite setLocalAnswerDetailWithModel:modelAnswer commentArray:arrComment userId:kLoginUserId];
            } else {
...
{
    ZWEAKSELF
    [snsV1 getAnswerDetailWithAnswerId:self.modelAB.ids userId:kLoginUserId commentId:self.modelAC.ids pageNum:self.pageNum+1 resultBlock:^(NSArray *arrComment, ModelAnswerBase *modelAnswer, ModelAnswerComment *modelDefaultComment, NSDictionary *dicResult) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
...
{
    [StatisticsManager event:kAnswer_Comment];
    if ([AppSetting getAutoLogin]) {
        ...
        if (self.commentType == 0) {
            ...
            [snsV1 postSaveCommentWithUserId:kLoginUserId content:content objId:self.modelAB.ids type:@"0" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            if (self.commentType == 1) {
                ///回复对答案评论的内容
                [snsV1 postCommentReplyWithUserId:kLoginUserId content:content replyUserId:self.modelLastAC.userId commentId:self.modelLastAC.ids parent_id:kZero resultBlock:^(NSDictionary *result) {
                    GCDMainBlock(^{
                ...
                ///回复对评论回复的内容
                [snsV1 postCommentReplyWithUserId:kLoginUserId content:content replyUserId:self.modelLastCR.user_id commentId:self.modelLastCR.commentId parent_id:self.modelLastCR.ids resultBlock:^(NSDictionary *result) {
                    GCDMainBlock(^{
                        [ZProgressHUD showSuccess:kCMsgReplySuccess];
...
-(void)btnAgreeClick
{
    [StatisticsManager event:kAnswer_Praise];
    if ([AppSetting getAutoLogin]) {
        //还在加载数据
        if (self.isLoaded) {
            return;
        ...
        if (self.modelAB.isAgree == 0) {
            [snsV1 postClickLikeWithAId:self.modelAB.ids userId:kLoginUserId type:@"2" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    ...
                    [sqlite updLocalAnswerDetailWithModel:weakSelf.modelAB userId:kLoginUserId];
                });
            } errorBlock:^(NSString *msg) {
                GCDMainBlock(^{
                    [weakSelf setIsAgreeing:NO];
            ...
            [snsV1 postClickUnLikeWithAId:self.modelAB.ids userId:kLoginUserId type:@"2" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    [weakSelf setIsAgreeing:NO];
    ...
    [shareView setOnItemClick:^(ZShareType shareType) {
        switch (shareType) {
            case ZShareTypeWeChat:
                [StatisticsManager event:kAnswer_Share_WeChat];
                [weakSelf btnWeChatClick];
                break;
            case ZShareTypeWeChatCircle:
                [StatisticsManager event:kAnswer_Share_WechatCircle];
                [weakSelf btnWeChatCircleClick];
                break;
            case ZShareTypeQQ:
                [StatisticsManager event:kAnswer_Share_QQ];
                [weakSelf btnQQClick];
                break;
            case ZShareTypeQZone:
                [StatisticsManager event:kAnswer_Share_QZone];
            ...
            case ZShareTypeDeleteAnswer:
                [StatisticsManager event:kAnswer_Delete];
...
-(void)btnCollectionClick
{
    if ([AppSetting getAutoLogin]) {
        ...
        [snsV1 getAddCollectionWithUserId:kLoginUserId cid:self.modelAB.ids flag:4 type:@"6" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
...
{
    NSString *title = self.modelAB.question_title;
    ...
    [ShareManager shareWithTitle:title content:content imgObj:image webUrl:webUrl platformType:type resultBlock:^(bool isSuccess, NSString *msg) {
        if (!isSuccess && msg) {
            ...
            if ([AppSetting getAutoLogin]) {
                [snsV1 postAddShareInfoWithShareId:weakSelf.modelAB.ids userId:kLoginUserId resultBlock:nil errorBlock:nil];
            }
        }
    }];
}
...
-(void)btnDeleteClick
{
    ...
    [ZAlertView showWithMessage:kCMsgDoneDeleteAnswer doneCompletion:^{
        ...
        [snsV1 postDeleteAnswerWithAnswerId:weakSelf.modelAB.ids userId:kLoginUserId resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
            ...
            switch (buttonIndex) {
                case 0://删除
                {
                    if (!self.isDeleteing) {
                        ...
                        [snsV1 postCommentReplyWithUserId:kLoginUserId ids:self.modelLastAC.ids type:0 resultBlock:^(NSDictionary *result) {
                            GCDMainBlock(^{
                                [weakSelf setIsDeleteing:NO];
            ...
            switch (buttonIndex) {
                case 0://删除
                {
                    if (!self.isDeleteing) {
                        ZWEAKSELF
                        self.isDeleteing = YES;
                        [snsV1 postCommentReplyWithUserId:kLoginUserId ids:self.modelLastCR.ids type:1 resultBlock:^(NSDictionary *result) {
                            GCDMainBlock(^{
                                [weakSelf setIsDeleteing:NO];
