...
-(void)innerInit
{
    self.scrollView = [[ZScrollView alloc] initWithFrame:CGRectMake(0, APP_TOP_HEIGHT, APP_FRAME_WIDTH, APP_FRAME_HEIGHT-APP_TOP_HEIGHT-APP_TABBAR_HEIGHT)];
    ...
    [self.tvItem1 setOnSubscribeClick:^(ModelCurriculum *model, int unReadCount) {
        [StatisticsManager event:kSubscription_List_Item dictionary:@{kObjectId:model.ids==nil?kEmpty:model.ids,kObjectTitle:model.title==nil?kEmpty:model.title}];
    ...
    [self.tvItem2 setOnBackgroundClick:^(ZBackgroundState state) {
        [weakSelf.tvItem2 setBackgroundViewWithState:(ZBackgroundStateLoading)];
...
-(void)innerData
{
    NSArray *arrayAlready = [sqlite getLocalSubscribeAlreadyArrayWithUserId:kLoginUserId];
...
-(void)setViewContentOffsetWithIndex:(NSInteger)index
{
    switch (index) {
        case 1:
        {
            [self.tvItem1 setScrollsToTop:NO];
            [self.tvItem2 setScrollsToTop:YES];
            [StatisticsManager event:kSubscription_Recommend];
            break;
        }
        default:
        {
            ...
            [StatisticsManager event:kSubscription_OnOrder];
            break;
        }
    }
...
-(void)setRefreshAlreadyHeader
{
    if ([AppSetting getAutoLogin]) {
        ...
        [snsV2 getSubscribeAlreadyWithPageNum:self.pageNumAlready resultBlock:^(NSArray *arrResult, NSDictionary *result) {
            ...
            [sqlite setLocalSubscribeAlreadyWithArray:arrResult userId:kLoginUserId];
        } errorBlock:^(NSString *msg) {
            [weakSelf.tvItem1 endRefreshHeader];
