...
-(void)innerInit
{
    ...
    [self.btnDownload setTag:ZDownloadStatusJoin];
...
{
    if (model.status == XMCacheTrackStatusDownloading) {
        [self.lbDownloadDesc setText:kClickToSuspendDownload];
    } else {
        [self.lbDownloadDesc setText:kHasBeenSuspendedClickToContinueToDownload];
    }
    
    [self setDownloadButtonImage:model.status];
}
///下载状态改变
-(void)setChangeDownloadStatusWithStatus:(XMCacheTrackStatus)status
{
    if (status == XMCacheTrackStatusDownloading) {
        [self.lbDownloadDesc setText:kClickToSuspendDownload];
    } else {
        [self.lbDownloadDesc setText:kHasBeenSuspendedClickToContinueToDownload];
    }
    [self setDownloadButtonImage:status];
}
-(void)setCheckShow:(BOOL)showCheck
{
    [self setCheckShow:showCheck animate:NO];
}
-(void)setCheckShow:(BOOL)showCheck animate:(BOOL)animate
{
...
-(void)setDownloadProgress:(double)progress
{
    ...
    NSString *percentageText = [NSString stringWithFormat:@"%.f%%", progress*100];
    if (progress == 0) {
        [self.lbPercentage setTextColor:DESCCOLOR];
        [self.lbPercentage setText:kDownloadWaiting];
        [self setStopDownload];
        [self.btnDownload setDownloadImage:@"icon_download_start_gray"];
    } else {
...
{
    [self.imgLogo setImageURLStr:model.coverUrlSmall placeImage:[SkinManager getPracticeImage]];
    
    [self.lbTitle setText:model.trackTitle];
    
    [self.lbLecturer setText:[NSString stringWithFormat:@"%@: %@", kSpeaker, model.announcer.nickname]];
    ...
    [self setDownloadButtonImage:model.status];
    [self setChangeDownloadStatusWithModel:model];
    
    if (model.downloadedBytes > 0) {
        [self setDownloadProgress:model.downloadSize/model.downloadedBytes];
    } else {
