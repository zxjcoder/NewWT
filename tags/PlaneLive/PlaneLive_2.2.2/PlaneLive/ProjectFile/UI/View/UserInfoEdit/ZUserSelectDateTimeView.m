...
- (void)dateTimeChange:(UIDatePicker *)picker
{
    if (self.onSelectChange) {
        self.onSelectChange([picker.date toStringWithFormat:kFormat_Date_Date]);
    }
}
-(void)btnCloseClick
{
    if (self.onSelectChange) {
        self.onSelectChange([self.pickerView.date toStringWithFormat:kFormat_Date_Date]);
    }
...
-(void)setDefaultSelectValue:(NSString *)selValue
{
    if (selValue != nil && ![selValue isEmpty]) {
        NSDate *date = [selValue toDateWithFormat:kFormat_Date_Date];
        if (date != nil) {
