...
-(void)tapAnswerClick:(UIGestureRecognizer *)sender
{
    if (sender.state == UIGestureRecognizerStateEnded) {
        if (self.onAnswerClick) {
            ...
            [modelAB setUserId:self.model.userIdA];
            [modelAB setNickname:self.model.nicknameA];
            [modelAB setHead_img:self.model.head_imgA];
    ...
    if (sender.state == UIGestureRecognizerStateEnded) {
        if (self.onImagePhotoClick) {
            ...
            if (self.model.aid && self.model.aid.length > 0 && self.model.content.length > 0) {
                [modelUB setUserId:self.model.userIdA];
                [modelUB setHead_img:self.model.head_imgA];
                [modelUB setNickname:self.model.nicknameA];
                [modelUB setSign:self.model.signA];
            } else {
...
-(CGFloat)setCellDataWithModel:(ModelCircleNew *)model
{
    [self setModel:model];
    ...
    if (self.model.aid && self.model.aid.length > 0 && self.model.content.length > 0) {
        [self.imgPhoto setPhotoURLStr:model.head_imgA];
        
        [self.lbNickName setText:[NSString stringWithFormat:@"%@%@", model.nicknameA, kAnswerTheQuestion]];
        [self.lbNickName setTextColor:DESCCOLOR];
        if (model.nicknameA.length > 0) {
            [self.lbNickName setLabelColorWithRange:NSMakeRange(0, model.nicknameA.length) color:MAINCOLOR];
        }
    } else {
        [self.imgPhoto setPhotoURLStr:model.head_imgQ];
        
        
        [self.lbNickName setText:[NSString stringWithFormat:@"%@%@", model.nicknameQ, kPutForwardTheQuestion]];
        [self.lbNickName setTextColor:DESCCOLOR];
        if (model.nicknameQ.length > 0) {
            [self.lbNickName setLabelColorWithRange:NSMakeRange(0, model.nicknameQ.length) color:MAINCOLOR];
        }
    }
