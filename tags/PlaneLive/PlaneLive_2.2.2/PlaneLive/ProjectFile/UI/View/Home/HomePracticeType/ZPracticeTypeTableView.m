...
-(void)innerInit
{
    ...
    [self.tvcItem setOnSortClick:^(ZPracticeTypeSort sort) {
...
-(void)setShowSortView:(int)sort
{
    if (self.viewSort) {
        ...
        ZPracticeTypeSortView *viewSort = [[ZPracticeTypeSortView alloc] initWithFrame:CGRectMake(sortFrame.size.width-105, sortFrame.origin.y+sortFrame.size.height, 100, 120)];
        [self addSubview:viewSort];
        [self bringSubviewToFront:viewSort];
        [viewSort setShowSortViewWithAnimate];
        ZWEAKSELF
        [viewSort setOnSortClick:^(ZPracticeTypeSort sort) {
            [weakSelf.tvcItem setSortButtonTag:sort];
            switch (sort) {
                case ZPracticeTypeSortHot:
                {
                    [weakSelf.tvcItem setSortButtonText:kSortByHot];
                    break;
                }
                case ZPracticeTypeSortNew:
                {
                    [weakSelf.tvcItem setSortButtonText:kSortByNew];
                    
                    break;
                }
                case ZPracticeTypeSortRecommendation:
                {
                    [weakSelf.tvcItem setSortButtonText:kSortByRecommendation];
                    break;
                }
            }
...
{
    return self.tvcItem.getSortValue;
}
