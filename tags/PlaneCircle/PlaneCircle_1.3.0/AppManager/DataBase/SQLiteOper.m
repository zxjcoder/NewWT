...
-(BOOL)setLocalPracticeDetailWithModel:(ModelPractice *)model  userId:(NSString *)userId
{
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        ...
        ///添加语音
        result = [db executeUpdate:@"REPLACE INTO sns_practice_detail(ids,userId,title,time,play_time,speech_img,bkimg,speech_url,share_content,nickname,create_time,speech_content,ccount,mcount,applauds,isCollection,isPraise,rowIndex,dataType,showText,sortby) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,0,?,strftime('%s%f','now'));" withArgumentsInArray:@[ids,userId,title,time,play_time,speech_img,bkimg,speech_url,share_content,nickname,create_time,speech_content,ccount,mcount,applauds,isCollection,isPraise,rowIndex,showText]];
    }];
...
///获取未上传的崩溃日志
-(NSArray *)getLocalCatchCrashAll
{
    __block NSMutableArray *result = nil;
    
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:@"SELECT t1.pkid,t1.name,t1.reason,t1.createtime, \
                           GROUP_CONCAT(t2.content,' \r\n ') AS content \
                           FROM sns_catchcrash t1 \
                           LEFT JOIN sns_catchcrash_stack t2 \
                           ON t1.pkid=t2.ccid \
                           GROUP BY t1.pkid \
                           ORDER BY t1.createtime,t1.pkid,t2.pkid ASC"];
        if (rs != nil)
        {
            result = [NSMutableArray array];
...
///添加未上传的崩溃日志
-(BOOL)setLocalCatchCrashWithDictionary:(NSDictionary *)dic
{
    BOOL __block result = NO;
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        NSArray *stackArray = [dic objectForKey:@"stackArray"];
        NSString *pkid = [[NSString stringWithFormat:@"%f", [NSDate timeIntervalSinceReferenceDate]] stringByReplacingOccurrencesOfString:@"." withString:@""];
        
        result = [db executeUpdate:@"REPLACE INTO sns_catchcrash(pkid,name,reason,createtime) VALUES(?,?,?,datetime('now', 'localtime'));" withArgumentsInArray:@[pkid,name,reason]];
        NSInteger row = 0;
        for (NSString *stackStr in stackArray) {
            result = [db executeUpdate:@"REPLACE INTO sns_catchcrash_stack(pkid,ccid,content) VALUES(?,?,?);" withArgumentsInArray:@[[NSNumber numberWithInteger:row],pkid,stackStr]];
            row++;
        }
...
///删除已上传的崩溃日志
-(BOOL)delLocalCatchCrashWithPkid:(NSInteger)pkid
{
    BOOL __block result = NO;
    
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        
        result = [db executeUpdate:[NSString stringWithFormat:@"DELETE FROM sns_catchcrash WHERE pkid = '%ld'", (long)pkid]];
        result = [db executeUpdate:[NSString stringWithFormat:@"DELETE FROM sns_catchcrash_stack WHERE catchcrashid = '%ld'", (long)pkid]];
...
-(BOOL)delLocalCatchCrashWithAll
{
    ...
    [dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        result = [db executeUpdate:@"DELETE FROM sns_catchcrash_stack"];
