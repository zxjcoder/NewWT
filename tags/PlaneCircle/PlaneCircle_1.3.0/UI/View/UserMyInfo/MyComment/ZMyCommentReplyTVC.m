...
#define kMyCommentReplyNickNameDesc @"回复了你的答案"
#define kMyCommentAnswerNickNameDesc @"评论了你的答案"
...
#define kZMyAnswerCommentNickNameDesc @"评论了你的答案"
#define kZMyCommentReplyNickNameDesc @"回复了你"
...
-(void)setCellDataWithModel:(ModelQuestionMyAnswerComment *)model
{
    ...
    if (model) {
        ...
        if (model.nickname.length > 0) {
            if (model.type == 0) {
                [self.lbNickName setText:[NSString stringWithFormat:@"%@%@",model.nickname,kZMyAnswerCommentNickNameDesc]];
                [self.lbNickName setTextColor:RGBCOLOR(251, 107, 33)];
                [self.lbNickName setLabelColorWithRange:NSMakeRange(model.nickname.length, kZMyAnswerCommentNickNameDesc.length) color:BLACKCOLOR1];
            } else {
                [self.lbNickName setText:[NSString stringWithFormat:@"%@%@",model.nickname,kZMyCommentReplyNickNameDesc]];
                [self.lbNickName setTextColor:RGBCOLOR(251, 107, 33)];
                [self.lbNickName setLabelColorWithRange:NSMakeRange(model.nickname.length, kZMyCommentReplyNickNameDesc.length) color:BLACKCOLOR1];
            }
        } else {
            if (model.type == 0) {
                [self.lbNickName setText:kZMyAnswerCommentNickNameDesc];
                [self.lbNickName setTextColor:BLACKCOLOR1];
            } else {
                [self.lbNickName setText:kZMyCommentReplyNickNameDesc];
                [self.lbNickName setTextColor:BLACKCOLOR1];
            }
        }
