...
-(void)innerInit
{
    ...
    [self.tvMain setOnReplyClick:^(ModelCommentReply *model, NSInteger row) {
        ...
        if ([Utils isMyUserId:model.user_id]) {
            ...
            ZActionSheet *actionSheet = [[ZActionSheet alloc] initWithTitle:nil delegate:weakSelf cancelButtonTitle:kCancel destructiveButtonTitle:nil otherButtonTitles:@[@"删除"]];
            ...
            [weakSelf.viewComment setPlaceholderText:[NSString stringWithFormat:@"回复%@:", model.hnickname]];
    ...
    [self.tvMain setOnCommentClick:^(ModelAnswerComment *model, NSInteger row) {
        ...
        if ([Utils isMyUserId:model.userId]) {
            ...
            ZActionSheet *actionSheet = [[ZActionSheet alloc] initWithTitle:nil delegate:weakSelf cancelButtonTitle:kCancel destructiveButtonTitle:nil otherButtonTitles:@[@"回复评论",@"举报评论"]];
    ...
    [DataOper130 getAnswerDetailWithAnswerId:self.modelAB.ids userId:[AppSetting getUserDetauleId] commentId:self.modelAC.ids pageNum:self.pageNum resultBlock:^(NSArray *arrComment, ModelAnswerBase *modelAnswer, ModelAnswerComment *modelDefaultComment, NSDictionary *dicResult) {
        GCDMainBlock(^{
            ...
            if (modelAnswer) {
                ...
                [ZProgressHUD showError:@"获取答案详情失败"];
            }
        });
...
-(void)btnPublishClick:(NSString *)content
{
    if ([AppSetting getAutoLogin]) {
        ...
        if (self.commentType == 0) {
            ...
            if (content.length == 0 || content.length > kNumberAnswerCommentMaxLength) {
                [ZProgressHUD showError:[NSString stringWithFormat:@"回复内容限制[1-%d]字符",kNumberAnswerCommentMaxLength]];
                self.isPublishing = NO;
                return;
            }
            if (self.commentType == 1) {
                ///回复对答案评论的内容
                [DataOper130 postCommentReplyWithUserId:[AppSetting getUserDetauleId] content:content replyUserId:self.modelLastAC.userId commentId:self.modelLastAC.ids parent_id:@"0" resultBlock:^(NSDictionary *result) {
                    GCDMainBlock(^{
                        [ZProgressHUD showSuccess:@"回复成功"];
...
-(void)shareWithType:(WTPlatformType)type
{
    ...
    [ShareManager shareWithTitle:title content:content imgObj:image webUrl:webUrl platformType:type resultBlock:^(bool isSuccess, NSString *msg) {
        if (!isSuccess && msg) {
            ...
            if ([AppSetting getAutoLogin]) {
                [sns postAddShareInfoWithShareId:weakSelf.modelAB.ids userId:[AppSetting getUserId] resultBlock:nil errorBlock:nil];
            }
        }
    }];
}
        ...
        case 10://别人的评论
        {
            switch (buttonIndex) {
                case 0://回复
                {
                    self.commentType = 1;
                    self.isComment = NO;
                    [self.viewComment setPlaceholderText:[NSString stringWithFormat:@"回复%@:", self.modelLastAC.nickname]];
            ...
            switch (buttonIndex) {
                case 0://删除
                {
                    if (!self.isDeleteing) {
                        ...
                        [DataOper130 postCommentReplyWithUserId:[AppSetting getUserId] ids:self.modelLastAC.ids type:0 resultBlock:^(NSDictionary *result) {
                            GCDMainBlock(^{
                                [weakSelf setIsDeleteing:NO];
            ...
            switch (buttonIndex) {
                case 0://删除
                {
                    if (!self.isDeleteing) {
                        ZWEAKSELF
                        self.isDeleteing = YES;
                        [DataOper130 postCommentReplyWithUserId:[AppSetting getUserId] ids:self.modelLastCR.ids type:1 resultBlock:^(NSDictionary *result) {
                            GCDMainBlock(^{
                                [weakSelf setIsDeleteing:NO];
        ...
        case 13://回复别人的回复
        {
            switch (buttonIndex) {
                case 0://回复
                {
                    self.commentType = 2;
                    self.isComment = NO;
                    [self.viewComment setPlaceholderText:[NSString stringWithFormat:@"回复%@:", self.modelLastCR.hnickname]];
                    [self.viewComment setKeyboardShow];
                    [self.tvMain setViewScrollToRowAtRow:self.lastRow];
                    break;
                }
