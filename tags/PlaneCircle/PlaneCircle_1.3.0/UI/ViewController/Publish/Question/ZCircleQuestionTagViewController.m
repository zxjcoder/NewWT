...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self setRightButtonWithText:@"发布"];
...
-(void)innerInit
{
    ...
    [self.textField setOnTextDidChange:^(NSString *text) {
        [sns getCircleQueryArticleWithArtName:text pageNum:1 resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
...
-(void)innerData
{
    ...
    ZWEAKSELF
    [sns getCircleQueryArticleWithArtName:self.modelQuestion.title pageNum:1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnRightClick
{
    ...
    if (self.arrSelTag.count == 0) {
        [ZProgressHUD showError:@"请至少选择一个话题"];
        return;
    }
    if (self.arrSelTag.count > 5) {
        [ZProgressHUD showError:@"最多只能选择5个话题"];
        return;
    }
    if (self.arrSelTag.count == 1) {
        if ([[(ModelTag*)self.arrSelTag.firstObject tagId] isEqualToString:self.createTagId]) {
            [ZProgressHUD showError:@"请再多关联一个话题,才能发布哦"];
            return;
        }
    }
    NSString *title = self.modelQuestion.title;
    NSString *content = self.modelQuestion.content;
    ...
    ZWEAKSELF
    [ZProgressHUD showMessage:@"正在提问,请稍等..."];
    if (self.modelQuestion.bePracticeId && self.modelQuestion.bePracticeId.length > 0) {
        [DataOper130 postCirclePublishQuestionWithUserId:[AppSetting getUserDetauleId] question:title quizDetail:content articles:[selTagId toString] speechId:self.modelQuestion.bePracticeId resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
                [ZProgressHUD dismiss];
                [ZProgressHUD showSuccess:@"问题发布成功"];
                
                [weakSelf postPublishQuestionNotificationWithIndex:ZCircleToolViewItemNew];
                
                weakSelf.modelQuestion.ids = [result objectForKey:kResultKey];
                id preVC = [(ZCircleQuestionDescViewController*)weakSelf.preVC preVC];
                if (preVC) {
                    [preVC performSelectorOnMainThread:@selector(setPublishPracticeQuestion:) withObject:weakSelf.modelQuestion waitUntilDone:NO];
        ...
        [sns postCirclePublishQuestionWithUserId:[AppSetting getUserDetauleId] question:title quizDetail:content articles:[selTagId toString] resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
                [ZProgressHUD dismiss];
                [ZProgressHUD showSuccess:@"问题发布成功"];
...
-(void)btnAddTagClick
{
    ...
    [self.alertEditView setOnSubmitClick:^(NSString *content) {
        GCDMainBlock(^{
            ...
            if (tagName.length == 0) {
                [ZProgressHUD showError:@"话题不能为空"];
            } else if (tagName.length > kNumberTopicMaxLength) {
                [ZProgressHUD showError:@"话题长度限制[1-10]字符"];
            } else {
                [weakSelf.view endEditing:YES];
                [ZProgressHUD showMessage:@"正在添加,请稍等..."];
                [sns getCircleSaveArticleWithUserId:userId artName:content resultBlock:^(NSDictionary *result) {
                    GCDMainBlock(^{
                        ...
                        if (dicTag) {
                            ...
                            [ZProgressHUD showSuccess:@"话题添加成功"];
                        } else {
                            [ZProgressHUD showError:@"话题数据解析错误"];
                        }
                    });
                } errorBlock:^(NSString *msg) {
                    GCDMainBlock(^{
