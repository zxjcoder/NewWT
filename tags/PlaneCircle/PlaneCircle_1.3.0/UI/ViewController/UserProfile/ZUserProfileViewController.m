...
{
    ZWEAKSELF
    [ZProgressHUD showMessage:@"正在更新,请稍等..."];
...
-(void)btnAttClick
{
    ...
    if (self.modelUser.isAtt) {///已经关注
        [self.tvcButton setButtonState:NO];
        [self.modelUser setIsAtt:NO];
        [self.tvcButton setCellDataWithModel:self.modelUser];
        ZWEAKSELF
        [sns postDeleteAttentionWithAId:self.model.userId userId:[AppSetting getUserDetauleId] type:@"1" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
                [weakSelf setIsAtting:NO];
                [ZProgressHUD showSuccess:@"成功取消关注"];
                if (weakSelf.preVC != nil) {
                    [weakSelf.preVC performSelectorOnMainThread:@selector(setDeleteUserWithRow:) withObject:[NSNumber numberWithBool:NO] waitUntilDone:NO];
                }
            });
        } errorBlock:^(NSString *msg) {
            GCDMainBlock(^{
        ...
        [sns postAddAttentionWithuserId:[AppSetting getUserDetauleId] hisId:self.model.userId type:@"1" resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
                [weakSelf setIsAtting:NO];
                [ZProgressHUD showSuccess:@"关注成功"];
                if (weakSelf.preVC != nil) {
                    [weakSelf.preVC performSelectorOnMainThread:@selector(setDeleteUserWithRow:) withObject:[NSNumber numberWithBool:YES] waitUntilDone:NO];
                }
            });
        } errorBlock:^(NSString *msg) {
            GCDMainBlock(^{
...
-(void)btnBlackListClick
{
    ...
    [sns postAddBlackListWithUserId:[AppSetting getUserDetauleId] addUserId:self.model.userId resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            [ZProgressHUD showSuccess:@"成功加入黑名单"];
...
-(void)btnRightClick
{
    ...
    if (self.modelUser.isBlackList) {
        ...
        ZWEAKSELF
        // TODO:ZWW备注-1.0版本 无 举报, 1.1版本 有 举报
        ZShareView *shareView = [[ZShareView alloc] initWithShowShareType:(ZShowShareTypeReportBlacklist)];
        [shareView setOnItemClick:^(ZShareType shareType) {
            switch (shareType) {
                case ZShareTypeWeChat: [weakSelf btnWeChatClick]; break;
                case ZShareTypeWeChatCircle: [weakSelf btnWeChatCircleClick]; break;
