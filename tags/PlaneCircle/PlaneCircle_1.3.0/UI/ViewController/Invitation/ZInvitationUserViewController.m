...
-(void)innerInit
{
    ...
    [self.textField setOnTextDidChange:^(NSString *text) {
        if (text.length > 0) {
            ...
            [sns getSearchUserWithUserId:userId questionId:weakSelf.modelQB.ids flag:@"1" content:text pageNum:1 resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
            ...
            [weakSelf.arrMain addObjectsFromArray:weakSelf.arrHot];
