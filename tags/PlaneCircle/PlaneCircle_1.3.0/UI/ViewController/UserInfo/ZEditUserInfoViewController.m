...
-(void)btnRightClick
{
    ...
    if (![newNickName isNickName]) {
        [ZProgressHUD showError:[NSString stringWithFormat:@"昵称[1-%d]的字母,数字,汉字",kNumberNickNameMaxLength]];
        return;
    }
    if (newSign.length > kNumberSignMaxLength) {
        [ZProgressHUD showError:[NSString stringWithFormat:@"个性签名限制%d字符以内", kNumberSignMaxLength]];
        return;
    }
    if (newTrade.length > kNumberTradeMaxLength) {
        [ZProgressHUD showError:[NSString stringWithFormat:@"行业内容限制%d字符以内", kNumberTradeMaxLength]];
        return;
    }
    if ([newCompany isChinese]) {
        if (newCompany.length > kNumberCompanyCNMaxLength) {
            [ZProgressHUD showError:[NSString stringWithFormat:@"公司名称限制%d文字以内",kNumberCompanyCNMaxLength]];
            return;
        }
    } else {
        if (newCompany.length > kNumberCompanyENMaxLength) {
            [ZProgressHUD showError:[NSString stringWithFormat:@"公司名称限制%d字符以内",kNumberCompanyENMaxLength]];
            return;
        }
    }
    ZWEAKSELF
    ...
    [sns postUpdateUserInfoWithAccount:[AppSetting getUserDetauleId] headImg:self.strPhoto nickname:newNickName signature:newSign sex:sexType trade:newTrade company:newCompany address:newAddress resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            ...
            [ZProgressHUD showSuccess:@"个人资料编辑成功"];
