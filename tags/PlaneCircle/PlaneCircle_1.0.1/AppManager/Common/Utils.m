...
+(BOOL)isVersion100
{
    if ([APP_PROJECT_VERSION isEqualToString:kVersion100] ||
        [APP_PROJECT_VERSION isEqualToString:kVersion101]) {
        return YES;
    }
    return NO;
}
...
+(BOOL)isVersion110
{
    if ([APP_PROJECT_VERSION isEqualToString:kVersion110]) {
        return YES;
    }
    return NO;
}
...
//        imgH = imgH*APP_FRAME_WIDTH/imgW;
        
//        UIImage *newImage = [Utils scaleToSize:image size:CGSizeMake(imgW, imgH)];
