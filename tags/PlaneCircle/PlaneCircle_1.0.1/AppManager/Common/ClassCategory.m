...
-(NSDate*)toDate
{
    return [self toDateWithFormat:kFormat_Data_All];
}
...
-(NSDate*)toDateWithFormat:(NSString*)format
{
    ...
    [dateFormatter setTimeZone:[NSTimeZone timeZoneWithName:@"Asia/Shanghai"]];//timeZoneWithName:@"GMT"]];
...
{
    NSString *str = self;
    str=str==nil||[str isKindOfClass:[NSNull class]]?@"":str;
...
+(NSString*)stringFormatWithTimeStamp:(NSTimeInterval)timeStamp
{
    ...
    if (i==1) {
        return [NSString stringWithFormat:@"%@ %@", kFormat_Data_Day, [date toStringWithFormat:kFormat_Data_Hour]];
    } else if (i==2) {
        return [NSString stringWithFormat:@"%@ %@", kFormat_Data_Yesterday, [date toStringWithFormat:kFormat_Data_Hour]];
    } else if (i==3){
        return [date toStringWithFormat:kFormat_Data_Time];
    } else {
        return [date toStringWithFormat:kFormat_Data_Time];
    }
}
...
{
    return [self stringWithTimeStamp:timeStamp format:kFormat_Data_All];
}
...
- (CGFloat)getLabelWidthWithMinWidth:(CGFloat)minwidth
{
    CGRect fontFrame = [self.text boundingRectWithSize:CGSizeMake(MAXFLOAT, self.frame.size.height)
                                            ...
                                            attributes:@{NSFontAttributeName: self.font}
                                               context:nil];
    CGFloat w= fontFrame.size.width<minwidth?minwidth:fontFrame.size.width;
