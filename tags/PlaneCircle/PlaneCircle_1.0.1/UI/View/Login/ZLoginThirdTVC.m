...
-(void)innerInit
{
    ...
    [self.lbTitle setFont:[UIFont boldSystemFontOfSize:kFont_Least_Size]];
...
//    self.btnSina = [UIButton buttonWithType:(UIButtonTypeCustom)];
//    [self.btnSina setImage:[SkinManager getImageWithName:@"login_btn_weibo"] forState:(UIControlStateNormal)];
//    [self.btnSina setImageEdgeInsets:(UIEdgeInsetsMake(5, 5, 5, 5))];
//    [self.btnSina addTarget:self action:@selector(btnSinaClick) forControlEvents:(UIControlEventTouchUpInside)];
//    [self.viewMain addSubview:self.btnSina];
...
-(void)layoutSubviews
{
    ...
    CGFloat spaceX = 40;
    [self.lbTitle setFrame:CGRectMake(0, self.space, self.cellW, self.lbH)];
