...
-(void)innerInit
{
    ...
    [self.btnOper setViewRound:8 borderWidth:0 borderColor:CLEARCOLOR];
...
-(void)layoutSubviews
{
    ...
    [self.btnOper setFrame:CGRectMake(15, 15, self.cellW-30, 35)];
}
...
-(void)setButtonState:(BOOL)isAtting
{
    if (isAtting) {
        ...
        [self.btnOper setTitle:@"关 注" forState:(UIControlStateNormal)];
    } else {
...
-(void)setCellDataWithModel:(ModelUserProfile *)model
{
    ...
    if (model.isAtt) {
        [self.btnOper setBackgroundColor:RGBCOLOR(201, 201, 201)];
        [self.btnOper setTitle:@"已 关 注" forState:(UIControlStateNormal)];
    } else {
        [self.btnOper setBackgroundColor:MAINCOLOR];
        [self.btnOper setTitle:@"关 注" forState:(UIControlStateNormal)];
    }
