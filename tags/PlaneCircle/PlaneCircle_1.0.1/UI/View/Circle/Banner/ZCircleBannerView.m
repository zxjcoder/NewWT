...
-(void)innerInit
{
    ...
    [self.pageControl setPageIndicatorTintColor:GRAYCOLOR];
...
-(void)setContentOffsetFrist
{
    ...
    [self.pageControl setNumberOfPages:self.svArray.count-1];
...
-(void)timerCallback
{
    if (_timerIndex==(self.svArray.count-1)) {//最后一条
        _timerIndex = 0;
...
{
    _timerIndex = (NSInteger)abs((int)(scrollView.contentOffset.x / scrollView.width));
    _currentPage = _timerIndex;
    if (_timerIndex==(self.svArray.count-1)) {//最后一条
        _currentPage = 0;
        _timerIndex = 0;
        [self.scrollView setContentOffset:CGPointMake(self.scrollView.width*_timerIndex, 0) animated:NO];
