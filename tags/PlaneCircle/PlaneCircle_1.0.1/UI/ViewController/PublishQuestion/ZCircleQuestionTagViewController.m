...
-(void)btnRightClick
{
    ...
    if (self.arrSelTag.count == 1) {
        if ([[(ModelTag*)self.arrSelTag.firstObject tagId] isEqualToString:self.createTagId]) {
            [ZProgressHUD showError:@"请再多关联一个话题,才能发布哦" toView:self.view];
            return;
        }
    }
    ...
    ZWEAKSELF
    [ZProgressHUD showMessage:@"正在提问,请稍等..." toView:weakSelf.view];
...
-(void)btnAddTagClick
{
    ...
    [self.alertEditView setOnSubmitClick:^(NSString *content) {
        GCDMainBlock(^{
            ...
            if (tagName.length == 0) {
                [ZProgressHUD showError:@"话题不能为空" toView:weakSelf.view];
            } else if (tagName.length > kNumberTopicMaxLength) {
                [ZProgressHUD showError:@"话题长度限制[1-10]字符" toView:weakSelf.view];
            } else {
