...
-(void)btnRightClick
{
    ...
    if ([newNickName isEmpty]) {
        [ZProgressHUD showError:@"昵称不能为空" toView:self.view];
        return;
    }
    if (![newNickName isNickName]) {
        [ZProgressHUD showError:@"昵称[1-12]的字母,数字,汉字" toView:self.view];
        return;
    }
    if (newSign.length > 30) {
        [ZProgressHUD showError:@"个性签名[<=30]字符,你输入太多了" toView:self.view];
        return;
    }
    if (newTrade.length > 10) {
        [ZProgressHUD showError:@"行业内容[<=10]字符,你输入太多了" toView:self.view];
        return;
    }
    if ([newCompany isChinese]) {
        if (newCompany.length > 20) {
            [ZProgressHUD showError:@"公司名称[<=20]文字,你输入的太多了" toView:self.view];
            return;
        }
    } else {
        if (newCompany.length > 60) {
            [ZProgressHUD showError:@"公司名称[<=60]字符,你输入的太多了" toView:self.view];
            return;
        }
    }
    ZWEAKSELF
