...
-(void)innerInit
{
    ...
    [self.viewAnswer setBackgroundColor:CLEARCOLOR];
...
-(void)setCellDataWithModel:(ModelMyNewQuestion *)model
{
    ...
    if (model) {
        ...
        if (model.nickname.length > 0) {
            ...
            [self.lbNickName setLabelColorWithRange:NSMakeRange(model.nickname.length, kAnswerYourQuestion.length) color:BLACKCOLOR1];
        } else {
            [self.lbNickName setText:kAnswerYourQuestion];
            [self.lbNickName setTextColor:BLACKCOLOR1];
        }
