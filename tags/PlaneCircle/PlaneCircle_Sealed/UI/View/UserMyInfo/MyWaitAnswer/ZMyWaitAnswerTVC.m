...
-(void)setCellDataWithModel:(ModelWaitAnswer *)model
{
    ...
    if (model) {
        ...
        if (model.nickname.length > 0) {
            ...
            [self.lbNickName setLabelColorWithRange:NSMakeRange(model.nickname.length, kInvitationYourAnswerQuestion.length) color:BLACKCOLOR1];
        } else {
            [self.lbNickName setText:kInvitationYourAnswerQuestion];
            [self.lbNickName setTextColor:BLACKCOLOR1];
        }
