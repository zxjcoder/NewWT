...
-(void)innerInit
{
    ...
    self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, scViewY, APP_FRAME_WIDTH, scViewH)];
    ...
    [self.tvTopicQuestion setOnRowSelected:^(ModelQuestionTopic *model) {
        if (weakSelf.onQuestionClick) {
            weakSelf.onQuestionClick(model);
        }
    }];
    [self.tvTopicQuestion setOnAnswerClick:^(ModelQuestionTopic *model) {
