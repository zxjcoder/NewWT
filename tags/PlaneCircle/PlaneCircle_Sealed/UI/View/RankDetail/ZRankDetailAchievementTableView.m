...
-(void)innerInit
{
    ...
    [self.btnCompany setTitle:kThreeNewBoard forState:(UIControlStateNormal)];
    [self.btnCompany setTitleColor:BLACKCOLOR forState:(UIControlStateNormal)];
    [self.btnCompany setTag:1];
    [[self.btnCompany titleLabel] setFont:[ZFont boldSystemFontOfSize:kFont_Default_Size]];
    ...
    [self.lbNumber setText:kRankTitle];
    ...
    [self.lbCode setText:kListingTheNumber];
...
-(void)setViewDataWithModel:(ModelRankCompany *)model
{
    switch (model.type) {
        case WTRankTypeL:
            ...
            break;
        case WTRankTypeK:
            [self.lbName setText:kAccountingFirm];
            break;
        case WTRankTypeZ:
            [self.lbName setText:kSecuritiesCompany];
            break;
        default:break;
    }
}
