//
//  ZPictureViewerViewController.m
//  PlaneCircle
//
...
@interface ZPictureViewerViewController () {
    // Data
...
///导航栏背景
@property (strong, nonatomic) UIView *viewNavBG;
///标题
...
///工具栏背景
@property (strong, nonatomic) UIView *viewToolBG;
///滑动条
...
@implementation ZPictureViewerViewController
...
- (void)viewDidLoad {
    ...
    [self.viewNav setBackgroundColor:CLEARCOLOR];
    [self.view addSubview:self.viewNav];
    
    self.viewNavBG = [[UIView alloc] init];
    [self.viewNavBG setBackgroundColor:BLACKCOLOR];
    [self.viewNavBG setAlpha:0.58f];
    [self.viewNav addSubview:self.viewNavBG];
    
    self.btnBack = [UIButton buttonWithType:(UIButtonTypeCustom)];
    [self.btnBack setImage:[SkinManager getImageWithName:@"btn_back1"] forState:UIControlStateNormal];
    [self.btnBack setImage:[SkinManager getImageWithName:@"btn_back1"] forState:UIControlStateHighlighted];
    [self.btnBack setImageEdgeInsets:(UIEdgeInsetsMake(15, 15, 0, 10))];
    [self.btnBack setTitleEdgeInsets:(UIEdgeInsetsMake(0, 0, 0, 0))];
    [self.btnBack addTarget:self action:@selector(doneButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    ...
    [self.viewNav addSubview:self.btnBack];
    
    self.lbTitle = [[UILabel alloc] init];
    if ([self numberOfPhotos] > 0) {
        [self.lbTitle setText:[NSString stringWithFormat:@"1/%ld",(unsigned long)[self numberOfPhotos]]];
    } else {
        [self.lbTitle setText:nil];
    }
    [self.lbTitle setTextAlignment:(NSTextAlignmentRight)];
    [self.lbTitle setTextColor:WHITECOLOR];
    [self.lbTitle setFont:[UIFont systemFontOfSize:kFont_Middle_Size]];
    [self.viewNav addSubview:self.lbTitle];
    
    self.viewTool = [[UIView alloc] init];
    [self.viewTool setBackgroundColor:CLEARCOLOR];
    [self.view addSubview:self.viewTool];
    
    self.viewToolBG = [[UIView alloc] init];
    [self.viewToolBG setBackgroundColor:BLACKCOLOR];
    [self.viewToolBG setAlpha:0.58f];
    [self.viewTool addSubview:self.viewToolBG];
    
    self.btnStop = [UIButton buttonWithType:(UIButtonTypeCustom)];
    [self.btnStop setImageEdgeInsets:(UIEdgeInsetsMake(3, 3, 3, 3))];
    [self.btnStop setTitleEdgeInsets:(UIEdgeInsetsMake(0, 0, 0, 0))];
    [self.btnStop addTarget:self action:@selector(btnStopClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.viewTool addSubview:self.btnStop];
    
    if ([[AudioPlayerView shareAudioPlayerView] isPlay]) {
        [self.btnStop setTag:111];
        [self.btnStop setImage:[SkinManager getImageWithName:@"p_shape_stop"] forState:UIControlStateNormal];
        [self.btnStop setImage:[SkinManager getImageWithName:@"p_shape_stop"] forState:UIControlStateHighlighted];
    } else {
        [self.btnStop setTag:112];
        [self.btnStop setImage:[SkinManager getImageWithName:@"p_play_play"] forState:UIControlStateNormal];
        [self.btnStop setImage:[SkinManager getImageWithName:@"p_play_play"] forState:UIControlStateHighlighted];
    }
    
    self.slider = [[ZSlider alloc] init];
    [self.slider setContinuous:YES];
    [self.slider setMinimumValue:0];
    [self.slider setMaximumValue:[self numberOfPhotos]-1];
    [self.slider setValue:_currentPageIndex];
    [self.slider setMinimumTrackTintColor:MAINCOLOR];
    [self.slider setMaximumTrackTintColor:WHITECOLOR];
    [self.slider setThumbTintColor:WHITECOLOR];
...
//    [self.slider addTarget:self action:@selector(sliderRemoveFocus:) forControlEvents:(UIControlEventTouchUpInside)];
    [self.viewTool addSubview:self.slider];
...
-(void)sliderValueChanged:(UISlider *)sender
{
    if (sender.value != slidervalue) {
        ...
        NSInteger index = ceil(slidervalue);
        if (index < [self numberOfPhotos]) {
            CGRect pageFrame = [self frameForPageAtIndex:index];
            
            [_pagingScrollView scrollRectToVisible:CGRectMake(pageFrame.origin.x - PADDING, 0, _pagingScrollView.width, _pagingScrollView.height) animated:NO];
            [self updateToolbar];
        }
    }
}
/// 停止按钮
-(void)btnStopClick:(UIButton *)sender
{
    if (sender.tag == 111) {
        [_btnStop setTag:112];
        [_btnStop setImage:[SkinManager getImageWithName:@"p_play_play"] forState:UIControlStateNormal];
        [_btnStop setImage:[SkinManager getImageWithName:@"p_play_play"] forState:UIControlStateHighlighted];
        [sqlite setSysParam:kSQLITE_LAST_STOPPLAYSTATE value:@"YES"];
        [[AudioPlayerView shareAudioPlayerView] pause];
    } else {
        [_btnStop setTag:111];
        [_btnStop setImage:[SkinManager getImageWithName:@"p_shape_stop"] forState:UIControlStateNormal];
        [_btnStop setImage:[SkinManager getImageWithName:@"p_shape_stop"] forState:UIControlStateHighlighted];
        [sqlite setSysParam:kSQLITE_LAST_STOPPLAYSTATE value:@"NO"];
        [[AudioPlayerView shareAudioPlayerView] play];
    }
}
...
- (void)tilePages {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            page = [[IDMZoomingScrollView alloc] initWithPrePhotoBrowser:self];
...
- (CGRect)frameForPagingScrollView {
    ...
    if (APP_SYSTEM_VERSION < 8) {
        ...
        if (UIInterfaceOrientationIsLandscape([[UIApplication sharedApplication] statusBarOrientation])) {
            frame.size.width = self.view.height;
            frame.size.height = self.view.width;
        }
    }
...
//
//    return CGRectMake(0, 10, 60, 54);//CGRectMake(screenWidth - 75, 30, 55, 26);
//}
...
- (void)setupScrollViewContentSize
{
    ...
    if (APP_SYSTEM_VERSION < 8) {
        if (UIDeviceOrientationIsLandscape([[UIDevice currentDevice] orientation])) {
            viewW = self.view.height;
            viewH = self.view.width;;
        }
    }
    [self.viewNav setFrame:CGRectMake(0, 0, viewW, 64)];
    [self.viewNavBG setFrame:self.viewNav.bounds];
    [self.btnBack setFrame:CGRectMake(0, 10, 60, 54)];
    [self.lbTitle setFrame:CGRectMake(self.viewNavBG.frame.size.width-200-15, 30, 200, 20)];
    
    [self.viewTool setFrame:CGRectMake(0, viewH-70, viewW, 70)];
    [self.viewToolBG setFrame:self.viewTool.bounds];
    
    [self.slider setFrame:CGRectMake(0, 5, viewW, 15)];
    [self.btnStop setFrame:CGRectMake(self.viewTool.frame.size.width/2-40/2, self.slider.frame.origin.y+self.slider.frame.size.height+5, 40, 40)];
}
...
- (void)updateToolbar {
    ...
    if ([self numberOfPhotos] > 1) {
        ...
        _lbTitle.text = [NSString stringWithFormat:@"%lu/%lu", (unsigned long)(_currentPageIndex+1), (unsigned long)[self numberOfPhotos]];
    } else {
        _lbTitle.text = nil;
...
- (void)jumpToPageAtIndex:(NSUInteger)index {
    ...
    if (index < [self numberOfPhotos]) {
        ...
        {
            [_pagingScrollView scrollRectToVisible:CGRectMake(pageFrame.origin.x - PADDING, 0, _pagingScrollView.width, _pagingScrollView.height) animated:YES];
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    [UIView animateWithDuration:(animated ? 0.1 : 0) animations:^(void) {
        ...
        [_viewNav setAlpha:alpha];
        [_viewTool setAlpha:alpha];
...
- (BOOL)areControlsHidden { return (_viewTool.alpha == 0); }
...
- (void)actionButtonPressed:(id)sender {
    ...
    if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
        ...
        else
        {
            ...
            self.actionsSheet.cancelButtonIndex = [self.actionsSheet addButtonWithTitle:kCancel];
