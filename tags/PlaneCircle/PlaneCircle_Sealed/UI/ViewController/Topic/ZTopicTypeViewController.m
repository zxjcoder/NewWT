...
-(void)refreshData:(BOOL)isRefresh
{
    ...
    [sns getTopicListWithUserId:[AppSetting getUserDetauleId] typeId:self.modelTT.typeId pageNum:self.pageNum resultBlock:^(NSArray *arrReulst, NSDictionary *result) {
        GCDMainBlock(^{
...
{
    ZWEAKSELF
    [sns getTopicListWithUserId:[AppSetting getUserDetauleId] typeId:self.modelTT.typeId pageNum:self.pageNum+1 resultBlock:^(NSArray *arrReulst, NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
