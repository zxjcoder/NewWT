...
{
    //添加统计事件
    [StatisticsManager event:kEvent_Practice_Detail_Text category:kCategory_Practice];
...
-(void)btnPraiseClick
{
    ...
    if ([AppSetting getAutoLogin]) {
        if (self.isPraiseing) {
            [ZProgressHUD showError:kCMsgPraiseing];
            return;
        }
...
-(void)btnCollectionClick
{
    ...
    if ([AppSetting getAutoLogin]) {
        if (self.isCollectioning) {
            [ZProgressHUD showError:kCMsgCollecting];
            return;
        }
        ...
        if (!self.modelP.isCollection) {
            ...
            [sns getDelCollectionWithUserId:[AppSetting getUserDetauleId] cid:self.modelP.ids type:@"3" resultBlock:^(NSDictionary *result) {
                GCDMainBlock(^{
                    [ZProgressHUD showSuccess:kCMsgCancelCollectionSuccess];
...
-(void)shareWithType:(WTPlatformType)type
{
    ...
    [ShareManager shareWithTitle:title content:content imageUrl:imageUrl webUrl:webUrl platformType:type resultBlock:^(bool isSuccess, NSString *msg) {
        if (!isSuccess && msg) {
            ...
            ///编辑任务
            ModelTask *modelTask = [sqlite getLocalTaskModelWithUserId:[AppSetting getUserDetauleId] rule:ZTaskTypeSharePracticeToCircle];
