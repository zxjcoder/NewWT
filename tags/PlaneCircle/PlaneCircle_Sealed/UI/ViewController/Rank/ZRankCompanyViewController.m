...
- (void)viewDidLoad
{
    ...
    [self setTitle:kInstitutionalDetail];
        ...
        GCDMainBlock(^{
            ...
            if (strUrl && strUrl.length > 0) {
                NSString *localPath = [[[AppSetting getOfficeFilePath]
                                        stringByAppendingPathComponent:weakSelf.modelRC.company_name]
...
-(void)shareWithType:(WTPlatformType)type
{
    ...
    if (self.modelRC.company_img == nil || self.modelRC.company_img.length == 0) {
        ...
        NSString *webUrl = kShare_RankOrgUrl(self.modelRC.ids,self.viewMain.selType);
        ...
        NSString *imageUrl = [Utils getMinPicture:self.modelRC.company_img];
        NSString *webUrl = kShare_RankOrgUrl(self.modelRC.ids,self.viewMain.selType);
