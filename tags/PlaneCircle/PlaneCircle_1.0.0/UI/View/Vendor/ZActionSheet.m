...
//背景区域设置
#define BACKGROUNDVIEW_INTERVAL_WIDTH           10
...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructionButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitlesArray
{
    ...
    if (destructiveButtonTitle) {
        ...
        if (otherButtonTitlesArray.count==0) {
            [destructiveButton setViewRound:10 borderWidth:0 borderColor:CLEARCOLOR];
        }
    }
        ...
        if (otherButtonTitlesArray.count > 0) {
            ...
            if (self.isHadTitle == YES || self.isHadDestructionButton == YES) {
                for (int i = 0; i<otherButtonTitlesArray.count; i++) {
                    ...
                    self.postionIndexNumber++;
                    if (i==(otherButtonTitlesArray.count-1)) {
                        [otherButton setViewRound:10 borderWidth:0 borderColor:CLEARCOLOR];
                    }
                }
            }
        }
    ...
    if (cancelButtonTitle) {
        ...
        if (self.isHadTitle == YES || self.isHadDestructionButton == YES || self.isHadOtherButton == YES) {
            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.ActionSheetHeight+10, cancelButton.frame.size.width, cancelButton.frame.size.height)];
            self.ActionSheetHeight = self.ActionSheetHeight + cancelButton.frame.size.height+BUTTON_INTERVAL_HEIGHT+10;
        }
        [cancelButton setViewRound:10 borderWidth:0 borderColor:CLEARCOLOR];
