...
-(void)innerInit
{
    ...
    [self.btnCheck setImage:[SkinManager getImageWithName:@"tianjiahuati_btn_default"] forState:(UIControlStateNormal)];
...
-(void)setTVCCheckTag:(BOOL)isCheck
{
    ...
    if (isCheck) {
        [self.btnCheck setImage:[SkinManager getImageWithName:@"tianjiahuati_btn_pressed"] forState:(UIControlStateNormal)];
    } else {
        [self.btnCheck setImage:[SkinManager getImageWithName:@"tianjiahuati_btn_default"] forState:(UIControlStateNormal)];
    }
}
...
-(void)setCellDataWithModel:(ModelTag *)model
{
    [self setModel:model];
    if (model) {
        [self.imgIcon setImageURLStr:model.tagLogo placeImage:[SkinManager getImageWithName:@"new_user_photo"]];
