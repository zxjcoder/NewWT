...
-(void)innerInit
{
    ...
    [self.lbTagName setFont:[UIFont boldSystemFontOfSize:kFont_Default_Size]];
    ...
    [self.lbAttCount setText:@"0个人关注"];
    [self.lbAttCount setFont:[UIFont systemFontOfSize:kFont_Small_Size]];
...
-(void)setViewDataWithModel:(ModelTag *)model
{
    ...
    if (model) {
        ...
        [self.lbAttCount setText:[NSString stringWithFormat:@"%ld个人关注",model.attCount]];
        if (model.isAtt) {
            [self.btnAtt setBackgroundColor:RGBCOLOR(201, 201, 201)];
        } else {
            [self.btnAtt setBackgroundColor:MAINCOLOR];
