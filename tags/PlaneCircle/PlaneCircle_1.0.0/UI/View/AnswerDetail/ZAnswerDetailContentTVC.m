            ...
            for (NSString *sepValue in arrSepValue) {
                if ([sepValue isEqualToString:value]) {
                    if (arrImage.count > row) {
                        ...
                        GBLog(@"self.modelAB.imageArray-Item ImgPath: %@", imgHtml);
                        
                        UIImage *img = [[SEImageCache sharedInstance] imageForURL:[NSURL URLWithString:imgPath] completionBlock:^(UIImage *image, NSError *error) {
                            if (image) {
