...
-(void)btnBlackListClick
{
    ...
    [sns postAddBlackListWithUserId:[AppSetting getUserDetauleId] addUserId:self.model.userId resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            [ZProgressHUD showSuccess:@"已成功加入黑名单" toView:weakSelf.view];
...
{
    ZWEAKSELF
    [sns postAddReportWithUserId:[AppSetting getUserDetauleId] objId:self.model.userId type:@"person" content:nil resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnRightClick
{
    ...
    if (self.modelUser.isBlackList) {
        ...
        ZWEAKSELF
        ZShareView *shareView = [[ZShareView alloc] initWithShowShareType:(ZShowShareTypeBlacklist)];
        [shareView setOnItemClick:^(ZShareType shareType) {
            switch (shareType) {
                case ZShareTypeWeChat: [weakSelf btnWeChatClick]; break;
                case ZShareTypeWeChatCircle: [weakSelf btnWeChatCircleClick]; break;
