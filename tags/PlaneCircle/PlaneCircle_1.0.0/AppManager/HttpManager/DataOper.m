 ...
 *  @param userId       用户ID
 *  @param objId        举报对象ID
 *  @param type         举报类型 答案:answer,人:person,实务:hotspot
 *  @param content      举报内容
 *  @result resultBlock 成功回调
 *  @result errorBlock  错误回调
 */
+(void)postAddReportWithUserId:(NSString *)userId objId:(NSString *)objId type:(NSString *)type content:(NSString *)content resultBlock:(void(^)(NSDictionary *result))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock
{
    @try {
        NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
        if (userId) {
...
+(void)getExportOrganizationWithOrganizationId:(NSString *)organizationId pageNum:(int)pageNum resultBlock:(void(^)(NSDictionary *result))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock
{
    @try {
        NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
        if (organizationId) {
            [dicParam setObject:organizationId forKey:@"organizationId"];
        }
        [dicParam setObject:[NSNumber numberWithInt:pageNum] forKey:@"pageNum"];
        [dicParam setObject:[NSNumber numberWithInt:kPAGE_MAXCOUNT] forKey:@"pageSize"];
