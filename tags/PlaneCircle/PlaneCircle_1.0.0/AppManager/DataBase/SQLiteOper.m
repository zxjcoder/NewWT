...
{
    NSString *filePath=[[AppSetting getRecvFilePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.plist",pathKey]];
...
{
    if (dic) {
        NSString *filePath=[[AppSetting getRecvFilePath] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@key",[Utils stringMD5:pathKey]]];
 ...
 *  操作一条数据
 *  param sql 插入语句
 *  param modelParams 插入每条数据的参数集合
 *  result 返回插入状态
 */
-(BOOL)executeUpdate:(NSString*)sql modelParams:(NSArray*)modelParams
{
    BOOL __block result = NO;
    
    if (!modelParams || modelParams.count == 0) return result;
    
    [dbQueue inDatabase:^(FMDatabase *db) {
        result = [db executeUpdate:sql withArgumentsInArray:modelParams];
    }];
...
-(ModelPractice *)getLocalPracticeDetailModelWithId:(NSString *)ids userId:(NSString *)userId
{
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet *rs = [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM sns_practice_detail WHERE ids = '%@' AND userId = '%@' LIMIT 0,1",ids,userId]];
        if (rs != nil)
        {
            while ([rs next]) {
                result = [rs resultDictionary];
...
-(BOOL)setLocalAppConfigWithModel:(ModelAppConfig *)model
{
    ...
    [dbQueue inDatabase:^(FMDatabase *db) {
        result = [db executeUpdate:@"REPLACE INTO sns_appconfig(appVersion,platform,appStatus,sortby) VALUES(?,?,?,strftime('%s%f','now'));" withArgumentsInArray:@[appVersion,platform,appStatus]];
    }];
