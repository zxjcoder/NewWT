...
- (void)postWithBlock:(void(^)(Response *result))resultBlock
{
    ...
    if (self.arrayFile.count > 0) {
        ...
        //TODO:ZWW备注-发布屏蔽此代码
#ifdef DEBUG
        NSDictionary *dicParam = self.parameters;
        NSArray *arrFile = self.arrayFile;
#endif
        ...
        dataTask = [manager dataTaskWithRequest:request
                              ...
                              completionHandler:^(NSURLResponse * _Nonnull response, id  _Nullable responseObject, NSError * _Nullable error) {
                                  if (error) {
                                      ...
                                      NSString * strResponseObject = [[NSString alloc] initWithData:jsonData encoding:NSUTF8StringEncoding];
                                      NSLog(@"RequestBase: %@ \n RequestParams: %@ \n RequestFiles: %@ \n ResponseData: %@", response, dicParam, arrFile, strResponseObject);
#endif
                                  }
                              }];
        [dataTask resume];
