...
@interface ZRegisterAccountTVC()<UITextFieldDelegate>
{
    NSTimer *_timer;
    int _time;
}
...
-(void)innerInit
{
    ...
    [self.btnSend setTitle:@"获取验证码" forState:(UIControlStateNormal)];
    ...
    [[self.btnSend titleLabel] setFont:[UIFont systemFontOfSize:kFont_Least_Size]];
    ...
    [self.textPassword setPlaceholder:@"请输入6-16位的新密码"];
...
-(void)layoutSubviews
{
    ...
    [self.textAccount setFrame:CGRectMake(textX, textY, textW, textH)];
    ...
    [self.textCode setFrame:CGRectMake(textX, textcY, textW-btnW, textH)];
    ...
    [self.textPassword setFrame:CGRectMake(textX, textpY, textW-40, textH)];
...
{
    [self.btnSend setTag:10];
    [self.btnSend setTitle:@"发送验证码" forState:(UIControlStateNormal)];
    [self.btnSend setTitleColor:MAINCOLOR forState:UIControlStateNormal];
}
...
-(void)timerCallback
{
    if (_time<=0) {
        ...
        _time--;
        [self.btnSend setTitle:[NSString stringWithFormat:@"重新发送(%d)",_time] forState:(UIControlStateNormal)];
