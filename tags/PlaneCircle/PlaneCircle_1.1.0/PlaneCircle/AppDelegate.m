...
-(BOOL)getHandleOpenURL:(NSURL*)url sourceApplication:(NSString*)sourceApplication annotation:(id)annotation isHttps:(BOOL)isHttps
{
    ...
    if (self.openUrlType==WTOpenURLTypeWechat) {
        self.openUrlType = WTOpenURLTypeNone;
...
-(void)setMonitorNetworkStatusChanges
{
    ...
    [manager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            case AFNetworkReachabilityStatusUnknown: GBLog(@"未识别的网络"); break;
            case AFNetworkReachabilityStatusNotReachable: GBLog(@"不可达的网络(未连接)"); break;
            case AFNetworkReachabilityStatusReachableViaWWAN: GBLog(@"2G,3G,4G...的网络"); break;
            case AFNetworkReachabilityStatusReachableViaWiFi: GBLog(@"wifi的网络"); break;
            default: break;
        }
    }];
