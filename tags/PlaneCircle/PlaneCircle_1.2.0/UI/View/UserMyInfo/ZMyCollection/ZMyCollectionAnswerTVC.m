...
-(void)setCellFontSize
{
    ...
    [self.lbQuestion setFont:[UIFont boldSystemFontOfSize:kSet_Font_Default_Size(self.fontSize)]];
    [self.lbAnswer setFont:[UIFont systemFontOfSize:kSet_Font_Small_Size(self.fontSize)]];
}
-(void)setViewFrame
{
    [self setCellFontSize];
...
-(void)setCellDataWithModel:(ModelCollectionAnswer *)model
{
    [self setModelCA:model];
    if (model) {
        [self.lbQuestion setText:model.questions];
        
        NSString *content = model.content.imgReplacing;
        if (content) {
