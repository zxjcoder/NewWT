...
@interface ZAlertEditView()<UITextFieldDelegate>
{
    NSString *_lastText;
}
///是否在动画
...
-(void)setContentView
{
    ...
    [[self.btnCancel titleLabel] setFont:[UIFont systemFontOfSize:kFont_Middle_Size]];
    ...
    [[self.btnSave titleLabel] setFont:[UIFont systemFontOfSize:kFont_Middle_Size]];
...
-(void)setViewFrame
{
    ...
    [self.textField setFrame:CGRectMake(20, 20, kAlertEditViewContentW-40, 40)];
    CGFloat btnH = 45;
...
-(void)btnSaveClick
{
//    NSString *tagName = self.textField.text.toTrim;
//    if (tagName.length == 0) {
//        [ZProgressHUD showError:@"话题不能为空" toView:self];
//        return;
//    }
//    if (tagName.length > 5) {
//        [ZProgressHUD showError:@"话题长度限制[1-5]字符" toView:self];
//        return;
//    }
...
{
    if (![_lastText isEqualToString:self.textField.text]) {
