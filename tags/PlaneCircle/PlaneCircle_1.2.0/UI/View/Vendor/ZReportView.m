...
///提交按钮分割线
@property (strong, nonatomic) UIView *viewLine5;
///底部区域
...
-(void)innerInit
{
    ...
    [self.btnOther addTarget:self action:@selector(btnOtherClick) forControlEvents:(UIControlEventTouchUpInside)];
    ...
    [self.viewLine5 setBackgroundColor:TABLEVIEWCELL_LINECOLOR];
    [self.viewBottom addSubview:self.viewLine5];
...
-(void)btnSubmitClick
{
    if (self.onSubmitClick) {
        NSString *content = kEmpty;
        if (self.btnItem1.isChecked) {
            content = [content stringByAppendingString:self.btnItem1.getText];
        }
        if (self.btnItem2.isChecked) {
            if (![content isEqualToString:kEmpty]) {
                content = [content stringByAppendingString:@","];
            }
            content = [content stringByAppendingString:self.btnItem2.getText];
        }
        if (self.btnItem3.isChecked) {
            if (![content isEqualToString:kEmpty]) {
                content = [content stringByAppendingString:@","];
            }
            content = [content stringByAppendingString:self.btnItem3.getText];
        }
        if (self.btnItem4.isChecked) {
            if (![content isEqualToString:kEmpty]) {
                content = [content stringByAppendingString:@","];
            }
            content = [content stringByAppendingString:self.btnItem4.getText];
        }
        self.onSubmitClick(content);
        if (![content isEqualToString:kEmpty]) {
            [self dismiss];
        }
    } else {
...
-(void)setViewFrame
{
    ...
    CGFloat itemW = viewW/2-0.5;
    [self.btnCancel setFrame:CGRectMake(0, 0, itemW, btnH)];
    [self.viewLine5 setFrame:CGRectMake(itemW, 0, lineH, btnH)];
    [self.btnSubmit setFrame:CGRectMake(self.viewLine5.x+lineH, 0, itemW, btnH)];
