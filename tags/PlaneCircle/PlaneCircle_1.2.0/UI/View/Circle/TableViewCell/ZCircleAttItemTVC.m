...
///实体对象
@property (strong, nonatomic) ModelCircleAtt *model;
///分割线
@property (strong, nonatomic) UIView *viewLine;

@end

@implementation ZCircleAttItemTVC
...
-(void)setCellFontSize
{
    ...
    [self.lbCount setFont:[UIFont systemFontOfSize:kSet_Font_Min_Size(self.fontSize)]];
    [self.lbTitle setFont:[UIFont boldSystemFontOfSize:kSet_Font_Middle_Size(self.fontSize)]];
    [self.lbNickName setFont:[UIFont systemFontOfSize:kSet_Font_Middle_Size(self.fontSize)]];
...
-(void)setViewFrame
{
    ...
    [self.viewLineAnswer setFrame:CGRectMake(self.space, self.lbTitle.y+self.lbTitle.height+self.lineH+5, self.viewRight.width, self.lineH)];
    ...
    if ([self.model.aid intValue] == 0) {
        ...
        if (self.lbAnswerContent.text.length > 0) {
            ...
            CGFloat maxH = [[ZCalculateLabel shareCalculateLabel] getMaxLineHeightWithLabel:self.lbAnswerContent];
...
-(void)setCellDataWithModel:(ModelCircleAtt *)model
{
    [self setModel:model];
