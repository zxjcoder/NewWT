...
-(void)btnRightClick
{
    ...
    if ([oldPwd isEmpty]) {
        [ZProgressHUD showSuccess:@"原始密码不能为空"];
        return;
    }
    if (![oldPwd isPassword]) {
        [ZProgressHUD showSuccess:@"原始密码限制[6-16]数字或字母"];
        return;
    }
    NSString *newPwd = self.tvcNewPwd.getText;
    if ([newPwd isEmpty]) {
        [ZProgressHUD showSuccess:@"新密码不能为空"];
        return;
    }
    if (![newPwd isPassword]) {
        [ZProgressHUD showSuccess:@"新密码限制[6-16]数字或字母"];
        return;
    }
    if ([oldPwd isEqualToString:newPwd]) {
        [ZProgressHUD showSuccess:@"新密码不能与原始密码一致"];
        return;
    }
    ZWEAKSELF
    [self setIsLoaded:YES];
    [ZProgressHUD showMessage:@"正在修改,请稍等..."];
