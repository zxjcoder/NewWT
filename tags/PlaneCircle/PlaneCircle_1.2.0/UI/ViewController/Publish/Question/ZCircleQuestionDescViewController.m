...
-(void)innerInit
{
    ...
    [self.textView setPlaceholderText:@"请输入您问题的具体描述"];
    ...
    [self.textView setOnRevokeChange:^(NSRange range, int inputLength) {
        if (weakSelf.textView.text.length > kNumberQuestionDescMaxLength) {
            [weakSelf setViewTitle:[NSString stringWithFormat:@"超长%d字符",(int)(weakSelf.textView.text.length-kNumberQuestionDescMaxLength)]];
        } else {
            [weakSelf setViewTitle:weakSelf.title];
        }
        [sqlite setSysParam:kSQLITE_LAST_QUESTION_CONTENT value:weakSelf.textView.text];
