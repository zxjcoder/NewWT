...
@interface ZSelectTopicViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
{
    NSString *_lastText;
}
///顶部区域
...
@implementation ZSelectTopicViewController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (self.preVC.modelQuestion.arrSelTag.count > 0) {
        [self.arrSelTag addObjectsFromArray:self.preVC.modelQuestion.arrSelTag];
    }
...
-(void)innerInit
{
    ...
    [self.textField setFont:[UIFont systemFontOfSize:kFont_Small_Size]];
    ...
    [self.lbAddTag setFont:[UIFont boldSystemFontOfSize:kFont_Small_Size]];
...
-(void)innerData
{
    ...
    ZWEAKSELF
    [sns getCircleQueryArticleWithArtName:weakSelf.preVC.modelQuestion.title pageNum:1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
...
-(void)btnRightClick
{
    ...
    NSString *title = self.preVC.modelQuestion.title;
    NSString *content = self.preVC.modelQuestion.content;
    ...
    [sns postCirclePublishQuestionWithUserId:[AppSetting getUserDetauleId] question:title quizDetail:content articles:[selTagId toString] resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            ...
            [sqlite setSysParam:kSQLITE_LAST_QUESTION_CONTENT value:kEmpty];
        });
...
-(void)setTagCheckWithModel:(ModelTag *)model selTagTVC:(ZQuestionSelectTagTVC *)selTagTVC
{
    ...
    [self.preVC.modelQuestion setArrSelTag:self.arrSelTag];
}
...
-(void)textFieldDidChange:(NSNotification *)sender
{
    if (![_lastText isEqualToString:self.textField.text.toTrim]) {
        ...
        if ([Utils isContainsEmoji:self.textField.text]) {
