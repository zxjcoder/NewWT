...
- (void)postWithBlock:(void(^)(Response *result))resultBlock
{
    ...
    if (self.arrayFile.count > 0) {
        for (id imgObj in self.arrayFile) {
            if (imgObj && [imgObj isKindOfClass:[ModelImage class]]) {
                [arrFile addObject:[(ModelImage*)imgObj getDictionary]];
            } else if (imgObj && [imgObj isKindOfClass:[NSString class]]) {
                [arrFile addObject:imgObj];
            }
        }
    }
    NSLog(@"\n RequestUrl: %@ \n RequestHeader: %@ \n RequestParams: %@ \n RequestFiles: %@",strUrl, manager.requestSerializer.HTTPRequestHeaders, self.parameters, arrFile);
#endif
    if (self.arrayFile.count > 0) {
