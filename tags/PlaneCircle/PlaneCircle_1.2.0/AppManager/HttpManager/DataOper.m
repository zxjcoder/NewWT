...
+(void)postErrorReportWithMsg:(NSString*)errorMsg resultBlock:(void(^)(void))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock
{
    @try {
        ...
        if (errorMsg) {
            [dicParam setObject:errorMsg forKey:@"error_msg"];
        }
...
+(void)getAppConfigWithAppVersion:(NSString *)appVersion resultBlock:(void(^)(ModelAppConfig *result))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock;
{
    @try {
        ...
        [self postJsonWithAction:@"share/appversion/queryAppVersion" postParam:dicParam postFile:nil resultBlock:^(Response *result) {
            if (result.success) {
                NSDictionary *dicResult = [result.body objectForKey:@"result"];
...
//                        NSMutableArray *arr = [NSMutableArray array];
//                        for (NSDictionary *dic in arrR) {
//                            [arr addObject:[[ModelCollection alloc] initWithCustom:dic]];
//                        }
//                        resultBlock(arr, result.body);
//                    } else {
                        resultBlock(nil, result.body);
 ...
 *  @result errorBlock  错误回调
 */
+(void)getMyAnswerWithUserId:(NSString *)userId pageNum:(int)pageNum resultBlock:(void(^)(NSArray *arrResult, NSDictionary *result))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock
{
    @try {
        NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
        if (userId) {
        ...
        [self postJsonWithAction:@"share/user/myReply" postParam:dicParam postFile:nil resultBlock:^(Response *result) {
            if (result.success) {
                if (resultBlock) {
                    NSArray *arrR = [result.body objectForKey:@"myReply"];
                    if (arrR.count > 0) {
                        NSMutableArray *arr = [NSMutableArray array];
                        for (NSDictionary *dic in arrR) {
                            [arr addObject:[[ModelQuestionMyAnswer alloc] initWithCustom:dic]];
                        }
                        resultBlock(arr, result.body);
                    } else {
                        resultBlock(nil, result.body);
 ...
 *  @result errorBlock  错误回调
 */
+(void)getMyAnswerCommentWithUserId:(NSString *)userId pageNum:(int)pageNum resultBlock:(void(^)(NSArray *arrResult, NSDictionary *result))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock
{
    @try {
        NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
        if (userId) {
 ...
 *  @result errorBlock  错误回调
 */
+(void)getCircleTopicListWithUserId:(NSString *)userId pageNum:(int)pageNum resultBlock:(void(^)(NSDictionary *result))resultBlock errorBlock:(void(^)(NSString *msg))errorBlock
{
    @try {
        NSMutableDictionary *dicParam = [NSMutableDictionary dictionary];
        if (userId) {
