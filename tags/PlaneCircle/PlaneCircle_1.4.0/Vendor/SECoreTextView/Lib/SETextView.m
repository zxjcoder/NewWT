                                    ...
                                    font:(UIFont *)font
{
    ...
    CTParagraphStyleSetting setting[] = {
        ...
        { 10, sizeof(lineSpacing), &lineSpacing },
...
{
    if (self.textLayout.textSelection) {
        NSMenu *menu = [[NSMenu alloc] initWithTitle:kEmpty];
        [menu addItemWithTitle:NSLocalizedString(@"Cut", nil) action:@selector(cut:) keyEquivalent:kEmpty];
        [menu addItemWithTitle:NSLocalizedString(@"Copy", nil) action:@selector(copy:) keyEquivalent:kEmpty];
        [menu addItemWithTitle:NSLocalizedString(@"Paste", nil) action:@selector(paste:) keyEquivalent:kEmpty];
        [menu addItem:[NSMenuItem separatorItem]];
        [menu addItemWithTitle:NSLocalizedString(@"Select All", nil) action:@selector(selectAll:) keyEquivalent:kEmpty];
...
- (CGRect)caretRectForPosition:(UITextPosition *)position
{
    ...
    if (text.length == 0) {
        ...
        UIFont *font = self.font ? self.font : [ZFont systemFontOfSize:[UIFont labelFontSize]];
