...
{
    [self.viewSearch setViewShowKeyboard];
}
        ...
        switch (model.type) {
            case ZBannerTypePractice:
            {
                ...
                if (modelP) {
                    ...
                    [sns getSpeechDetailWithSpeechId:model.code userId:[AppSetting getUserDetauleId] pageNum:1 resultBlock:^(NSDictionary *result) {
                        GCDMainBlock(^{
...
-(void)setPublishQuestion:(NSNotification *)sender
{
    ...
    if (modelQ && [modelQ isKindOfClass:[ModelQuestion class]] && [modelQ.ids integerValue] > 0) {
        ...
        if (self.lastSearchContent.length > 0) {
            [self setSearchWithContent:self.lastSearchContent];
        }
    }
}
///顶部内容区域切换
