...
-(void)textFieldDidChange:(NSNotification *)sender
{
    ...
    if (content.length > 0) {
        ...
        [sns getSearchUserWithUserId:userId questionId:self.modelQB.ids flag:@"1" content:content pageNum:1 resultBlock:^(NSDictionary *result) {
            GCDMainBlock(^{
...
-(void)setRefreshFooter
{
    ZWEAKSELF
    NSString *userId = [AppSetting getUserDetauleId];
    [sns getSearchUserWithUserId:userId questionId:self.modelQB.ids flag:@"1" content:self.lastContent pageNum:self.pageNum+1 resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            weakSelf.pageNum += 1;
...
-(void)btnInvitationClick:(ModelUserInvitation *)model itemTVC:(ZInvitationItemTVC *)tvc
{
    ...
    [sns postInviteUserWithUserId:userId hisId:model.userId questionId:self.modelQB.ids resultBlock:^(NSDictionary *result) {
        GCDMainBlock(^{
            ...
            ///编辑任务
            ModelTask *modelTask = [sqlite getLocalTaskModelWithUserId:[AppSetting getUserDetauleId] rule:ZTaskTypeInvitationAnswerQuestion];
