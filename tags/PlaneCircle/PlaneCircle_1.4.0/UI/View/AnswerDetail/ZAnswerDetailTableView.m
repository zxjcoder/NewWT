...
{
    if (row < 0) {
        [self scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:2 inSection:0] atScrollPosition:(UITableViewScrollPositionBottom) animated:NO];
    } else {
        [self scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:row inSection:1] atScrollPosition:(UITableViewScrollPositionBottom) animated:NO];
    }
}
///设置数据集合
...
-(void)setViewDataWithCommentArray:(NSArray *)arrComment modelAnswer:(ModelAnswerBase *)modelAnswer modelDefaultComment:(ModelAnswerComment *)modelDefaultComment isHeader:(BOOL)isHeader  isScrollToRow:(BOOL)isScrollToRow
{
    @synchronized (self) {
        ...
        if (isScrollToRow && modelDefaultComment && modelDefaultComment.ids.length > 0) {
            [self scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:defaultRow inSection:1] atScrollPosition:(UITableViewScrollPositionTop) animated:NO];
        }
    }
}
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0: return self.tvcQuestion.getH;
            case 1: return self.tvcUser.getH;
            case 2: return self.tvcContent.getH;
            default: return self.tvcQuestion.getH;
        }
    }
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            case 0: return self.tvcQuestion;
            case 1: return self.tvcUser;
            case 2: return self.tvcContent;
            default: return self.tvcQuestion;
        }
    }
