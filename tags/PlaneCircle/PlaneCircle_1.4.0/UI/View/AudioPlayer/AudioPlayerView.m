...
-(void)innerInit
{
    ...
    self.progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(2, 15/2, self.width-4, 1)];
    [self.progressView setProgress:0.0f];
    [self.progressView setProgressTintColor:RGBCOLOR(209, 209, 209)];
    [self.progressView setTrackTintColor:RGBCOLOR(235, 235, 235)];
...
//    [self.btnComment setTitle:kMyComment forState:(UIControlStateNormal)];
...
{
    if (playerItem && _playingPracticeModel && _playingPracticeModel.ids && _playingPracticeModel.title) {
        //获取视频播放长度
        ...
        [StatisticsManager event:kEvent_Practice_List_Item_PlayTime category:kCategory_Practice value:(long)currentNSTime*1000 dictionary:dicAttrib];
    }
}
/**
 *  开始播放
...
///获取播放中的对象
-(ModelPractice *)getPlayingModel
{
    return _playingPracticeModel;
}
/**
 *  暂停播放
 */
    ...
    if (context == &AudioPlayerItemStatusContext) {
        ...
        if ([keyPath isEqualToString:@"status"]) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.progressView setProgress:y animated:YES];
                });
                if (y >= 1) {
                    // 下载文件缓存到本地
