...
-(void)innerInit
{
    ...
    [self.tvCompany setScrollsToTop:YES];
    ...
    [self.tvUser setScrollsToTop:NO];
...
-(void)setViewScrollsToTop
{
    if (_offsetIndex == 0) {
        ...
        [self.tvCompany setScrollsToTop:NO];
        [self.tvUser setScrollsToTop:YES];
    }
}
...
-(void)setViewFrame
{
    ...
    CGFloat scH = self.height-self.viewTool.height-self.viewTool.y;
    [self.scrollView setFrame:CGRectMake(0, self.viewTool.y+self.viewTool.height, viewW, scH)];
    [self.scrollView setContentSize:CGSizeMake(viewW*2, scH)];
    
    [self setViewHeaderFrame];
    
    [self.tvCompany setFrame:CGRectMake(0, 30, self.scrollView.width, scH-30)];
    [self.tvUser setFrame:CGRectMake(self.scrollView.width, 30, self.scrollView.width, scH-30)];
}
...
-(void)setViewDataWithModel:(ModelRankCompany *)model
{
    ...
    switch (model.type) {
        case 1:
            [self.viewTool setButtonText:kAccounting index:2];
            break;
        case 2:
            [self.viewTool setButtonText:kBroker index:2];
            break;
        default:
            [self.viewTool setButtonText:kLawyer index:2];
            break;
    }
    [self setViewFrame];
}
