...
-(UIToolbar *)setToolbarStyle
{
    ...
    UIBarButtonItem *lefttem=[[UIBarButtonItem alloc] initWithTitle:kClear style:UIBarButtonItemStylePlain target:self action:@selector(remove)];
    
    UIBarButtonItem *centerSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:kDetermine style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
    ...
    if (_pickerView) {
        ...
        if (_resultString) {
            ...
            if (_isLevelString) {
                ...
                _resultString=kEmpty;
                for (int i=0; i<_plistArray.count;i++) {
                    _resultString=[NSString stringWithFormat:@"%@%@",_resultString,_plistArray[i][0]];
                }
            }else if (_isLevelDic){
                if (_state==nil) {
                    NSDictionary *dicState = [_plistArray objectAtIndex:[_pickerView selectedRowInComponent:0]];
                    NSLog(@"dicState: %@", dicState);
                    _state =[dicState objectForKey:@"state"];
                    NSArray *arrCities=[dicState objectForKey:@"cities"];
                    _city=[arrCities objectAtIndex:[_pickerView selectedRowInComponent:1]];
                }
                if (_city==nil){
